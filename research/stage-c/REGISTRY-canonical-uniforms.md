...
**Date:** 2026-06-06 · **Project:** `~/projects/well-dipper` · **Lab:** `world-engine-lab.html` + `planet-lod-lab-core.js`
...
| Province weight (Stage-D) | `uProvinceWeight` (influence dial) + `provinceWeight(FEATURE_ID)` accessor | uniform float + GLSL fn | **1.0** (full provinces) | Stage-D provinces | **ALL combiners** (F1–F11/F17/F18/F22/F23 wired; the remaining fan-out domains adopt the multiply as they land) | **LIVE 2026-06-10** (workstream `stage-d-provinces-2026-06-10`, commit `3d04110`) — 3 decorrelated low-freq FBM threshold fields (`gProvince`: tectonic/volcanic/ancient, seed-tied via `uMacroOffset`, computed once per fragment by `initProvinces()`); per-feature `{field, polarity, floor}` affinities are DATA in `planet-archetypes.js` `PROVINCES`, mirrored in the GLSL if-chain (vitest drift-guard parses both). `uProvinceWeight` is now the global influence dial: 0 ⇒ legacy uniform look (regression escape hatch), 1 ⇒ full provinces. Still a multiplier, not a gate (floors keep features faintly present; frost floor 1.0 = neutral, climate not geology). Combiner signatures unchanged — the 2026-06-07 reservation paid off as designed. **Fan-out convention for new combiners: multiply by `provinceWeight(PROV_<FEATURE>)` + add the affinity row to BOTH `PROVINCES` and the GLSL chain.** | `world-engine-lab.html` (accessor above F1; consts `PROV_*`); data `planet-archetypes.js`; oracle `planet-lod-lab-core.js`; convention = index §8 |
