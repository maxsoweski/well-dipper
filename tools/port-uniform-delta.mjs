...
//                main.js:7603 `new Planet(scenePMData, pmStarInfo)`. They are RARE — MoonGenerator.js:99
...
//   src/main.js:6291-6118  (planets)
//   src/main.js:6359-6186  (planet-class moons; identical shape, pmRatio spelled separately)
...
function toSceneData(rec) {
  ...
  if (!Number.isFinite(ratio) || ratio <= 0) {
    throw new Error(`toSceneData: radius/radiusScene is not finite-positive (${rec.radius}/${rec.radiusScene}). `
      + 'main.js:7516 `const mapToSceneRatio` assumes both exist on every body it renders.');
  }
  return {
    ...rec,
...
const GAME_ONLY_WATCHED = [
  ...
  { game: 'cloudScale',         tier: 'record', why: 'planetData.clouds.scale × the toSceneData ratio (main.js:7516 `const mapToSceneRatio`, through :6299).' },
...
const UNWATCHED = [
  ...
  { game: 'lightDir',            reason: 'runtime', why: 'overwritten per frame — src/main.js:11173 `entry.planet._lightDir.copy(_sunDir)` (also :8770 `planet._lightDir`, :11202 `moon.planet._lightDir`).' },
  { game: 'lightDir2',           reason: 'runtime', why: 'overwritten per frame — src/main.js:11179 `entry.planet._lightDir2.copy(_sunDir2)` (binary companion); constructed as (0,0,0).' },
  ...
  { game: 'starPos1',            reason: 'runtime', why: 'star world position — src/main.js:11238 `pu.starPos1`.' },
  { game: 'starPos2',            reason: 'runtime', why: 'second-star world position — src/main.js:11239 `pu.starPos2.value.copy(_star2Pos)` (also :10066 planet-class moons, :10072 textured moons). ⚠ The old ref here was :11328, a comment inside _updateRenderVisuals stating these are NOT written there — a citation that read as evidence and pointed at its own negation.' },
  { game: 'shadowMoonCount',     reason: 'runtime', why: 'eclipse casters — src/main.js:11243 `if (pu.shadowMoonCount)`, through :10022, rewritten every frame.' },
  { game: 'shadowMoonPos',       reason: 'runtime', why: 'eclipse casters — src/main.js:11247 `pu.shadowMoonPos`.' },
  { game: 'shadowMoonRadius',    reason: 'runtime', why: 'eclipse casters — src/main.js:11243 `if (pu.shadowMoonCount)` block.' },
  { game: 'shadowPlanetCount',   reason: 'runtime', why: 'eclipse casters — src/main.js:11254 `if (pu.shadowPlanetCount)`, assigned at :11268 `pu.shadowPlanetCount.value = shadowPlanetIdx`.' },
  { game: 'shadowPlanetPos',     reason: 'runtime', why: 'eclipse casters — src/main.js:11258 `pu.shadowPlanetPos.value[shadowPlanetIdx].copy(inner.planet.mesh.position)` (and :10039 for the outer caster).' },
  { game: 'shadowPlanetRadius',  reason: 'runtime', why: 'eclipse casters — src/main.js:11259 `pu.shadowPlanetRadius.value[shadowPlanetIdx] = inner.planet.data.radius` (and :10040).' },
...
const CITE_SOURCES = [
  ...
  // §10-form refs in these files that were WRONG (main.js:7600/:7412 for text at :7603/:7415) and
