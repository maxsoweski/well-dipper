...
//                main.js:7752 `new Planet(scenePMData, pmStarInfo)`. They are RARE — MoonGenerator.js:99
...
//   src/main.js:6320-6118  (planets)
//   src/main.js:6388-6186  (planet-class moons; identical shape, pmRatio spelled separately)
...
function toSceneData(rec) {
  ...
  if (!Number.isFinite(ratio) || ratio <= 0) {
    throw new Error(`toSceneData: radius/radiusScene is not finite-positive (${rec.radius}/${rec.radiusScene}). `
      + 'main.js:7643 `const mapToSceneRatio` assumes both exist on every body it renders.');
  }
  return {
    ...rec,
...
const GAME_ONLY_WATCHED = [
  ...
  { game: 'cloudScale',         tier: 'record', why: 'planetData.clouds.scale × the toSceneData ratio (main.js:7643 `const mapToSceneRatio`, through :6328).' },
...
const UNWATCHED = [
  ...
  { game: 'lightDir',            reason: 'runtime', why: 'overwritten per frame — src/main.js:11367 `entry.planet._lightDir.copy(_sunDir)` (also :8953 `planet._lightDir`, :11396 `moon.planet._lightDir`).' },
  { game: 'lightDir2',           reason: 'runtime', why: 'overwritten per frame — src/main.js:11373 `entry.planet._lightDir2.copy(_sunDir2)` (binary companion); constructed as (0,0,0).' },
  ...
  { game: 'starPos1',            reason: 'runtime', why: 'star world position — src/main.js:11443 `pu.starPos1`.' },
  { game: 'starPos2',            reason: 'runtime', why: 'second-star world position — src/main.js:11444 `pu.starPos2.value.copy(_star2Pos)` (also :10095 planet-class moons, :10101 textured moons). ⚠ The old ref here was :11357, a comment inside _updateRenderVisuals stating these are NOT written there — a citation that read as evidence and pointed at its own negation.' },
  { game: 'shadowMoonCount',     reason: 'runtime', why: 'eclipse casters — src/main.js:11448 `if (pu.shadowMoonCount)`, through :10051, rewritten every frame.' },
  { game: 'shadowMoonPos',       reason: 'runtime', why: 'eclipse casters — src/main.js:11452 `pu.shadowMoonPos`.' },
  { game: 'shadowMoonRadius',    reason: 'runtime', why: 'eclipse casters — src/main.js:11448 `if (pu.shadowMoonCount)` block.' },
  { game: 'shadowPlanetCount',   reason: 'runtime', why: 'eclipse casters — src/main.js:11459 `if (pu.shadowPlanetCount)`, assigned at :11473 `pu.shadowPlanetCount.value = shadowPlanetIdx`.' },
  { game: 'shadowPlanetPos',     reason: 'runtime', why: 'eclipse casters — src/main.js:11463 `pu.shadowPlanetPos.value[shadowPlanetIdx].copy(inner.planet.mesh.position)` (and :10068 for the outer caster).' },
  { game: 'shadowPlanetRadius',  reason: 'runtime', why: 'eclipse casters — src/main.js:11464 `pu.shadowPlanetRadius.value[shadowPlanetIdx] = inner.planet.data.radius` (and :10069).' },
  ...
  { game: 'starBrightness2', reason: 'harness-blind', why: 'Planet.js:1530 `this._starBrightness2 = starInfo?.brightness2 ?? 0.0`; harness passes no starInfo.' },  /* X3 — GLOBAL DISPLAY SETTING (B2P). Not a function of THIS body, so it has nothing to say about whether the port moved a body: it is the SAME object on every material in the game, and a row for it would record one number 514 times. It is fenced by CONSTRUCTION instead — one shared uniform object means a body cannot silently disagree with its neighbours. ⛔ RIDES THIS LINE: a new line in UNWATCHED moves every cited line below :553. */ { game: 'uPosterizeLevels', reason: 'global-setting', why: 'B2P — the colour quantum, ONE shared vec2 (levels, 1/levels) across SIX game fragment programs (gas, rocky, exotic, ring, moon, belt) in three files, declared at four sites because FRAG_HEADER’s one declaration serves the three body programs. Planet.js:26 `uniform vec2 uPosterizeLevels;` declares it for the three body programs; Planet.js:1624 `uPosterizeLevels: POSTERIZE_QUANTUM,` hands the material THE object and not a copy. Global, never body-derived: no pack writes it.' },
];
...
const TIER_BY_NAME = {
  ...
  uEjectaLump: 'condition',   uCoarseCut: 'condition',   // ⭐⭐ CLASSIFIED 2026-08-27, AND INSTRUMENT C HAD BEEN DEAD SINCE IT LANDED. coarseReliefCut(condition), emitted by the rockySurface pack beside uNoiseScale. 'condition' and not 'record' because rockySurface.js:373 says it in its own words: "this name has NO legacy counterpart at all — the game has never had a term for 'how much large-scale relief was erased' … It is new physics, not a port." There is nothing for it to reach byte-identity WITH, so it is derived, not recorded. ⛔⛔ AND THIS IS THE SECOND TIME THIS EXACT FAILURE HAS HAPPENED, WHICH MAKES IT A PATTERN RATHER THAN AN ACCIDENT. uProvinceWeight killed this instrument on 2026-08-25 the same way and was repaired on 2026-08-26; uCoarseCut landed in b0c0cda during that SAME session and re-killed it, unnoticed, because resolveSharedUniforms THROWS on an unclassified name and a thrown instrument reads exactly like a passing one. Any new watched uniform must get a tier IN THE COMMIT THAT ADDS IT, or the drift measure goes blind and nothing says so.
...
const CITE_SOURCES = [
  ...
  'tests/driver-pack-polardeck.test.js', 'tests/driver-pack-giantsurface.test.js', 'tests/one-pipeline-fence.test.js', // ⛔ APPENDED TO THIS LINE, never inserted below it. Pack #8's suite joins the scanned set in the SAME commit that creates it — §11.3.4 — so its refs land in the FATAL column rather than the UNCHECKED pile, which is where B3's five tracked-but-unregistered modules hid 27 refs while every gate printed exit 0.
  ...
  // §10-form refs in these files that were WRONG (main.js:7629/:7412 for text at :7632/:7415) and
  ...
  'src/worldengine/drivers/rockySurface.js', 'docs/FEATURES/root0-seam-delta-table.md', 'tools/root0-seam-delta.mjs', 'tests/root0-seam-laws.test.js', 'src/worldengine/base/surfaceMaterial.js', 'docs/FEATURES/oxidation-window-calibration-2026-08-20.md', 'tools/oxidation-window-delta.mjs', 'docs/FEATURES/b2-differentiation-calibration-2026-08-20.md', 'docs/FEATURES/crater-floors-calibration-2026-08-20.md', 'src/worldengine/base/macroWavelength.js', 'tests/macro-wavelength-law.test.js', // ── B2 LEG 3 (the macro wavelength), REGISTERED 2026-08-21 BY THE REPAIR PASS. §11.3.4 again: leg 3's module and its law test carry 13 line-anchored citations between them and were in NO source, so the fence never opened them and gate 2's headline read as coverage it did not have — the exact Step-9a failure the note above convicts. ⛔ APPENDED ON THIS EXISTING LINE, NOT INSERTED — `const CITE_SOURCES = [` is cited at :1023 and live refs sit below it. ⭐ CHECKED MUST RISE.  // ── B2 PAPER GATES, 2026-08-20: the block's single calibration deliverable plus leg 1's, which shipped at 5afef82 UNREGISTERED and was therefore scanned by nothing. ⛔ APPENDED ON THIS EXISTING LINE, NOT INSERTED — `const CITE_SOURCES = [` is cited at :1023 and three live refs sit below it. ⭐ CHECKED MUST RISE, since BROKEN == 0 cannot tell "every ref resolves" from "no ref was read". // ── B2 LEG 2 (the oxidation window), 2026-08-20: the file the leg edits plus its calibration doc and its regenerable delta tool, all three joined in the SAME change per §11.3.4. ⛔ APPENDED ON THIS EXISTING LINE, NOT INSERTED — `const CITE_SOURCES = [` is cited at :1023 and live refs sit below it. // ── B1 (ROOT-0), 2026-08-20. §11.3.4: every file a block edits joins the scanned set in the SAME commit, not after the next round finds it. ⛔ APPENDED ON AN EXISTING LINE, NOT INSERTED — `const CITE_SOURCES = [` is cited at :1023 and three live refs sit BELOW it (:1086, :1090, :1093), so a new line here would move them. Same rule B0 recorded one line down. ⭐ The delta table is a GENERATED artifact and its refs are gated anyway: a regeneration that stales a ref now turns the fence red instead of rotting silently.
  'tests/driver-pack-rockysurface.test.js', 'docs/FEATURES/comprehensive-wiring-plan-2026-08-20.md', 'docs/FEATURES/lab-pipeline-into-game-PLAN.md', 'docs/FEATURES/review-2026-08-11-camera-api-and-packs.md', // ── B2P CORRECTOR PASS, 2026-08-20. ⛔ APPENDED ON AN EXISTING LINE, NOT INSERTED — three live refs sit BELOW `const CITE_SOURCES = [` at :1086/:1090/:1093 and a new line would move every one. ⭐ WHY THESE TWO AND NOT THE OTHER TWO CARRIERS OF THE SAME SENTENCE: four documents carried a citation attributing the game material's `uPosterizeLevels` to posterizeLevels.js:45 `export const POSTERIZE_LEVELS = { value: 31.0 };`, which is the LAB's scalar; all four are re-pointed to :55 in this pass, and adding their carriers here is what turns the repair from prose into something a gate holds. MEASURED before adding: `docs/NOW.md` brings 1 unresolved basename and `docs/FEATURES/planet-surface-lod-research-2026-08-10.md` brings 15, both pre-existing and unrelated to B2P, so admitting them would exit 2 on someone else's debt — they stay out and stay recorded here. ⭐ THE COUNT MUST RISE (a green `BROKEN == 0` cannot distinguish `every ref resolves` from `no ref was read`): measured at the close of B2P: 620, up from 613. // ── B0 item 11, 2026-08-20. ⛔ APPENDED ON AN EXISTING LINE, NOT INSERTED: this file is its own CITE_SOURCE and ~5 live refs point into it, so a new line would move them. Same reason the plan itself is edited line-count-neutral. ⭐ CHECKED must RISE (the count is the only thing that separates "every ref resolves" from "no ref was read"); measured on this change: 542 → see the run in the B0 report.
];
...
function printResolution(res) {
  ...
  console.log('                  passes — every body would record the same fallback.\n    global-setting a user DISPLAY setting held as one shared uniform object across every\n                  material; per-body sameness is a property of the wiring, not a measurement.');
  console.log('');
