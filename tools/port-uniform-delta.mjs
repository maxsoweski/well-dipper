...
//                main.js:7675 `new Planet(scenePMData, pmStarInfo)`. They are RARE — MoonGenerator.js:99
...
function toSceneData(rec) {
  ...
  if (!Number.isFinite(ratio) || ratio <= 0) {
    throw new Error(`toSceneData: radius/radiusScene is not finite-positive (${rec.radius}/${rec.radiusScene}). `
      + 'main.js:7588 `const mapToSceneRatio` assumes both exist on every body it renders.');
  }
  return {
    ...rec,
...
const GAME_ONLY_WATCHED = [
  ...
  { game: 'cloudScale',         tier: 'record', why: 'planetData.clouds.scale × the toSceneData ratio (main.js:7588 `const mapToSceneRatio`, through :6328).' },
...
const UNWATCHED = [
  ...
  { game: 'lightDir',            reason: 'runtime', why: 'overwritten per frame — src/main.js:11245 `entry.planet._lightDir.copy(_sunDir)` (also :8842 `planet._lightDir`, :11274 `moon.planet._lightDir`).' },
  { game: 'lightDir2',           reason: 'runtime', why: 'overwritten per frame — src/main.js:11251 `entry.planet._lightDir2.copy(_sunDir2)` (binary companion); constructed as (0,0,0).' },
  ...
  { game: 'starPos1',            reason: 'runtime', why: 'star world position — src/main.js:11310 `pu.starPos1`.' },
  { game: 'starPos2',            reason: 'runtime', why: 'second-star world position — src/main.js:11311 `pu.starPos2.value.copy(_star2Pos)` (also :10095 planet-class moons, :10101 textured moons). ⚠ The old ref here was :11357, a comment inside _updateRenderVisuals stating these are NOT written there — a citation that read as evidence and pointed at its own negation.' },
  { game: 'shadowMoonCount',     reason: 'runtime', why: 'eclipse casters — src/main.js:11315 `if (pu.shadowMoonCount)`, through :10051, rewritten every frame.' },
  { game: 'shadowMoonPos',       reason: 'runtime', why: 'eclipse casters — src/main.js:11319 `pu.shadowMoonPos`.' },
  { game: 'shadowMoonRadius',    reason: 'runtime', why: 'eclipse casters — src/main.js:11315 `if (pu.shadowMoonCount)` block.' },
  { game: 'shadowPlanetCount',   reason: 'runtime', why: 'eclipse casters — src/main.js:11326 `if (pu.shadowPlanetCount)`, assigned at :11340 `pu.shadowPlanetCount.value = shadowPlanetIdx`.' },
  { game: 'shadowPlanetPos',     reason: 'runtime', why: 'eclipse casters — src/main.js:11330 `pu.shadowPlanetPos.value[shadowPlanetIdx].copy(inner.planet.mesh.position)` (and :10068 for the outer caster).' },
  { game: 'shadowPlanetRadius',  reason: 'runtime', why: 'eclipse casters — src/main.js:11331 `pu.shadowPlanetRadius.value[shadowPlanetIdx] = inner.planet.data.radius` (and :10069).' },
