...
> ## ⭐ 2026-09-03 (session 4, later) — **F3 EJECTA RAYS = `VERIFIED_PENDING_MAX 6d70d56` (verdict `2e60453`: unit PASS · integration PASS · AC-7 = Max) · F35 TERMINATOR = built, merged (`eb58ac9`), live-measured (`15ea92c`), `verifying` (full verify running at `2e60453`).** ▶ **MAX'S WALKS:** (1) rays — `?system=rocky-13`, the FIFTH planet's ice moons ("f IV" 1.06 R⊕, "Dione" 0.96 R⊕), key **`Y`** while moving in, then any planet with air and `Y` again (nothing changes); (2) terminator — `?system=rocky-2`, the second planet at a crescent, key **`.`**: the band appears (the law value) and vanishes (what ships, OFF, his 2026-07-16 ruling). Overturned this session: the rays' input is LIVE on every plain moon (the plan's 100 %-hasAtmo claim was planets-only); the game DREW the terminator band he turned off in the lab. Instrument A re-recorded at `eb58ac9` (its old record never covered `src/**/__tests__`; ⚠ nested worktrees double a bare vitest run — ALWAYS `--dir tests`). Records: PLAN § THE EJECTA RAYS, WIRED and § THE TERMINATOR, CONVERGED (EOF); backlog QB-25..28.
>
