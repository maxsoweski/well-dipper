...
> **Feature-association manifest — Tier-1 landed (2026-06-14):** `planet-feature-associations.js`
> got its cross-source (vs-shader) test tier + the grounded defect fixes from the functional audit
> (`docs/superpowers/plans/2026-06-14-manifest-functional-audit.md`). `modifies` is now DERIVED
> from `dependsOn.features` (single source — Max's Decision 1); massWasting deps completed to all 20
> grad-writers; lakes added to frost/dust/sunglint/cityLights (liquidMask readers) + sunglint kit;
> spurious lakes→rivers deleted; hexTess flagged `rendersOnDivergent` (Decision 2). 11 green + 1
> skipped (driver-validity Claim 8 DEFERRED — Decision 4). **Tier-2 (GPU render-delta sweep / Phase
> 2.5) is the next step, pending Max's Decisions 3 & 4.** Handoff `/tmp/handoff-manifest-tier1-fixes-2026-06-14.md`.
