...
> ## 🔧 2026-09-02 — ⭐⭐ **THE RIVER ROUTER IS BUILT AND FENCE-GREEN** (F11/F12), at `a737ffd` (verdict.json) + the close-out docs commit. ▶ NEXT = **THE LIVE PAIR** (contract AC-4 relief A/B, AC-5 rivers A/B) — it needs lane A's dev server, and **nothing in this wire has been seen in a browser yet**, the lab included.
>
...
> On `feature/world-engine-production-L1` (lane A, `~/projects/well-dipper`), **PUSHED 2026-09-02 (origin at `08946b7` + the handoff commit)**. The province cube is HISTORY now: shipped `ccee0d1`, Max UAT 2026-09-02 — *"it does read as a crust and coheres"*.
...
> **Corpus (24 `rocky-*` seeds, 156 bodies, 124 solid / 32 gas):** wet **4** · relict **64** · airless **56**; routed = wet ∪ relict = **68**. On every wet body: 0 orphan nodes, 0 uphill receivers, 0 undrained channels, ocean fraction 0.347–0.354 against a 0.35 target, max Strahler 3–4 and R_b 9.6–13.0. VRAM 57.3 MB per ROUTED body + 7.0 MB per further solid one (derived, not measured) — and **routed is wet ∪ relict**, 3.24 of them per routing system (max 5), so **≈ 204 MB mean and ≈ 300 MB worst per system**, not the 57 MB the first draft implied.
...
>
> **Gates:** `river-bake-host` + `driver-pack-fluvialdeck` suites green · B 8/8 · C 0 of 57 over 633 bodies (⚠ the fluvial family is outside C's basis — expected, NOT evidence) · citations 858/858, PAST-EOF 3→0, BROKEN 8→0 · boundary / one-pipeline / sampler fences green. **Instrument A: 20 failing / 8 files, IDENTICAL on a clean worktree of `2e39f36` and on HEAD** (set-difference empty; the suite grew 3695 → 3765 tests). **`verdict.json` at `a737ffd`: AC-0/1/2/3/7 PASS; AC-4/AC-5 (+ AC-6's lab-in-Chrome clause) pending the live drive.**
>
> ### ⭐ OPEN FOR MAX
> 1. Start the dev server — `scripts/dev.sh` (`cd ~/projects/well-dipper && npm run dev -- --port 5175`. Chrome:9223 is launched by Claude. **Nothing else here can move until it is up.** Handoff: [`FEATURES/handoff-2026-09-02-river-router-built-live-pair-next.md`](FEATURES/handoff-2026-09-02-river-router-built-live-pair-next.md) (prior: `handoff-2026-09-01b-province-cube-wired-live-check-next.md`).
> 2. Then, once the pair is driven: fly toward a wet rocky world in a `rocky-*` system and tap **`J`** and **`U`** *while moving* — **"do the rivers sit in their own valleys and drain into a sea that looks right, with deltas at the mouths?"** That is AC-8, it is yours alone, and no agent passes it. ⭐ **GO TO `rocky-14` AND ITS MOON.** Of the four wet bodies in the whole 24-seed corpus it is the ONLY one inside the relief A/B's size window (0.95 R⊕; the other three are 0.35–0.45 R⊕ and their relief crossover barely moves), so it is the one body where **both keys say something on the same world** — measured 2026-09-02.
> 3. Three calls that are yours, all recorded and none of them blocking: **QB-21** — every wet world gets the same 35 % ocean, because the router's target is a lab UI constant; **QB-23** — F13's outflow ramp saturates, so 60 of 64 relict worlds render a megaflood; and **only 4 of 124 corpus bodies come out wet** under the lab's own gate — is ~3 % the density you want rivers at? ⭐ **And a fourth, which is a phone question:** this now costs **about 200 MB of cube maps per system on average, up to about 300 on the worst one in the corpus** — the first figure written down was ~57 MB and it was scoped to the wrong class of body. Your desktop will not notice; **the phone is the only instrument** and nothing else can answer it.
>
