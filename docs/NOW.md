...
> (3) **Bucket B (driver tightening) — SPEC'd, NOT built.** Genuine leaks (silicate-relief on icy
> worlds, dust on gas giants, weatherBands on Titan) → tighten drivers. Design + exact sites in the
> handoff + verdict doc. Then re-run `renderDeltaSweep()` + regen audit (counts are pre-fix/stale).
> (4) **Max's bigger ask = MENU/INFO OVERHAUL** (his goal #3): relevance indicators + per-feature info
> cards + archetype info view + auto-correct mismatches + a USABILITY/QA pass (menus too complex, dedup
> archetype-selector vs indicator). Sequence Max set: band✅ → triage(B remaining) → menu. Handoff doc
> `/tmp/handoff-lod-triage-round3-2026-06-15.md`.
