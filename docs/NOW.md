...
> ## ⛔⛔ 2026-09-04 (session end) — **F1 MOUNTAINS ANSWERED, AND IT IS A GALAXY-WIDE GENERATION DEFECT.** The plate path claims 0 bodies because **the generator NEVER makes a temperate WET world** — over **1,183 solid bodies / 200 seeds the temperate and wet sets do not intersect once**, missing by a wide margin both ways (wettest temperate V=0.0595 vs the band's 0.12; warmest wet T=186 K vs the band's 250 K). **Root cause, one law:** `PhysicsEngine.deriveComposition` makes volatiles a pure function of `orbitAU / frostLineAU` while `T_eq` is a function of the SAME variable inverted, so *temperate ⇒ dry* holds **by construction** — **this law would generate Earth itself at V ≤ 0.06.** ⭐⭐ **AND THE SEAM IS ENGINE-WIDE:** the engine anchors Earth at **0.15** in three places (`passiveMargins.js:54`, `labCore.js:693`'s bone-dry floor, the `Rocky (Earthlike)` preset); through the engine's OWN gate, **of 135 temperate bodies 78.5 % are at or under the bone-dry floor, 21.5 % essentially dry, ZERO wet**, while 26.7 % of all solid bodies DO read wet and every one is frozen. **Bimodal galaxy: hot deserts and cold ice, nothing between** — and the same field feeds the fluvial stack, karst, dunes, dust, margins AND the plate gate. Second independent block: `if (locked)` sits ABOVE both roads to `plate()` and eats **74 %** of bodies. Record: `WORKSTREAMS/f1-mountains-generation-2026-09-04/REPORT.md` (+ its ADDENDUM) and three re-runnable scripts.
>
> ▶ ⭐⭐ **MAX'S RULING, which WIDENED THE CHARTER'S INTENT FRAME:** *"yes; I want this to be a simulation of the milky way galaxy with a wide variety of physically-plausible worlds"* — the 2026-07-19 frame with emphasis moved from fidelity PER FEATURE to **variety across the drawn population**, plus a new test: **a generation law is wrong if it makes a whole class of physically-real world unreachable.** Recorded at `FEATURES/planet-lod-CHARTER.md` § INTENT FRAME. **THE FIX'S SHAPE IS DERIVED:** `deriveComposition` does two jobs with one field — accreted bulk ice (frost-line driven, correct today) and SURFACE volatile inventory, which inside the frost line is **delivered late** from scattered outer-system material and is decoupled from the body's own frost ratio. Adding that delivery term is the job. ⛔ **NEW MULTI-SYSTEM WORKSTREAM — `dev-collab-scope` FIRST**, parent population captured BEFORE any edit, acceptance on the DRAWN population never a preset. **Max chose to scope it in a FRESH SESSION.** Handoff: [`FEATURES/handoff-2026-09-05-volatile-delivery-then-lighting.md`](FEATURES/handoff-2026-09-05-volatile-delivery-then-lighting.md). Then **(3) the lighting engine / F52**, still third.
>
