...
>
> **Serves the 35% SCREENSAVER MVP milestone, SCREENSAVER tier:** the nav computer is the pilot's instrument for choosing where the ship goes, and a menu that cannot say what a thing IS is a defect against that tier's target experience.
>
> **GOAL → WHERE WE ARE.** The 2026-09-18 audit ranked what the two 240p menus LOST against the old nav. Its twelve defects were built that day; its eleven restorations waited for his review. He approved the recommendations, and workstream **`nav-restorations-2026-09-20`** (`intent.md` + `contract.json` + **`SEAM.md`**, 11 ACs, two waves) built nine of them in four workflow runs (LAB / DRIVER / HOST owners on disjoint files + an integrator each, opus/high) plus one fixup agent — ~3.6 M subagent tokens, ~6 h wall-clock.
>
> ⭐ **WHAT THE NAV CAN DO NOW.** Hover anything on any screen in either design and a callout names it with the old nav's facts (sector name; tile kpc; star name · class · pc/ly · height; planet type · R⊕ · AU · K · moons; the star's class · R SUN · age · planets). Design 1's prism has drop lines, a 1 pc plane lattice (doubling its cell as the camera pulls back), far-to-near drawing, spectral ink, a halo on catalogue stars and NAMES where the eight-digit seeds were. Both designs print HEIGHT with its disk region, PLAYER Y, Y RANGE and one `VIEW <n> LY` (legacy's own radius integer). The ladder and the orrery name their bodies (selected > ship > belt > AU; a name that would collide yields to its tag); belts read ASTEROID BELT / KUIPER BELT; with nothing selected the star line (`G2 · 8 PLANETS · 4.6 GYR`) prints where NO BODY SELECTED was. The ship diamond stands on the body the game is at, with a dashed trajectory and chevron to the hovered-or-selected body; nothing in a foreign system. Design 2's orrery zooms with the wheel and a draggable zoom gauge; design 2's prism has the y-gauge (one shared `yGauge()` publishes the rect for both designs). A second click on a selected planet with moons opens a design-side sub-view — a moon ladder (design 1) or a moon orrery (design 2) — where a moon click arms `target:'moon'` for the burn `main.js:5994` already consumes; right-click or an empty click steps back out; in design 1 a moon's rail row selects it directly. Legacy is byte-identical at every level, fresh and after a V cycle.
>
> ⭐⭐ **FOUND ON THE WAY.** (1) A real **Esc never reaches `handleEscape`** — `_onKeyDown` has no Escape clause, main.js closes the whole nav (Max, 2026-07-29: *"esc should just dismiss"*, guarded by `NavComputer.escape.test.js`); so the sub-view's glass says RIGHT CLICK BACK and whether Esc should step back first is HIS call. (2) Under a design a **foreign-system planet click arms no selection** (`NavComputer.js:4589-4592`), so the moon sub-view is current-system only — follow-on, HOST. (3) `CALLOUT_GAP` at 6 put the plate's edge on the mark and silently hid every 2D callout; 7. (4) Design 2's PRISM bar was already overflowing before this work (`R/F UP` had never been visible) — its star count and key hints moved to a legend row. (5) The sub-view's rail rows resolved to planets 0,1,2,2,3 until `pickFromRow` read the DRAWN rows (`S.railBodies`).
>
> **HEALTH:** `src/ui` **922 passed / 44 files** at `--testTimeout=60000` (was 767/35; at vitest's 5 s default ~17 contention timeouts, green alone), `src/cockpit` 698, `tests/` 20 failed across the same 8, `extract --check` 0, `NavComputer.js` 4711 (five folds: 156 / 1441 / 4355 / 4405 / 4415), `main.js` 15161 untouched. Every fix pinned by a test proved red on a named mutant. **VERIFY-WORKSTREAM (full, 43 agents) at `ab610fa` → `verdict.json`: unit 6/7 PASS by consensus (AC-1/3/7/8/9/10); AC-2 INSUFFICIENT 0/3 because its observable still said "1 pc spacing" while the lattice cell doubles on zoom-out and no test asserted spacing — the observable is rewritten to the measured rule (`0da6b15`) and a spacing test follows; AC-4/5/6 INSUFFICIENT only for want of a browser; AC-11 deferred-to-max.** A re-run at HEAD would flip AC-2 (~45 agents; not spent — Max's call). ✅ **LIVE-CHECKED 2026-09-21** (Max started the server; one browser agent, 237 tool calls): every AC's observable held in the running game — frame time at design 2's widest prism 13 ms mean / zero frames > 33 ms (672 marks); callouts at all five levels inside their panes; the real wheel ×1.15/×0.87 to both clamps and a real drag on both gauges without rotating the picture; the ship on the moon the game is at, gone in a foreign system; a moon burn reaching the game (`[BURN START] moon … Mimas` in HELM; inert in ORRERY by the game's own rule). AC-1..10 → `VERIFIED_PENDING_MAX 0100fd5`; the page has an **"After the restorations"** section with sixteen captures (artifact republished). ⚠ Max's window gave a **480×240** buffer today, not 417. **EIGHT LIVE FINDINGS**, numbered 24-31 on the page: 24 Sol's planets carry GENERATED names (CADEIN, AZCAI) where the game says Mercury/Venus/Earth, and the set changes with the arrival path (state.js seeds the name RNG from `star.seed`, which is `'Sol'` or the hash depending on how you arrived) — prefer `_knownSystemNames`; 25 design 2's stems run through its own hint rows at the widest zoom; 26 `0.0 GYR` / bare `G` are absent-data artefacts printed as facts; 27 ladder names on one row run together; 28 mouseleave does not clear the callout; 29-31 his eye (unlabelled ship at zoom 1, the callout's doubled name, the crowded moon orrery centre).
>
> ▶ **FOR MAX** (numbered): **1** start the lane-A server (`cd ~/projects/well-dipper && npx vite --port 5175`) so the live checks and your pictures can happen. **2** Esc inside the moon sub-view: step back one screen, or keep closing the whole nav as you asked in July? (recommend keep; one host fold flips it). **3** push lane A (`8a62ad0` → `ab610fa`, seven commits)? (recommend yes). **4** carried: the two 09-18 decisions (ladder in AU order under every sort; the trimmed tab-strip legend) — recommend keep. **5** carried: the review's leftover defects — recommend go on three (a painter throw freezing the nav; design 2's PRISM map-mode status losing `R/F UP` — NOTE wave 1a's legend row may have absorbed this, re-measure; the sector click drilling a double-size tile), park the rest.
>
> ▶ **HANDOFF → `docs/FEATURES/handoff-2026-09-20-nav-restorations.md`.** Skill `wd-browser-up` is the bring-up.

> ## ▶ 2026-09-18 — **THE NAV AUDIT: THE OLD MENU MAPPED BESIDE THE NEW ONE, A 37-FINDING CODE REVIEW, AND 22 DEFECTS BUILT, VERIFIED AND LIVE-CHECKED. ▶ MAX'S REVIEW OF THE ELEVEN RESTORATIONS IS THE GATE.** Lane A, `48159e6` + this entry (unpushed since `783e32b`). Max resumed with three asks — a code review, the old menu mapped like the new one and compared, and a full click/key audit of the new menus (*"the 3D prism appears too simplistic to read now"*, *"the 2D system menu doesn't really work intuitively; it's hard to scroll and get info"*).
>
> **GOAL → WHERE WE ARE.** The two 240p nav menus were functionally complete (close pass, 09-08) but had never been compared to what the old nav could do. Now they have: **Old Nav vs New Nav** (artifact https://claude.ai/artifact/F8Ut2bLQbK4hh5V8TJ27UY, `docs/FEATURES/nav-menu-map-old-vs-new.html`) maps the old nav in the walk sheet's format from live captures, puts every screen beside both designs, ranks what was lost (hover-inspect everywhere, planet detail with moons, the prism's drop lines and plane grid, names, height numbers, ship position, SYSTEM wheel zoom) and gained (keys, eases, lists, search, handles), measures the two things he named (16 marks and ≤1 name on the prism at his window; Sol's ladder FITS so nothing scrolls, the wheel is dead, the selection never cleared), tabulates the same input across screens, and ends in **23 numbered items: 1-12 defects, 13-23 restorations**. His ruling: *"1 yes (use subagents/workflows) 2 I need to review first 3. Yes"*.
>
> ⭐ **WHAT MOVED.** Workstream `nav-defects-batch-2026-09-18` (22 ACs): the commit row/chip click commits on every screen; a real no-selection state (and a star the frame follows); 2D pans move 1:1; a press on chrome arms nothing; keys act only where they draw; legacy's cameras survive a V cycle; right-click goes back in every look (real secondary button through CDP, measured); the × is on screen in legacy and the designs print ESC CLOSE; the locator is eaten at SYSTEM; multiplicity is filled on every star row (5,577 multiples on Sol's prism); the tab/commit one-texel overlap is gone; the hidden keys are printed. From the review (37 confirmed / 3 refuted / 13 nits, `REVIEW-2026-09-18.md`): a drag released over a control no longer fires it (a pan could start a warp), closing the nav resets its session state, the ladder snapshot is released, V drops legacy sub-views and ends gestures, row hit bands match the drawn rows, design 2's status fits, 2D markers stay inside the map, one letter per body, chords stay out of the search and the look. Built by three/four owner agents on disjoint files plus an integrator, twice, zero conflicts; every fix pinned by a test proved red on a mutant.
>
> ⭐⭐ **FOUND LIVE ON THE WAY, AND FIXED:** the designs' "am I home" test compared Sol's string seed with the star row's number, so in Sol both designs armed the chip off the FOREIGN branch with nothing selected and design 2 said WARP. `D.isCurrent` now mirrors the host's `_isCurrentSystem()`; the harness stands the pilot on the system star so "home" means what the host means.
>
> **HEALTH:** `src/ui` **767 passed / 35 files** (was 630/29; a full run overlapping the verify workflow's vitest showed 13 five-second timeouts that vanish alone), `src/cockpit` 698, `tests/` 20 failed across the same 8, `extract --check` green, `NavComputer.js` 4711, `main.js` 15161. Legacy fresh-open frames byte-identical to HEAD's (text stream at all five levels). ⚠ AC-21 is deliberately partial: the WASDRF clause still eats Ctrl+W/Ctrl+R while the nav is open. ⚠ verify-workstream FULL ran against `b3f7737`, one commit before the home-test fix.
>
> ▶ **FOR MAX** (numbered): **1** push the lane-A commits since `783e32b` (recommend yes). **2** the page's "After the fixes" pictures and two decisions — the ladder is now sort-independent (AU order, so its letters match the rail), and design 1's tab-strip level count plus two hint clauses were cut to fit the legends. **3** items 13-23 on the page, yes / no / park by number; 13 (hover-inspect) and 14 (the prism's drop lines and grid) first.
>
> ▶ **HANDOFF → `docs/FEATURES/handoff-2026-09-18-nav-defects-batch.md`.** Skill `wd-browser-up` is the bring-up.

> ## ▶ 2026-09-08 (later) — **THE NAV WALK SHEET IS BUILT: EVERY SCREEN IN ALL THREE LOOKS, EVERY CONTROL, WITH A PASS/FAIL CHECKLIST HE FILLS IN.** Lane A, `d3db23f` (NOT pushed). Max's first word this session was not the defect list the handoff asked for but *"create a visual guide to the menu system (it can be simple but must explain all functions of each screen), so I can test it."* Built and published.
>
> **WHAT IT IS.** `docs/FEATURES/nav-menu-guide.html` (served at `/well-dipper/docs/FEATURES/nav-menu-guide.html`) and the artifact **https://claude.ai/code/artifact/1dd706c5-3d98-490a-b508-1ad689c332de**. 19 live captures from `:5175` in Sol (design 1, design 2, design 2's list, legacy, both searches, the GALAXY click mid-drill), numbered pins on every region, and 70 rows over GALAXY/SECTOR/REGION/PRISM/SYSTEM/search/between-screens: element → do this → what should happen → status (MAX SIGNED OFF / YOUR EYES / INERT / KNOWN GAP / LEGACY). Each row has ✓ ✗ + a note, saved to the artifact db (`walk/results`) so the next session reads his defects back instead of asking. Asks 2-5 are on the sheet under "Rulings still open".
>
...
>
> **GOAL → WHERE WE ARE.** Max's eight items on the two 240p nav menus: seven were closed by 2026-09-07; the eighth ("anything on screen should be clickable") had five rectangles left, plus the GALAXY half of the click-highlight, the inbound half of the lost animation, and the SYSTEM counter. All of those are now on the glass: the rail's pager row pages by halves; design 2's list headers sort the list (five keys, two new); `HERE · SECTOR` re-centres on the player at four of five levels; the companion strip stops a press falling through; a GALAXY click frames its SECTOR and the map zooms into it; a tab into PRISM or SYSTEM eases instead of cutting; the `N-M OF K` counter is a scrubber. Built by three owner agents on disjoint files to INTERFACE §8, integrated, then put through a 14-agent adversarial pass and a full live walk on `:5175`.
>
> ⭐⭐ **WHAT THE VERIFICATION CHANGED, PLAINLY.** Every builder mutant reproduced red, and every slice was still "refuted as written": the claims in the comments and the interface were wider than the tests pinned, because each rectangle's tests took their probe point from the field under test. Two findings were pilot-facing and are fixed: the minimap band was eating presses on bare glass (the widget is brackets over a visible starfield, not a plate — a hoverable star went dead; **withdrawn**), and a press in the tab strip past the last tab cancelled a running ease and cut (now only a real tab cancels). The rest were test gaps, closed with absolute paint-derived assertions (34 → 71 cases). Two of my own claims were false and now say so: "Sol keeps its `...` cap" (Sol FITS at Max's window — no cap, no counter, no scroll) and "the pager is absent while the search is open" (true, but nothing had pressed it).
>
> ⭐⭐ **AND THE LIVE WALK FOUND THE BIGGEST ONE: GALAXY WAS DRAWN FROM A FIXED DISC.** `levelView(0)` returned `{0, 0, 44}` regardless of the game's frame, so the host's own 2D pan at GALAXY moved `_viewCenter` with a byte-identical canvas (design 2's 20 cropped sectors had no way to be reached), the locator's level-0 re-centre eased a field nothing drew, and a GALAXY drill CUT to SECTOR after 500 ms — the "44 → 0.5 walk" measured a day earlier was the field, not the picture. One line: level 0 reads `S.view` like levels 1-2. Entry frame unchanged (`_viewStack[0]` IS that literal). Live after the fix: a 60-texel drag moved design 2's band (hash changed, cx −16.5), the locator eased it back onto the player (cx 8.000), and the drill closes on the sector (size 36 → 22.5 over the first 250 ms with the highlight held).
>
> ⛔ **FOUND AND FIXED ON THE WAY:** `d1Ladder` padded its axis `+8` where `vpx` already carries a 4-texel margin, so EVERY ladder overflowed by exactly four texels — a `...` cap promising nothing and a `1-N OF N` counter on every system. Now `+4`. Consequence: on Sol at Max's window there is no cap, no counter, and the hint no longer names `SCROLL , . OR CLICK ...` — AC-11's one recorded exception. A tap with no preceding hover framed a sector and did not drill (the drill read the previous frame's hover); the click now resolves hover at its own point first.
>
> **HEALTH:** `src/ui` **630** (was 551 at session start), `src/cockpit` 698, `tests/` 20 failed across the same 8 files, `--check` green, `NavComputer.js` 4711, `main.js` 15161. Live checks A–H (first walk) and the six re-checks after the correction all pass with controls. ⚠ The first prism entry's frames are ~450 ms (the synchronous star load), so the 350 ms ease stretches once; warm it is clean. ⚠ At a 227-wide buffer design 1's SYSTEM status row collides and the guard fires ~240×/s — a narrow-window layout defect, not present at Max's size, logged.
>
...
>
> ⛔⛔ **AND I TOLD HIM THE WRONG CAUSE ONCE, WHICH IS WORTH MORE THAN THE FIX.** I said design 2's numerals were overlapping EACH OTHER. Measured on Sol and three deliberately clustered systems, 120 frames each: label-on-label damage was **ZERO** — the placer already dropped rather than smeared. They were **sitting on the planets**, and because `plated()` knocks out a BG rect, each numeral was ERASING the dot it covered — which from the pilot's seat reads exactly like overprinting. It was AC-14 wearing a disguise, and it was also a live **mis-selection**: clicking a numeral selected whatever mark its plate had hidden. **4,854 → 0** foreign-mark hits, **507 → 0** erased glyphs, and **more** labels drawn than before (7,155 vs 6,909) because the placer can now go left as well as up and down.
>
> ⚠ **`review.html` is the instrument for a picture ruling** — three flip A/Bs served off the running dev server at `/well-dipper/docs/WORKSTREAMS/nav-screens-close-pass/review.html`. A flip beats side-by-side for the same reason a bare-key A/B in the live build beats a screenshot.
>
> ⭐ **THE TWO QUESTIONS THE HANDOFF SAID TO SETTLE FIRST BOTH CHANGED THE PLAN.** The drill zoom was **already back** — a real GALAXY click walks `S.view.size` 44 → 0.5 and lands at SECTOR — so what was missing was only the *highlight before* it, plus every transition touching PRISM or SYSTEM, which snap (`:4450` gates `_startDrillAnim` on `idx <= 2 && _levelIndex <= 2`). And the second-selection freeze was **none of the three suspects**: the sim clock ran at 1000 ms/s with the tab foregrounded, so the throttled-tab artifact never fired. Selecting a planet **that has moons** sets `_systemMode='planet'`, and that branch writes `_selectedPlanetIdx` instead of `_hoveredBody.index` — correct for legacy, whose `_renderPlanetDetail` draws ONE planet, and wrong the moment the designs keep all fifteen on the glass. Fixed by **pinning the mode under a view mode**, so legacy stays byte-identical.
>
> ⭐⭐ **ROTATION IS WIRED AND THE RULED PICTURE IS PROVEN UNMOVED.** At the default angle **64,000 prism projections and 20,000 orrery bodies are BIT-IDENTICAL** to the old picture by `Object.is` on the raw floats — and the probe is live, because `rotY = 1e-9` and `rotX + 1e-12` both diverge. `projectPrism` keeps an explicit early return spelled with the literals, because `K*sin(atan2(0.42,0.55))` is `0.42000000000000004` and those raw floats feed the bounds culls and go unrounded into `S.prismHits`. ⛔ Azimuth applies to `(dx, dz)` BEFORE the anisotropic gains — after, it shears rather than spins.
>
> ⭐ **MAX'S RULINGS, BOTH TAKEN.** (1) *"I want the nav screen to reflect the actual orientation of the planets in the game, so if B is correct then that's what we use."* (2) *"remove the negative/non-selectable space and redraw the cells from there"* + *"the chunky cells of design1 today are good."* The galaxy is re-fit to the **reachable** footprint — 18.000 kpc, derived by bisecting 32 rays through the picker's OWN `getSectorAt` — so 44 → 36 kpc, **52 of 64 cells, 0 of them dead (was 32), 0 sectors lost, cell size still 27 texels**.
>
> ⛔⛔ **AC-16 IS THE ONE TO PICK UP, AND IT IS NOT GREEN.** His ruling set a higher bar than the A/B asked about: planets MOVE. `main.js:7875` COPIES `orbitAngle` into the scene entry and the sim advances only the copy, so the orrery drew a snapshot of opening day. Fixed with a `_live` back-reference folded onto `main.js:7878` PLUS a per-frame angle refresh in `state.js` — **both halves required**, because `buildBodies` runs once per system and would freeze the live value right back. Mutation-proved headlessly. ⛔ **BUT LIVE CONFIRMATION FAILED TO EVEN RUN**: `window._systemData` is undefined in the ORRERY boot state — **`spawnSystem` had never run** — so `_currentSystemData` is null, `NavComputer:2458` falls through to `resolveArrivalSystem`, and the SYSTEM screen is drawing a system **the nav generated for itself**. There are no in-game planets to track yet. Select a body, commit the BURN, and re-sample.
>
> ⭐ **FIVE DEFECTS FOUND IN PASSING, ALL FIXED.** Every body name in both menus was a **swallowed error** — `state.js`'s `makeRng` returned `{next, child}` while `NameGenerator` calls `rng.float()`, so all 39 of Sol's bodies threw, every name was the catch-block fallback, and its alphabet ran out at index 10 putting the literal **`Sol undefined`** on the glass. The orrery placed bodies by **draw-loop index** while `[`/`]` re-sorts them, so the sort key **teleported every planet around its ring**. `d2System`'s `maxR` carried a second hard-coded `0.42` that overflowed the pane by 93.5 texels per side under rotation. `dottedEllipse` walked at equal *parameter*, so a flat ring read as two dashes. And **four** unguarded `D.playerSector` dereferences would freeze the glass for any player past R=18 — the worst mode here, because `PanelHost` catches once then stops uploading and the screen **looks alive**.
>
> ⚠ **A NEW MARK GUARD EARNED ITSELF IMMEDIATELY**: `rect`/`sprite`/`dottedEllipse` were unguarded (`assertFits` is reached only through `T()`), and it caught a pre-existing moon-pip overflow nothing else could see. 2,880-frame sweep now reports **zero**, with a sabotage control proving the channel is live.
>
> Suites, over a tree verified unchanged for the whole run: `src/ui` **527 passed (28 files)**, `src/cockpit` 698, `tests/` 20 failed across the same documented 8. `--check` green. `NavComputer.js` **4711**, `main.js` **15161**.
>
> ▶ **STILL OPEN:** ⭐ **AC-16's live confirmation is the one to pick up** — the code is mutation-proved but could not be exercised, because `spawnSystem` had never run in the ORRERY boot state; get into a spawned system first. Then AC-2 (everything clickable — inventoried, not built); AC-5's GALAXY half (the clicked cell is not the drill target, so it needs the sector's own rect published out of the paint); AC-9 (grabbable indicators); AC-6's inbound half (needs an `S.level` lag in `navViewModes/`, not a legacy-painter animation). ✅ AC-14 is CLOSED.
...
>
> ⭐ **THE MAP IS A PICKER AT ALL FIVE LEVELS IN BOTH DESIGNS, AND THE GEOMETRY COMES OUT OF THE PAINT.** `S.mapProj` / `S.prismHits` / `S.bodyHits` / `S.railTiles` / `S.listGeom` / `S.tabRects` / `S.chipRect` are all published at their own draw sites, the way `d1Ladder` already published `ladderStops`. Restating a layout in a control is the AC-4 defect shape and it is now unrepresentable for the map. ⛔ Two traps found by measuring, not reading: `row` is **Z-flipped** (the lab counts +z down, `_handleClick` counts +z up), and the designs were **player-anchored** — `levelView` always returned the PLAYER's sector, so a drill moved the game and not the picture.
>
> ⛔⛔ **THE SHIPPED LIST PICKER WAS ALREADY ACTING ON THE WRONG STAR, AND THE SUITE COULD NOT SEE IT.** Measured live: rail row 2 hovered `XND J3DK8MQE-RLZ16U6`, and **one `render()` later the field said `XND J3DJFN6W+A2AFBIA`** — because the legacy painters run first and rewrite all three hover fields every frame from `_mouseX`/`_mouseY` against LEGACY geometry. The test renders BEFORE the mousemove and never between the mousemove and the click. Same shape as the dead `V` key: a green test whose sample contains no input that could fail it. **Hover now resolves at the tail of the driver's `render()`**, which wins by construction.
>
> ⭐ **THE PRISM WAS CAMERA-BLIND, MEASURED WITH A LIVENESS CONTROL.** With `_localStars` frozen, mutating `_localRotY` / `_localRotX` / `_localRadius` / `_localCenter` left the map's pixel hash **byte-identical**, while moving `_playerX` changed it — so the probe was live and `projectPrism` was anchored to the SHIP. It now takes `S.cam` from the game's own camera, and `levelView` takes `S.view` from the game's own frame. **Both are byte-identical at entry** (`_localRadius` enters at 0.0015, which IS `ZOOM_STOPS[0]`), so the picture Max ruled on is reproduced and only a real move changes it.
>
> ⭐ **`/` OPENS A SEARCH DRAWN IN EACH DESIGN'S OWN INK, AND THE PIPELINE IS REUSED, NOT REBUILT** — `_runSearch`, `_moveSearchHighlight`, `_activateSearchHighlight`, `_selectSearchResult` and `resolveKnownObjects` are untouched; only the presentation was DOM. Live, "sol" returns Sol off the real catalog, Sol off the registry and Chi Fisolbe 9560 off the named-system box, and Enter fires the supported warp with Sol's own seed.
>
> ⛔ **SIX DEFECTS FOUND ON THE WAY, ALL FIXED:** `D.selBody` had **never once** tracked the pilot's selection (it read `pick.index`; all four write sites set `planetIndex`/`moonIndex`/`starIndex` — only the declaration comment says `index`); `_handleWheel`'s lower clamp sat ABOVE the prism's entry radius, so the first wheel event zoomed OUT by 33%; the ladder's `...` grab zone made the outermost planet unclickable on any overflowing ladder; `_autopilotButtonRect` is a live invisible button under both designs and `_handleClick` tests it FIRST; `nav-lowres` never came off, so `V` back to CURRENT left today's nav wearing the mode's dress; and the clause order on `:349` let the **L in "SOL"** flip design 2 into list mode mid-query.
>
> ⭐ **PIXEL IDENTITY PROVED, NOT ASSERTED:** 1,791,060 draw calls diffed for phase 1 (18 differences, all 18 the strings deliberately corrected) and 0 of 1,089,886 for phase 2 with the search closed. Guard violations live: **0**. Suites: `tests/` 20 failed / the same 8 files, `src/cockpit` 698, `src/ui` **480** (339 + 141 new). `NavComputer.js` 4711, `main.js` 15161, `--check` green.
>
> Workstream `docs/WORKSTREAMS/nav-menu-elements-functional/` — `intent.md`, a 12-AC `contract.json` (status `verifying`) and `INTERFACE.md`, the coordination contract three parallel agents built against on disjoint files.
...
>
> ⛔ **§4.7 IS NOT THE CRITICAL PATH IT WAS CALLED.** The handoff named `navLayout`'s saturation "the single biggest thing to fix", on the premise that a 240p fullscreen nav would be built on the existing renderer and inherit desktop chrome — reproduced exactly: `navTabHeight(240) = 32` (13.3% of the rows), `navMapSize(427,240) = 160` (a 25% square in a 427-wide frame). But **both designs Max ruled on draw their own chrome** and never call `navTabHeight` / `navChromeReserve` / `navMapSize`. Desaturating would move the shipped overlay and the pinned fixtures to fix a surface that stops existing. It reopens the moment anything asks the LEGACY renderer into a low-res buffer.
>
> ⛔ **§4.5's "84% OF PRISM MARKERS ARE OFF CANVAS" DOES NOT REPRODUCE.** Proper A/B (real function vs a `() => true` stub, same scene, 1560x860): **0%** at the default zoom, **0%** at the 10.34 pc ceiling, **0%** under rotation, **0%** on a lateral pan, **13%** (31 of 237 arcs) on an R/F vertical pan. ⭐ The geometry says why: `x = cx + dx * halfW * 0.92` with `dx` normalised by the QUERY radius, so `|dx| <= 1` and every queried star lands inside the frame; only Y escapes, having its own window. **The cull ships anyway as a CORRECTNESS guard, not a perf fix** — `halfW` equals the canvas only while the prism owns the whole canvas, and design 1 gives the map 43 of 71 columns beside the rail.
>
> ⭐ **AND THE FIRST LIVENESS PROBE FOR IT WAS VACUOUS — recorded in the test file so it is not rewritten.** It rendered the same star count at two world offsets; both drew exactly 250, because `render()` QUERIES the starfield and overwrites `_localStars` (212 real stars replaced the 120 hand-built ones), so the two branches were one scene. It now mocks the module instead and is scene-independent; proved by sabotage — unwiring reads 250 against 250 and fails.
>
> ▶ **NEXT: Max's greenlight on the contract, then build the two designs as modes.** `NavComputer.js` is length-stable at 4711 so its ~700 citations hold; the panel and the overlay are **two separate instances** (`main.js:4700` vs `:5895`), so the mode axis cannot cross to the cockpit glass. Suites: `tests/` 20 failed / same 8 files, `src/cockpit` 698, `src/ui` **306** (+11).

> ## ▶ 2026-09-07 — **THE ORDER OF WORK REVERSED, AND MAX WANTS BOTH NAV DESIGNS AS MODES.** ⭐⭐ His ruling 2026-09-06: *"we're doing this the wrong way, though...we need to begin by making these nav computer screens work in fullscreen first (the N menu) then figure out how to represent that in the diegetic screens. What you've done here is fine for now."* **THE FULL-SCREEN NAV IS THE SOURCE; THE COCKPIT PANEL IS A REPRESENTATION OF IT.** Then, against a rendered lab: *"1. I love both 1 and 2 for both; I want all of these modes! 2. We keep both for now 3. going forward for all these menus and main gameplay I only need 240, 288 and 360 as comparison options."*
>
> ▶ **THE SPEC IS A LAB HE HAS ALREADY RULED ON** — `nav-240p-lab.html`, `http://localhost:5175/well-dipper/nav-240p-lab.html` (`b7c8155`). Draws at exactly 427x240 upscaled `pixelated`, with the REAL `drawPixelText`, the GPU `NavGalaxyRenderer`, 775 sectors, 15,606 HYG stars and `resolveArrivalSystem`. Keys `1 2 3` design · `← →` level · `R` lines · `Z` zoom · `S` system · `;` face · `X` sabotage. **Design 1** = map + 26-column ranked star list (judges: use 30, build 31). **Design 2** = full-bleed sky, two 8-row bars (era 30). **Design 3 killed by all three judges** for deriving fullscreen from the 52x43 panel — the inverse of the ruling. ⛔ Do not resurrect it.
>
> ✅ **RULING 3 IS LANDED** (`68ab988`): `RENDER_LINE_OPTIONS = [240, 288, 360]`, was seven. Stored 480→360, 144→240 — settings converted, never reset. ⭐⭐ **AND IT EXPOSED A TEST THAT COULD NOT FAIL FOR ANY MODE THAT NOW SHIPS.** `PhosphorScreen.test.js` pinned `lines === 7` "at every shipped resolution" and sampled 240/480/720 — **43, 86, 129 rows, exactly 1x/2x/3x `gridRows()`**. On an exact multiple there is no remainder and seven is all you get, so the sample held no other kind of number. 288 and 360 are not multiples: upper 51→**8**, 64→**10**; lower 55→**9**, 69→**11**. Two of the three modes he just chose hand the panel MORE rows than its layout is authored for. Not a defect (painters read `lines`), but the panel is NOT the identical picture at 288/360. Test split: exact counts per mode, and the invariant that does hold (the type SHAPE never moves) stated on its own. ⭐ **Ask what would make a green test fail; if the answer is "no input in its sample", it pins nothing.**
>
> ⛔⛔ **THERE ARE THREE TEST SUITES AND THE STANDING PAIR MISSES ONE.** `--dir tests` (20 failed / 8 files), `--dir src/cockpit` (**698**), and **`--dir src/ui` (295)** — where every NavComputer test lives, absent from both previous handoffs, and it caught a real break during AC-4 that the other two could not see.
>
> ⚠ **OPEN AND UNRESOLVED:** *"for both"* in ruling 1 reads either as both SURFACES or both LEVELS — one line to confirm before building. And ruling 3 replaced, rather than answered, "should type scale with the buffer?".
>
> ▶ **NEXT: a NEW workstream, not a continuation** — `chrome-and-ui-at-240p` is finished. `dev-collab-scope` first. Cheapest independent wins: `navLayout` is still SATURATED at 240p so a full-screen 240p nav inherits DESKTOP chrome (the tab strip alone eats 13% of the rows); and the prism has **no screen-bounds cull** — 84% of its markers at default zoom are off canvas and still cost a glyph and two hover tests each. Handoff: `docs/FEATURES/handoff-2026-09-07-nav-fullscreen-modes-next.md`.

> ## ▶ 2026-09-06 (later) — **AC-4 IS LANDED AND ITS OBSERVABLE IS CLOSED END-TO-END ON THE RUNNING GAME.** `b5947f5` (lane A, unpushed). The nav computer draws and works in a 43-row panel. ⭐ **DRIVEN WITH REAL MOUSE EVENTS IN HELM, NOT HEADLESSLY:** all five level tabs land on panel row 38 and change level correctly (G→galaxy, S→sector, R→region, P→prism, **Y→system** — Y because SECTOR owns S and only ONE glyph fits a 10.4-texel cell); a click on a body selects it; `[ BURN ]` renders fully on the glass and pressing it moved the ship — pilotPhase IDLE→**ALIGN**, flightMode manual→**assist**, pilot false→**true**, panel self-dismissed. That is AC-4's whole walk, including the PanelPointer world-hit→UV→panel-pixel link that had never been verified.
>
> **Four numbers were absurd at 43 rows, not merely wrong:** `tabH = 32` made the strip 74% of the panel; `h - 50` was **-7** at four sites; ⭐ `min(w,h) - 80` was **-37** at three sites — which is why GALAXY/SECTOR/REGION drew an inverted collapsed map that could not be clicked, **and that site was in no anchor list**; `btnY = drawH - 52` put the commit button off the glass, as two independent copies feeding one hit-test.
>
> ⛔ **NOT `_bare` — THE OBVIOUS SEAM WAS A TRAP.** `NavPanel` writes `chromeless = false` every paint under Max's 2026-08-01 ruling, so `_bare` is permanently FALSE on the panel; keying type off it would have been a no-op that looks exactly like a wiring failure, and it is level-gated to SYSTEM so the whole tab walk would never have seen it. `NavComputer`'s own `dimSurface` comment records that identical mistake being made once and corrected. The driver copies `dimSurface`'s shape instead.
>
> ⭐ **THE OVERLAY IS PROVED UNMOVED, NOT INSPECTED:** old-vs-new draw-call diff at four canvas sizes, 3617 calls each, **0 stable differences**, scene-varying lines masked by running each revision twice — **and the control was made to fail** (restoring one duck-typed helper put the literal string `"undefined"` on all five tabs and diverged the stream by 2318 lines).
>
> ⚠ **WHAT IS STILL WRONG AND IS DELIBERATELY NOT THIS STEP'S JOB — Max will see the first one immediately.** The PRISM stars are `baseRadius 4.5/3.5` **absolute** px, so on a 52-texel panel each star is ~9% of the panel wide and PRISM — the panel's DEFAULT open level — reads as a wall of orange blobs. Same class: SYSTEM's planet discs, and the 12-14px body hit radii. This step was type and chrome geometry; every one of those is a GRAPHIC constant and none was touched. Also open: zoomed NAV is still a 43-row texture blown up to ~180 rows, and NAV's type stops growing above ~172 buffer rows where the other three panels' keeps going (only reachable in the lab above BUFFER 129, or in-game at 720 lines with fov ≤ 45).
>
> ⛔ **THE `src/ui/` SUITE IS IN NEITHER STANDING INVOCATION AND IS WHERE EVERY NavComputer TEST LIVES** — `npx vitest run --dir src/ui --root ~/projects/well-dipper`. It caught a real break the other two could not see. Baselines now: `tests/` 20 failed / 8 files (**zero drift**), `src/cockpit/` **705**, `src/ui/` **295**.
>
> ▶ **AC-9 IS NOW THE ONLY THING LEFT AND IT IS MAX'S EYE ALONE.** All four panels draw, the cabin is on the world grid, NAV is operable. Expect him to react to the PRISM blobs first.

> ## ▶ 2026-09-06 — **AC-1 IS LANDED AND VERIFIED ON THE RUNNING GAME. THE CABIN IS ON THE WORLD'S PIXEL GRID.** `5701622` (lane A, not pushed). `cockpitTarget` is **427x240**, not 1600x900 — height is the line count. The four panel canvases are authored at the rows they actually get: **NAV/DRIVE 52x43, INFO/TARGET 55x46**, `eyeFound: true`, no host error, clean console. ⭐ **THE LIVENESS PROBE PASSES ON EVERY AXIS**, driven through the real settings handler in HELM: all seven resolutions match the plan's table exactly (144→26/28, 180→32/35, 240→43/46, 288→51/55, 360→64/69, 480→86/92, 720→129/138), widths included; FOV 70→43, 50→64, 90→30; a window resize at a fixed setting moves nothing. So the derivation is WIRED, not coincidentally right.
>
> ⛔⛔ **AND THE THING THAT WAS WRITTEN BACKWARDS AND IS NOW MEASURED.** The first draft of `panelBufferRows.js` justified its number by claiming off-axis panels subtend LESS than fronto-parallel ones. A projection divides by DEPTH, not radial distance, so they are STRETCHED. The screens sit **41.5° off the view axis**; their glass covers **51.9–68.1 rows** (upper) and **53.6–70.5** (lower) against the 43 and 46 authored — a **1.16x–1.59x** band, constant at every resolution and fov. ⭐ The smaller number is KEPT DELIBERATELY: under Nearest sampling magnifying DUPLICATES texels and minifying DELETES them, and every glyph here has one-texel stems. Never-minifying is now pinned in the spec, not asserted in a comment. **The lever, if AC-9 says the panels sit off the world's grid, is the floored minimum covered edge (51 upper / 53 lower)** — checked safe, and the 7x12 grid survives it.
>
> ⚠ **NAV IS NOW VISIBLY WORSE THAN BEFORE, AND THAT IS BATCH 2 STEP 4's JOB, NOT A REGRESSION TO CHASE.** `NavComputer` draws in ABSOLUTE canvas pixels into a canvas whose size it never reasons about, and `NavPanel.js:234` resizes it to the live buffer every paint — so 14px type now lands in a 43-row canvas. Seen in HELM: the NAV panel is a smear of overlapping giant type. ⛔ **The AC-9 whole-game look is not worth taking until step 4 lands** — one of the four screens would dominate the verdict for a reason already known.
>
> Two other things found and fixed rather than left: `panels.test.js`'s upper-240p width was **51 where the game has 52** (`round(43 * 1.2) = 52`; the other five entries agreed, which is why the wrong one read as derived), so the overlap guard had never run on the shipped geometry; and `cockpit-screens-lab.html` could not reach ANY size that ships (smallest 256 against 43), so a legibility verdict taken there would pass where the glass fails — re-anchored to boot at 43. Baselines: `tests/` 20 failed / 8 files, the same eight by name, **zero drift**; `src/cockpit/` **701 passed**.

> ## ▶ 2026-09-08 (session end) — **THE PANEL REDESIGN IS BUILT AND MERGED TO LANE A. THE PARKED KIT LANDED WITH IT. AC-1 IS THE ONLY THING LEFT AND IT IS NOW UNBLOCKED.** `04db4a8`. ⭐ **MAX PASSED THE PANELS 2026-09-08 against the rendered lab: *"1. this seems good to me for now"*** — a pass with a shelf life in his own words (*"we'll probably switch this up in the near future"*), which is exactly why every long form stays producible. ⛔ Do not re-raise it; it reopens if the panels change or if AC-9 says otherwise. ▶ **MAX'S THREE CONTENT RULINGS, ALL BUILT:** (1) *"1, okay"* — CAP and TURN off DRIVE. (2) *"2. sounds good"* — TARGET shouts the SHORT name and the distance and **drops the full designation** (⚠ NOT the batch plan's option (a); the full name is off the glass, not in small type). (3) *"1. this is fine"* against a rendered proposal — INFO values are **five** characters, ATM keeps the pressure and loses the gas mix. ⛔⛔ **AND THE CONDITION HE ATTACHED, WHICH OUTRANKS ALL THREE:** *"know we'll probably switch this up in the near future so don't get rid of any code that allows you to display what we want to display."* Every short form is a SECOND projection beside the long one — `INFO_ROWS` gained `abbr`/`brief` over one `read`, `AlertCue` gained `ALERT_BRIEF`, `FlightReadout` gained `speedValue`/`tierLine`, `formatSpeedCap`/`formatTurnCap` are exported and still tested though nothing draws them. Nothing was deleted. ⛔ **A NUMBER I GAVE HIM WAS WRONG AND THE LAB CAUGHT IT:** the handoff grouped INFO with the UPPER pair; `PanelLayout.js:53-56` puts it on `Screen_LL`, the lower and WIDER pair, so an INFO value gets 5 characters and NAV is the one that drops to 4. The arithmetic was right and the input was wrong. ⭐ **THREE DEFECTS FOUND BY MEASUREMENT, TWO PRE-EXISTING:** TARGET's DIST row overlapped itself (3-char label + 8-char value on a 9-char panel — a smear, not a clipped string); TARGET's hero probe cleared the whole buffer and redrew one size down, at a size that also did not fit; and **`decodePixelText` INVENTED TEXT** — its anchor search ignored unmatched cells, so an origin on one line reached into the next and reported runs no painter drew, which every panel assertion sat on. It also could not read '1' or '-' at all, because a run's first LIT column is not its cell's left edge. ⚙ **The guard that would have caught the first two now exists:** every panel, at 240p/480p/720p, on the pair of screens it actually lives on — no ink outside the buffer, no two strings sharing a texel, no cell that fails to decode. It found two more on its first run. ▶ **`cockpit-panel-budget-lab.html`** (root, served at `/well-dipper/`) renders the SHIPPED painters over a real snapshot at true 240p size — before/after for all three panels. ▶ **NEXT = batch 2 step 3 = AC-1**, and it is unblocked: `cockpitTarget` still allocated at raw window size (`RetroRenderer.js:1021`), panel buffers still one 512 scalar for four panels, `magFilter` still defaulting to Linear in `PanelPointer.js`. ⚠ Baseline verified against lane A in a throwaway worktree, not assumed: `tests/` 20 failed / 8 files (same eight), `src/` 31 failed / 5 files on BOTH branches.

> ## ▶ **HANDOFF = `docs/FEATURES/handoff-2026-09-09-panels-redesigned-ac1-next.md`. START THERE.** The panels are built, passed and pushed; AC-1 (batch 2 step 3) is the whole remaining job and its two halves cannot be split. ⛔ Superseded: `handoff-2026-09-08-resolution-revisions.md`, whose §4 puts INFO on the wrong pair of screens.

> ## ▶ (superseded) `docs/FEATURES/handoff-2026-09-08-resolution-revisions.md`. Max asked the next session to OPEN with an overall recap of the resolution revisions — §0 of that file says how. ⛔ Do not open by asking what to work on.

> ## ▶ 2026-09-07 (session end) — **AC-6 CLOSED (HUD + RETICLE AT 240p, LIVE-VERIFIED). THE IN-GAME FACE IS NOW 5x5 WITH LOWERCASE. AC-1 IS BLOCKED ON A PANEL REDESIGN AND THAT IS MEASURED, NOT ASSUMED.** Pushed + verified at `74630f6`. ▶ **SHIPPED:** batch 1 step 4 (`3e9d899`, SupercruiseHud into the world buffer — zero semi-transparent pixels measured across the whole canvas) · step 5 (`bf2879e`, targeting reticle + cabin mask; cabin coverage INVARIANT across a 56x sampling range, 0.5497→0.5504 from 41k to 2.3M px, and within 0.0063 of the same-session ray oracle) · the bracket shape fix (`fbc3f85`) · the one-texel selected stroke (`d496a97`) · the 5x5 face (`49caa98`) · batch 2 step 1 glyph coverage (`d372704`) · lowercase (`74630f6`). ⭐⭐ **MAX'S THREE UAT CORRECTIONS, ALL MINE TO OWN.** (1) The batch plan said the reticle's `vx` step was 'one block outward, contradicting its own ASCII art' and told me to move it inward; **the art was wrong and the outward step WAS the rounding** — applied as written it turns every corner into a T, which he caught on sight. ⛔ §3.5 item 2 of the batch plans now flags that instruction as actively wrong. (2) A 2-texel selected bracket was the plan's reasoning and it reads as a block, not a corner — back to 1 texel, colour and alpha carry the state as they always have. (3) The 3x5 face was measurably unreadable: **twelve confusable letter pairs, M/N one pixel apart of fifteen**; the defect was CELL WIDTH, not resolution, which is why it looked fine in a spec. ⛔⛔ **AC-1 CANNOT LAND WITHOUT THE PANEL TYPE WORK — MEASURED IN THE LIVE GAME.** Setting `cockpitTarget` to the world buffer puts the cabin, bezels and ribs correctly on the world grid AND turns the INFO panel into a column of grey mush in the same frame, because the kit sizes type as fractions of panel height and draws it with a vector font; at ~43 rows that is 2-5 pixel rows per line. **AC-1 and AC-2 are one change.** ▶ **PARKED, DELIBERATELY: `wip/cockpit-kit-bitmap-face`.** The kit is converted (integer grid instead of five independent ratios, bitmap face, absolute texel floor) WITHOUT touching a single panel file — the panels only consume `screen.type.*` and `screen.text()`. 23 tests still encode the VECTOR contract (sets `ctx.font`, sets textAlign every draw, measures under the font it is about to draw with, `TYPE_RATIOS` as fractions, leading 1.4x body) — all mechanisms deliberately gone. **Max, 2026-09-07: *"don't spend too much time here, my guess is we will substantially redesign these panels in the near future"* → land this WITH that redesign, which rewrites those tests anyway, rather than rewriting them twice.** ⭐ **`decodePixelText` is the asset to keep** (on the WIP branch): reads strings back OUT of drawn texels, so panel assertions stay AT THE GLASS instead of becoming a painter self-report. It took the cockpit suite from 46 red to 25 on its own and caught two of its own bugs — including one lowercase introduced, where it anchored on the topmost lit pixel, which is the cell top only when a run contains a capital or a digit. ⚠ **THE LESSON OF THE SESSION, FOUR TIMES:** a measurement of MY OWN DRAFT is not a measurement of the design space. I called 5x5 'disqualifying' off a bad draft (redrawing six glyphs beat 5x7), and nearly reported that a 5-row cell cannot carry lowercase off another (eight confusable pairs → three after redrawing eight glyphs). ▶ **NEXT:** AC-9 is Max's eye alone. Then the panel redesign, which carries batch 2 + the parked kit + batch 1 step 6 (still on hold — the plan places its composite branch AFTER the grain block it exists to collect: `RetroRenderer.js` grain `:883`, cockpit `:898`). Baseline unchanged all session: **20 failed / 8 files, the same eight.**

> ## ▶ 2026-09-06 (session end) — **HANDOFF = `docs/FEATURES/handoff-2026-09-07-chrome-and-ui-at-240p.md`. RESUME AT BATCH 1 STEP 4.** Pushed + verified at `6e84601`. ⭐⭐ **THE STEP-BY-STEP EDIT PLANS FOR EVERYTHING REMAINING ARE IN `docs/FEATURES/chrome-240p-BATCH-PLANS.md`** — both batches, each adversarially attacked, step 4 written out in full with exact anchors. ⛔ Do NOT re-derive them; DO re-check every anchor, line numbers drift. ▶ **DONE:** orbit rings (no change needed) · HUD buffer→world grid (`5044158`) · minimap pixel floors + sprite chevron (`8f75de8`) · `PixelText` one-glyph-set (`2f9db20`). ⏭ **NEXT: step 4, `SupercruiseHud` bitmap text — the FIRST thing Max can SEE.** He reloaded, saw nothing, and asked; tell him to reload when it lands. Then step 5 (reticle + cabinMask), then batch 2 (cockpit) against the **fixed 7x12 character grid at one integer scale** — same picture at every setting from 240p up, only sharper. ⛔⛔ **THE PANEL IS 42.84 ROWS, NOT 48.9 — I SOLVED THE ANGULAR FRACTION AND A PERSPECTIVE PROJECTION IS LINEAR IN TAN.** `panelPose.js:34-49` already warned about exactly this. Tiers: display 7.14, lead 3.89, body 2.52, label 2.14. Every cockpit number derives from 42.84. ⚠ Two more of my own metrics were wrong and are corrected in-tree: the minimap gap was 2.644x not 5.2x (I omitted the shader's `* aspect`), and the star billboard is 1x2 buffer px not 9x11 (I counted non-contiguous bright pixels). **All three were plausible metrics measuring the wrong quantity.** ⛔ NEW TRAP: `preserveDrawingBuffer` is FALSE, so `gl.readPixels` on the default framebuffer returns ALL ZEROS — assert the probe is live before believing a difference. ⚠ `]` (framebuffer quantise) fires but is a VISIBLE NO-OP: every material already posterizes to the same 31 levels, so it moves 47.9% of pixels by at most 6/255. Not broken, just redundant. ⚠ Baseline all session: 20 failed / 3944, 8 files.
>
> ## ▶ 2026-09-06 — **ACTIVE WORKSTREAM: `chrome-and-ui-at-240p`.** Contract greenlit by Max and building. ⭐ **HIS RULE IS THE WHOLE SCOPE DISCIPLINE, AND IT INVERTS THE USUAL ONE:** *"I want the whole game to read as a 5th gen game (there are things that are going to be anachronistic and I'm totally fine with that, but some things like the resolution are harder limits to get that aesthetic); so we simply need to redesign anything that does not read properly at this new resolution."* Resolution is the HARD constraint; a surface that fails at it gets **redesigned, never exempted**. ⛔ The intent retires "exempt the text" BY NAME — it was offered and not taken. ▶ **WHERE TO SEE STATUS:** `wsstatus.sh` (one row per workstream) · `docs/WORKSTREAMS/chrome-and-ui-at-240p/{intent.md,contract.json}` (9 ACs = the checklist) · `git log` on lane A (every commit carries its measurement). ⭐ **BATCH 1 — chrome onto the world's grid.** STEP 0 orbit rings: **NO CHANGE NEEDED**, `OrbitConicField` is a full-screen quad working in `gl_FragCoord`, so its stroke is one world pixel at every resolution by construction (`OrbitRingSDF`/`OrbitLine` draw nothing — proxy + hit-test shim on layer 10). STEP 1 **SHIPPED** (`5044158`): `_hudSize` was a fixed 320 ignoring the setting, so the minimap drew **2.644x finer** than the world beside it AND the relationship MOVED with the slider, inverting by 720p. Now derived as `_hudFrac * renderWidth`; measured world/HUD texel ratio **1.005 / 1.000 / 0.9995 / 1.000** across 144→720p. ⚠ The minimap's SPRITES were sized against the old 320 and now render small — **expect it to look worse before step 2 redesigns it** (isotropic frustum, pixel floors, pixel-art chevron). ⚠ `src/rendering/renderBuffer.js` shipped INERT — nothing reads it until the reticle lands. ⏭ **NEXT:** step 2 SystemMap redesign → reticle + HUD content (`PixelText.js`, one 3x5 glyph set for the WHOLE game — ⛔ do not let a second one exist) → **batch 2: cockpit + nav panel**, where the real work is. ⭐⭐ **THE COCKPIT NUMBER MATTERS AND MAX RULED ON IT KNOWINGLY:** panels subtend 14.25° of a 70° FOV (measured off `cockpit.glb`), so at 240 lines a panel is **48.9 buffer px** → display 8.1, body **2.9**, label **2.4**, against `PhosphorScreen.js:81`'s own stated floor. His ruling: all of it at 240p, text included, redesigned until it reads. ⛔ In HELM **the nav computer IS a cockpit panel** (`main.js:5630`), so it coarsens with the cabin — the first split named the same pixels twice. ⛔ OUT OF SCOPE: PhosphorDither one-ink law (lane F). ⚠ Baseline all session: 20 failed / 3936, 8 files.
>
> ## ▶ 2026-09-05 — **THE CLOSE STAR IS FIXED AND IT WAS THREE DIFFERENT BUGS WEARING ONE COMPLAINT.** Max closed it: *"much better"* (pushed, `992c1bb`, lane A). ⭐ **THE HANDOFF'S LEAD WAS RIGHT ABOUT A MISSED SITE AND WRONG ABOUT WHICH ONE.** `StarFlare` has TWO programs and swaps by distance (`billboardSwitchDistance`): far = `_createBillboard`, near = `_createFlareDisc`. The billboard got the 2026-09-06 dither-authority fade; the flare disc — the one that draws a CLOSE star — got neither 2026-09-06 fix. At 240p it rendered a hard-edged Bayer **square about a third of the screen wide**. ⭐⭐ **AND THE STIPPLE HAD THREE JOBS, WHICH IS WHY REMOVING IT TOOK THREE PASSES.** `if (dither > brightness) discard` was (1) a texture, (2) a GAMMA — its expected value is `brightness²`, which is what held every faint feature down — and (3) the thing keeping `scene.a` at ZERO across most of the quad, which is what let the starfield show through at all. Removing it without accounting for (2) drew the pre-existing lens-ghost ring and outer halo SOLID (Max: *"a bunch of discs"*); without accounting for (3) it stamped opaque world over the sky (Max: *"a big black disk"*), since `RetroRenderer`'s composite is `mix(bg.rgb, scene.rgb, scene.a)`. ⭐ **THE ANSWER IS ONE TERM: write the glow's own coverage as alpha.** `mix(sky, color, b)` = `b·color + (1-b)·sky` — exactly what the stipple averaged to, at any resolution, with no screen-space pattern and no threshold. ⚠ RGB blend factor must be `One`, not `SrcAlpha`, or the composite applies `b` twice and the star comes out dimmer than it has ever been. ▶ **`[` A/Bs it live**: BAYER (checkerboard) → HARD CUT (no halo) → LINEAR (discs) → COVERAGE (shipped); reasoning in `src/rendering/glowGradientMode.js`. Also fixed: `StarfieldLayer` + `WarpTunnelStarfieldLayer` got the authority fade (`cc06693`). ⛔ **STILL OPEN, AND IT IS THE ACTUAL ARC:** apparent magnitude. Max: *"The star and planets should simply appear to be brighter than the other stars ... I'm fine with cheating a bit and using some screen-space effects or post processing."* Proposed and unrejected: once a pixel saturates, brightness must read as **SPREAD** — background stars stay 1px, in-system bodies get a halo scaled by magnitude. Closes *"billboards still illegible"* too. ⚠ NEW FENCE: `tests/glsl-template-literal-fence.test.js` — a backtick in a `/* glsl */` literal truncates the shader and NOTHING else catches it (the suite compiles no GLSL). Its own first version reported 3 offences, all false; escaped backticks are legal. Baseline unmoved all session: 20 failed / 3920, 8 files.
>
> ## ▶ 2026-09-06 (session end) — **NEXT ARC: APPARENT MAGNITUDE AT 240p. Handoff = `docs/FEATURES/handoff-2026-09-07-apparent-magnitude-at-240p.md`.** Max: *"even the close star looks no brighter (dimmer in some cases) than stars much farther away; we have to reconsider how apparent magnitude functions at this resolution"* — plus billboards *"still illegible"* against the starfield. ⭐ **ONE PROBLEM, TWO FACES:** at 240p a star cannot encode magnitude with SIZE (everything collapses to 1–2 px), so brightness must carry it, and the range is small; billboards are deliberately dimmer than stars (1.2× vs 1.8× HDR), which worked when the starfield was sparse. ⭐ **THIS IS A DESIGN QUESTION, NOT A BUG HUNT** — brainstorm with Max before shader code. ⭐ **ONE CONCRETE LEAD ALREADY LOCATED: `StarfieldLayer.js:336` is the ONE dither site that never got the authority fade** — its `solid` bypass spares only SMALL stars, so a CLOSE star still takes the full Bayer stipple across its whole disc, which is exactly his "close stars look the same". Do that first. ▶ **RULINGS THIS SESSION, SETTLED:** 240p is the target; 31/RGB555 stays; framebuffer-wide quantisation OFF (he said yes, I measured the cost, he said off — `]` still toggles); the sky at the world's resolution is FINE (the first attempt only looked bad from a `gl_PointSize` bug); **Sol is an easter egg, NOT a priority**. ⚠ **11 COMMITS UNPUSHED — ask before pushing.** ⛔ NEW TRAPS THAT COST TIME: `esbuild` without `--bundle` does NOT resolve imports (a missing import emptied the whole sky scene and Max saw it before I did); backticks inside a GLSL template literal terminate the string (did it THREE times); `gl_FragCoord`/`gl_PointSize` are in BUFFER pixels, not screen pixels; a `visible = true` write does not survive a frame if the renderer re-derives it.
>
> ## ▶ 2026-09-06 — **THE SEVEN-SURFACE INVENTORY IS DONE AND IT IS IN `docs/FEATURES/rendering-scale-seven-surface-INVENTORY.md`. Max's sense was right, and the shape is sharper than "some numbers differ".** ⭐ **THE HEADLINE, MEASURED LIVE IN SOL: the 8.0 is not mistuned, it is UNWIRED.** Driving the shared setter 31 → 2 → 31 with Luna framed at 3 body radii, **16 materials stayed pinned at 8 the whole time while 35 tracked** — `TexturedBodyShader.js:24` / `MaterialBodyShader.js:26` wrap their level in a PRIVATE `{value}` at build time, so `settings.posterizeLevels` cannot reach them at all. Two quantisers run in the same frame: 16 bodies at 3.17 bits, 35 at 5 bits. ⭐ **AND THE 8.0 WAS DELIBERATE — against a default that no longer exists.** It landed 2026-03-28 (`167598f`) when the global was **6.0**, its own comment reading *"more levels = more detail visible"*, i.e. 8 meant *a little crisper than the game* for photographic source material. 31 landed 2026-08-21 (`564e0db`) and the textured path did not follow, so **the relationship inverted**: 8.0 was 1.33× the game's levels, it is now 0.26×. ⛔ **THAT DOES NOT MAKE "set them to 31" THE ANSWER** and I am not recommending it — the original logic (*photos want MORE levels*) argues for something ABOVE 31, and that is Max's taste call on Sol. ⭐ **RENDERING SCALE IS FIVE TIERS, NOT ONE:** scene 1/3 (721×480), sky + cockpit 1/1 (2162×1440), HUD a FIXED 320×320 that does not scale with the window, menus DOM-native vector text — each individually justified at its own line, **never judged together**. Also found: **`pixelScale` has TEN hardcoded copies** of a user-adjustable setting (7 bare `/ 3.0` shader literals + `SkyFeatureLayer:448` + 2 reticle constants); **`DitherPass.js` is DEAD** (zero importers, still posterizes to a third number, 12.0); and the cockpit's `PhosphorDither` one-ink law is deliberate, current, and should be left alone. ⚠ **THE INSTRUMENT WAS VACUOUS ON THE FIRST TRY** — film grain (0.045) gave a noise floor of mean 7.909 against a signal of 6.263, so it would have reported "no change" for a surface that WAS changing. `_lab.freezeFrame()` fixed it: control 31-vs-31 = **exactly 0 changed pixels**, signal 31-vs-2 = **254,089 px confined to the planet disc** while the starfield behind it moved zero. ▶ **Nothing changed yet — the deliverable is evidence. Max's calls are in the doc's §7.**
>
> ## ▶ 2026-09-05 (session end) — **NEXT ARC: THE RENDERING SCALE / AESTHETIC CONSISTENCY PASS. Handoff = `docs/FEATURES/handoff-2026-09-06-rendering-scale-aesthetic.md`.** Max: *"we need to make sure the rendering scale (resolution, etc) is aesthetic appropriate for all the planets/moons/stars, the starfield, the giant galactic objects, the menus, and the cockpit. I have the sense they are not all consistent with our chosen aesthetic."* ⭐ **THE BAR ALREADY EXISTS AND IS SOURCED, so this is an obedience question, not a taste one:** `Settings.js:12` `posterizeLevels: 31` is his own 2026-08-21 ruling — 31 ⇒ 32 values ⇒ **5 bits ⇒ RGB555**, the framebuffer depth of the N64, PlayStation and Saturn alike, for *"that era's aesthetic"*. ⚠ **FIRST MEASUREMENT ALREADY IN HAND AND IT IS A REAL HIT: `posterizeLevels` is `8.0` at 22 sites against `31` at 1** — the lone 31 is the SETTING; the 8.0s are in `main.js` and `KnownBodyProfiles.js` (**Sol's known bodies**). ⛔ NOT yet a defect — Sol is the NASA-photo-textured system on a different renderer, so an 8 may be deliberate; the job is to find out which, **not to normalise 22 numbers because they differ from 1**. ⛔ **AND THE SEVEN-SURFACE INVENTORY IS DELIBERATELY NOT DONE** — my quick grep matched test files and gave junk, and a bad inventory is worse than none. ▶ **First task: for each of the seven surfaces, does it go through `retroRenderer` at all, does it posterize, and is that answer deliberate or inherited — each with a DISCRIMINATING check (change the setting, confirm the surface MOVES), because a surface that looks right at 31 may simply never have read it.** Evidence in front of Max before any change.
>
> ## ⛔ 2026-09-05 — **PARKED BY MAX'S RULING: the world engine features.** *"park these other world engine features (that's what we'll call the world/moon rendering engine from now on)"*. ⭐ **TERMINOLOGY: "world engine" = THE WORLD/MOON RENDERING ENGINE** — use it that way in docs, commits and conversation. Three parked items, each with its evidence already written and none scoped: **(1) frozen oceans / sea ice** (`WORKSTREAMS/frost-budget/FINDING-frozen-oceans.md` — `:452` zeroes frost on all standing liquid, `:526` claims the draw order delivers sea ice, and `:452` makes that impossible; the fix is CONDITIONAL because `:452` is right for Titan); **(2) an ice SHEET as geometry — an ADDITION, not a backlog item** (F22's card states its own position: *"this is a COVERAGE term, not relief"*; Max: *"this is just coloring applied to continents that would be there either way"* — he is right, `frostCoverage()` writes no `h` and no `grad`, and F17 glacial, the only ice geometry, moves the height field **0.26% rms**); **(3) the height field's kilometre scale** (two instruments disagree — `sampling.js:44`'s conversion makes the tallest mountain **1.99× the body's own radius** — and it sits under every altitude-dependent law, not just frost).

> ## ✅ 2026-09-05 — **THE SNOW BUDGET IS SHIPPED ON MAX'S UAT** — *"the mountains are ice/snow-covered so I'd say that's a success"*, and *"ice worlds still look like ice"*. ⭐ His one reservation is explicitly NOT this law: *"I don't think I'd say it looks 'right' because of all the semi-developed features e.g. rivers"* — the half-built things around it, not the frost. Workstream `frost-budget`, lane A, **pushed (`f186bcb`, verified by `git ls-remote`)**. The room-temperature ocean world Max walked is no longer a snowball: `uFrostMaxCoverage` 0.834 → **0.619**, sea-level snowline 26° → **60°**, and the A/B is `docs/WORKSTREAMS/frost-budget/live-rocky126-{before,after}.png` — white-and-grey over most of the disc, versus deep blue oceans, tan continents, green coastal belts and rivers. ⭐ **TWO TERMS, each replacing something underived:** `frostPermanence` gives the budget the temperature term it never had (MEASURED identical at 0.802 from 100 K to 1200 K at the same volatiles), and `frostLatChill` derives the equator→pole gradient from ATMOSPHERIC PRESSURE (airless 0.60 / Mars 0.33 / Earth 0.168) where a flat 0.35 had stood for every world in the galaxy. The shader's snowline also became mean-conserving at all THREE call sites — the sphere-average of sin²(lat) is exactly 1/3, so `uPlanetTempEq` is now the world's MEAN instead of pinning its EQUATOR at the mean. ⭐⭐ **THE METRIC WAS THE HARD PART AND THE CONTRACT HAD IT WRONG:** `frostCover` is an OPACITY, not an area, so EXTENT (temperature) and WHITENESS (volatiles) are independent — a pale wash over 61% of a world scored the same as a white cap over 10%. Two of the seven ACs were written before I understood that and are recorded as MISSED/RESTATED rather than retuned to hit invented numbers (`DEVIATIONS.md` §1–2). ⛔ **AND THE NEXT THING IS ALREADY NAMED BY ITS OWN MEASUREMENT:** AC-4 ("in high places") passes — 180 K spread between high and low ground at one latitude — but the number it forced me to measure says the ALTITUDE term is now dominant: `uFrostLapseRate` spans **215.6 K** across a world's relief (Earth: Everest is 58 K), and on `Rocky (Earthlike)` the sea-level snowline yields nothing while 33.8% of the surface is painted by altitude alone. **That is last session's lesson repeating — I fixed the term I was looking at and a different one took over the answer.** It is the second of the two knobs the follow-up named, still 0.3 with nothing behind it. ⚠ Zero of the 118 genuinely frozen bodies move; Frozen/Titan/Europa are byte-identical. Suite: 20 pre-existing failures before and after, zero new — fifteen ratchets fired first and every one was ANSWERED, not loosened (four shipped fixtures re-captured values-only, the new driver name declared at each gate). ⭐⭐ **AND HIS QUESTION *"what's wrong with the mountain snow?"* RETIRED MY OWN RECOMMENDATION — `FOLLOWUP-altitude-snowline.md`.** I could establish that `uFrostLapseRate` is underived and now dominant (33.8% of an Earth-like world painted by altitude alone, the latitude snowline contributing zero). I could NOT establish that its 216 K span is TOO MUCH, and that is the finding: the engine's own two instruments DISAGREE about the height field's physical scale. `sampling.js:44` calls its km conversion exact, and applied to the field tapped off the compiled shader it makes the tallest point **1.99× the body's own radius** — a mountain twice as tall as the planet is wide. ⛔ So "216 K is too much" was UNPROVEN, not established, and retuning a constant away from an outcome Max just called a success — on a comparison I cannot make — is how a working thing gets broken. ▶ **The real next item is smaller and sharper: resolve the height field's km scale.** It is not a frost problem; it sits under every altitude-dependent law in the engine (frost, glacial confinement, sublimation, the LOD approach criterion).

> ## ✅✅ 2026-09-04 (session 3, later) — ⭐⭐ **THE GAME GATES ARE SHIPPED — Max's UAT: *"Otherwise seems about right"*, with ONE defect named and fixed in the same breath: *"Everything still has the dunes drawn across the surface."*** `3136f46` + `6de7191`, lane A, pushed. ⭐⭐ **HIS UAT SHARPENED THE BAR, and this is the part to carry forward: "reads the accumulated surface" is NOT ONE PROPERTY.** PERMISSIVE gates (`lowGround`, `gentle` — smoothsteps DOWN from relief) evaluate to **1** on flat ground, so the gate **OPENS**; RESIDUAL gates (`gradIn − gradBase`) evaluate to **0** and the combiner returns before writing. Turning the eight surface-blind features off FLATTENED the field dunes and karst were being trusted to read, both terms went to 1, and only the province floor (0.30 / 0.25) was left — **the first cut made dunes WORSE, by its own doing.** The dune combiner's own comment had conceded the shape all along: *"No absolute-h floor anywhere ⇒ low-relief worlds pass the masks."* **So dunes and karst went OFF too (karst was NOT named by Max — ruled on the character-identical gate rather than waiting for the next walk to find it; one flip returns it). TEN of the eleven are now OFF; `massWasting` is the only survivor, and the registry now says WHY: it is the one whose terrain read CLOSES as relief goes to zero.** ⭐ **The rule for the next feature that asks to be switched on:** reacting to relief is not the same as reacting to its absence — ask which way the gate moves as relief goes to zero. Max: *"turn off the world engine features that have not yet been developed/worked into the pipeline … then we'll flip them on once developed. Part of the point of wiring these up is I want to be able to continue developing the features in the lab then seamlessly be able to switch them on in game when ready."* **His bar, ruled over two alternatives AND with the cost stated: GROWN FROM THE ENGINE, NOT PAINTED ON** — a feature is ON in the game only if it reads the bake's ACCUMULATED LANDFORMS, not just the province mask with a floor under it. ⭐⭐ **THE SWITCH ALREADY EXISTED AND HAD NEVER BEEN USED:** `solidRelief` declared all eleven gate names, `gatesFor` answered `GATE_RULINGS[g] ?? true`, and `index.js:267` said so outright — *"all eleven default ON and none is ruled off today."* The work was the RULING, plus splitting two kinds of ruling apart. **Which side of the bar each feature falls on was MEASURED PER COMBINER in `height.glsl.js`, not taken from prose:** karst (`:1194`) and dunes (`:1255`) read `lowGround` off the running height, mass-wasting (`:1364`) reads `gradIn − gradBase` — those three STAY ON; the other eight go to **0 bodies** (mountains 103, canyons 124, scarps 122, plateaus 124, tessera 46, lava 103, sublimation 40, dust 68). ⛔⛔ **TWO KINDS OF RULING, AND ONE MAP COULD NOT CARRY BOTH — both failure modes MEASURED, not theorised.** CONVERGENCE (`GATE_RULINGS`, both front-ends: the feature is wrong everywhere — `terminator` is the only one) vs DEVELOPMENT (`GAME_GATE_RULINGS`, game only: fine in the lab, not ready here). Flipping the lab composer to ALL_ON resurrects the terminator and **reverses Max's 2026-07-16 ruling**; leaving the lab on the ruled map **SILENCES THE LAB** — which is what the first cut did, caught by `material-parity-list` as eight names leaving `LEDGER.labVarying`. ⛔ **AND THE FIRST CUT OF THE INSTRUMENT WAS VACUOUS:** it fed `ray-pack-corpus`'s already-RESOLVED numbers back into `resolveDriver`, so the gate branch never saw a descriptor — it reported `mountains: 103` with the gate ruled OFF **and its own liveness check PASSED**, because both columns were the same ungated number. The control is now discriminating (RULED must DIFFER from ALL_ON, and go to zero). ✅ **AC-3 VERIFIED LIVE IN THE LAB — the claim the whole workstream exists for:** with the lab's own checkboxes on, every gated-off feature is alive there (mountains 0.4775, chasma 0.0362, scarps 0.0762, plateaus 0.0258, lava 0.15, sublimation 0.0222). Suite at the pre-existing baseline (same 8 files / 20 tests, **zero new failures**) + 9 new passing. Documented in the PLAN **§ THE GAME GATES**; source of truth is `solidRelief.js` `SOLID_RELIEF_GAME_GATES`, one row per feature carrying its state, its reason and **its exit condition** — tests fail if a gate has no row or an OFF row names no exit. ⚠ **Every OFF row is DEBT with a named exit, never a steady state** (`converge-dont-declare-divergence`); the exit is FOLLOWUP **(b)** — make the combiner read the accumulated relief, as the three ON rows already do.
>
> ▶ ⭐ **MAX'S UAT IS THE GATE, and it is the one that matters here because THE GAME IS DELIBERATELY BARER.** The question is whether the solid worlds read as *simpler* rather than *damaged* — no half-drawn features, no world that now looks like a bare sphere when it should not. ⚠ **AC-2's live half was NOT captured:** the gate is proven headless with a discriminating control, but repeated attempts to read the mounted game material's relief uniforms in the browser (`bodySurfaces`, `previewPack`) returned nothing usable, so "the pixels are gone on a live body" is asserted, not seen.
>
> ## ✅✅ 2026-09-04 (session 3) — ⭐⭐ **THE LABEL SPLIT IS SHIPPED — Max's UAT: *"1 works"*. ARC (1) OF HIS RULING, CLOSED.** Pushed. `23453f4` + `6d7f427`, lane A, NOT pushed. **`PlanetGenerator.type` was doing two jobs with one name — the SAME shape as the `iceFraction`/`volatileFraction` split the session before.** It is a zone-weighted roll over real Kepler rates (so NOT "randomized" — Max's premise was half right) but it is drawn FIRST and then *chooses* the radius range, the mass, the atmosphere, the cloud/ring chance, the moon count and the palette. Being UPSTREAM is what makes "read the physics" a LOOP. **The split:** `type` keeps its real job as the FORMATION SEED; **`worldClass`** is derived from the finished body's condition vector and is what everything DESCRIPTIVE reads. ⛔ It is assigned on the LAST statement before `generate()` returns, and that position IS the enforcement — feeding it into a law means visibly moving it up past every quantity it came from. **Five surfaces rewired:** info panel · both orrery colour tables (3 reads) · the seed search's habitable filter · **and the cockpit TYPE row (`CockpitSnapshot.js:271`), a FIFTH surface found only by driving the game** — in HELM that row IS "the panel" from Max's own criterion. **MEASURED, 200 seeds / 476 solid planets, every count to zero:** misnamed warm-wet worlds **11 → 0** · "ocean" that is hot and dry **7 → 0** (median was 355 K) · "lava" below 400 K **15 → 0** · "ice" above 273 K **6 → 0**. ⭐ **`rocky-126` planet 1 — the exact world Max UAT'd with *"yep, it works"* — was labelled an ICE WORLD and now reads TERRESTRIAL** (1.08 R⊕, 293 K, volatiles 0.310). **Every threshold is an engine constant already named for that meaning** (`compositionClass`, `biosphereOf`, `T_ICE_HI`, `BIO_T_LIMIT`, `BIO_P_HI`, `T_LIQUIDUS_BASALT`); `T_MELT_LO` was tried and REJECTED — its own comment says it is a melt-*sheen* rendering threshold and it called 100 of 476 bodies lava. **Two candidate classifiers measured and thrown out:** the stored `habitability` score (D15) has **no water term at all** — 189 of 462 non-habitable bodies score at or above the lowest real one, a Venus medians 0.80 — and `inSeededBand`'s Earth-MASS gate cuts 14 habitable worlds to 5; **Max ruled warm wet super-Earths COUNT**, so the classifier carries its own radius bound. ⛔ **ADDITIVE, PROVEN WITH A LIVE CONTROL:** with `worldClass` stripped all 1,577 body records hash identical to HEAD, and with the strip disabled the hash MOVES — so the equality is the strip's doing, not the script's blindness. Instrument B's **DRAW STREAM channel stayed GREEN** while value+shape went red, the signature `body-identity-fence.test.js` documents as proof an addition was additive; baseline re-blessed on that basis. Suite at the pre-existing baseline (the same 8 files / 20 tests fail at `6a6c0e4`, none touched here) + 14 new passing. Records: `WORKSTREAMS/derived-world-class/` — contract (`verifying`, per-AC verdicts), intent, LIVE-CHECK, DEVIATIONS, and two re-runnable instruments (`census.mjs`, `additive-check.mjs`).
>
> ⚠ **ONE AC NOT CLOSED, AND IT IS NOT THIS CHANGE: the orrery dots.** The lookup is verified three ways (the draw site, `displayClassOf` called in-page on live records, and ~300 instrumented frames showing class colours filled) — but a KNOWN habitable world was never seen drawn green, because **the orrery's planet list does not match the scene's**: it drew **Sol** while the scene was `rocky-126`, and rocky/rocky/venus/ice for a venus/terrestrial/rocky/sub-neptune system. That is a divergence about WHICH SYSTEM the orrery renders and it predates this work. ⛔ It matters for the goal — "spot a habitable world on the map before you fly to it" cannot be trusted while the map may describe a different system than the one you are in. **Own session.** Also found and NOT fixed: a **second** one-name-two-meanings pair (`compositionClass` cuts carbon at C/O > 1.0, `deriveComposition` at 0.8), **`carbon` has no physical referent in this galaxy at all** (C/O tops out at 0.769 while the roll labels 126 bodies carbon), and a month-old workflow worktree inside `.claude/worktrees/` that doubles single-file vitest runs.
>
> ▶ **NEXT, per Max's order: (2) THE SNOW BUDGET** — `frostMaxCoverage` has NO temperature term; the whole test is delegated to `uFrostLatChill = 0.35`, a hand-set "lab knob" that puts the sea-level snowline at 26° latitude on a 293 K world (56 % of the surface; Earth's is ~66°/~10 %). A LAB defect of long standing. → `WORKSTREAMS/volatile-delivery/FOLLOWUP-frost-budget.md`. Then **(3) the lighting engine / F52**.
>
> ## ✅✅ 2026-09-04 (session 2) — ⭐⭐ **THE GALAXY HAS EARTH-LIKE WORLDS IN IT. VOLATILE DELIVERY IS SHIPPED — Max's UAT: *"yep, it works"*.** Pushed at `0982ea1`. The defect: `PhysicsEngine.deriveComposition` made `volatileFraction` a pure function of `orbitAU / frostLineAU` while `T_eq` is the SAME variable inverted, so *temperate ⇒ dry* held **by construction** — over 1,183 solid bodies the temperate and wet sets never intersected once. **THE FIX IS A FIELD SPLIT, forced not preferred:** `MoonGenerator.js:266` derives `moon.massEarth` FROM `composition.density` and `checkTidalLock` reads that mass, so a rescale would have re-rolled every moon's lock flag. `iceFraction` (accreted bulk, old law VERBATIM, still density's only input) split from `volatileFraction` (surface inventory, new law at §3b: `max(floor, in-situ + softCap(delivered × retained))`). Both system inputs already sat on `zones` — metallicity and **`solidInventory`, computed and marked UNREAD BY DESIGN since plan B3; this is its first reader.** Constants SOLVED from six real bodies: **Earth 0.1500 · Venus 0.0200 · Mars 0.1082 · Moon/Mercury 0.0200 · Titan 0.4400 · Europa 0.3100.** **MEASURED, 200 seeds: temperate ∩ wet 0 → 14 · plate band 0 → 5 · `plate()` 0 of 124 → 4 of 124 · corpus wet worlds 2 → 7 · temperate planets past the bone-dry floor 14 → 36 of 76.** ⛔ **NOTHING ELSE MOVED** (density, moon mass, `locked`, radius, orbit, type, eccentricity, atmosphere byte-identical on all 1,577; zero edits under `src/worldengine/`), and the **ATTRIBUTION CONTROL** proves it: restore that ONE field and all 12,481 resolved driver values return to byte-identity, residual 0. All four instruments green. Records: `WORKSTREAMS/volatile-delivery/`.
>
> ▶ ⭐⭐ **MAX'S UAT OPENED THE NEXT TWO, AND HE RULED THE ORDER: (1) THE LABEL, (2) THE SNOW, (3) the lighting engine.** His words: *"it sounds like we have a bunch of systems here that are not talking to each other. The most obvious candidate for a fix is the label."* **(1) THE LABEL SPLIT — `PlanetGenerator._pickType` is the SAME BUG SHAPE we just fixed: one name doing two jobs.** It is a zone-weighted roll drawn FIRST, and it is **UPSTREAM** — it picks the radius range, hence mass, plus atmosphere strength, cloud/ring chance and moon count — so it helps CREATE the history it fails to describe, and "make the label read the physics" is a loop, not a relabel. Split it: the roll stays a formation seed, a DERIVED classification is computed from the finished body. **Measured over 200 seeds: 11 of the 14 Earth-like worlds carry some other label (6 `ice`, 2 `rocky`, 2 `carbon`, 1 `fungal`; only 3 are `terrestrial`), and all 7 worlds the game calls `ocean` are hot and dry (median 355 K).** The galaxy cannot tell you where its habitable worlds are. ⚠ The label does NOT paint world-engine bodies (no `planetType` uniform on that material) — this changes the HUD/orrery, not the surface. **(2) THE SNOW — `frostMaxCoverage` has NO temperature term** (`smoothstep(0.05, 0.4, volatileFraction)`); the whole temperature test is delegated to the shader's `uFrostLatChill = 0.35`, a hand-set "lab knob", which puts the sea-level snowline at **26° latitude on a 293 K world** (56 % of the surface; Earth's is ~66°/~10 %). ⭐ A LAB defect of long standing, not ours — the lab's own `Ocean (temperate)` preset scores a **0.945** budget and always has; the game simply never had a warm wet world to show it on. → `WORKSTREAMS/volatile-delivery/FOLLOWUP-frost-budget.md`. ⛔ Block B (the `locked` gate, 74 % of bodies, shuts out tidally-locked temperate worlds) stays parked in its own workstream, per Max.
>
> ## ✅✅ 2026-09-04 — ⭐⭐ **THE `solidRelief` DECK IS SHIPPED — Max's UAT: *"They work … So they're wired up"*** (build `1428533`, A/B `4b7e6dc`, ship `8b76e86`; lane A, NOT pushed). Driver pack **#11**, `src/worldengine/drivers/solidRelief.js`, forwarding **23 names across ten F-rows** — F1 mountains, F4 canyons, F5 scarps, F6 plateaus+tessera, F8 lava, F15 dunes, F16 dust, F18 sublimation, F19 mass-wasting, F21 karst. **Population, recomputed from the pack's own output:** chasma + plateau 124/124, scarp 122, mountains + lava 103, karst/dunes/dust 68, tessera 46, sublimation 37. ⭐ **124 DISTINCT seeded orientations over 124 bodies** — `labCore.reliefAxesFor` + its `labPackCtx` spread; off the condition bundle every world in the galaxy would have shared ONE rift direction, the failure `solidFeatures.js` refused F10's axes over. **Two modules had to exist first:** `base/surfaceProcesses.js` (the four laws that lived ONLY in `world-engine-lab.html` — why the audit found them unforward*able*) and `reliefAxesFor`. **Nothing else moved:** 0 of 10,492 pre-existing driver values; the suite's failing set byte-identical to the parent's 20 per test ID both ways; all four instruments green.
>
> ⛔⛔ **THREE AUDIT ROWS CORRECTED BY MEASUREMENT.** **F46 bioluminescence is NOT queue-(c) dead** — `habGate ≡ 0` is false, `habitability` is real on **68 of 124** (min 0.55) so the gate is wide open; it is held out because the AMOUNT is a lil-gui slider with no law on either side. **F1's RUNTIME gate is live on 103 of 124** — the block is in the BAKE's `plate()` closure, not the gate; shown this, Max reversed the hold-out (*"2 yes"*) and mountains rode along. **F43 IS dead, 0 of 124**, but by `world-engine-lab.html:2748`'s four-term predicate, not by `retained === false` (which never occurs).
>
> ▶ ⭐⭐ **THE FOLLOW-UP MAX OPENED IN THE SAME BREATH, and it is the next arc's other half:** *"these features … are just applied over the underlying world engine generative models and don't actually communicate with that process AFAIK"*. **Measured, not paraphrased** (`WORKSTREAMS/solid-relief-deck/FOLLOWUP-not-fully-developed.md`): communication DOES exist — all eleven read `provinceWeight(PROV_*)` off the bake's craton/orogen/basin cube with per-feature polarity, and karst/dunes/mass-wasting also read the accumulated surface — **but** a 3-channel mask is not a landform, the floors run 0.15–0.50, **EIGHT of the eleven read neither the accumulated height nor its gradient**, and **NONE reads the generative geometry the bake actually draws** (`plates.js` uplift lines, `stagnantLid` rift corridors, `shellRelief` lineaments). Three separable jobs: (a) a per-feature suitability FIELD instead of a mask with floors; (b) make the eight surface-blind combiners read the accumulated relief; (c) let the generative writers PLACE the landforms. ⭐ **(c) IS THE SAME QUESTION AS THE NEXT ARC** — F1 mountains, *"why does `plates.js` claim 0 of 124"*, is that job's generative half. ⛔ This does NOT reopen the workstream: his greenlight framing was *"much more development is needed but this lays the groundwork."*
>
> ⚠ **THE INSTRUMENTED LIVE PAIR WAS NEVER CAPTURED** — Max ran the walk himself and his UAT closed it, but the ON/OFF pixel evidence is absent: the first attempt read 60.5 % of pixels changed (the planet ROTATES between two shots — `_lab.freezeFrame()` is the control) and was discarded, and chrome-devtools dropped before a frozen pair could be taken. What IS confirmed live on rocky-2: 8 materials registered, `uniformValues === packValues` on all eleven masters, 7 distinct `uOrogenyAxis` across 8 bodies. → `WORKSTREAMS/solid-relief-deck/live-pair-2026-09-04/STATUS.md`. **A/B key `[`** (square bracket). **Declared deviations** (`DEVIATIONS.md`): the lab's four laws become seed-live (`_fp`→`_dp`) and F19's repose becomes radius-aware — both the lab catching up to rulings made elsewhere; no delta in the game. **The preset-NAME relevance table does not travel** (Max 2026-07-19: *"presets remain dev fixtures … NOT the product"*), so the game renders five of these on MORE bodies than the lab's preset view shows. Record: **PLAN § THE SOLID RELIEF DECK, WIRED** (EOF).
>
> ▶ **NEXT, per Max's standing order "1 then 3 then 2" with (1) now closed: (2) F1 mountains — why `plates.js` claims 0 of 124 bodies.** A GENERATION question, and per `feedback_physics-first-worldengine-scoping` it is derived, not asked. Related and possibly one cause: `writePassiveMargins` never fires (plate-path only) and there is no bake crossover at all below 0.22 R⊕ (41 bodies). Then **(3) the lighting engine / F52**.
>
> ## ⭐ 2026-09-03 (session 4, later) — **F3 EJECTA RAYS = ✅✅ SHIPPED — Max's UAT 2026-09-03 on rocky-13's fifth planet's ice moons, his words: *"they do read as spokes"* (verified `6d70d56`, verdict `2e60453`) · F35 TERMINATOR = ✅✅ SHIPPED `2e60453` — Max's UAT 2026-09-03 on rocky-2 (key `.`, half phase): the band he called *"really bad as usual"* is the one the game had been drawing since 2026-08-21 against his 2026-07-16 ruling; **the change IS the removal**, and he shipped it (*"1 yes"*). The band is now OFF in both front-ends by one constant; its LOOK is F52 / the lighting engine's. ⭐⭐ **THE WIRING PASS IS DONE** — every F-row the lab has reaches the game except queue (c) (F43 `uFacetStrength`, F46 `habGate`), which is world-GENERATION work. The parked development list goes back to Max.** ▶ **MAX'S WALKS:** (1) rays — `?system=rocky-13`, the FIFTH planet's ice moons ("f IV" 1.06 R⊕, "Dione" 0.96 R⊕), key **`Y`** while moving in, then any planet with air and `Y` again (nothing changes); (2) terminator — `?system=rocky-2`, the second planet at a crescent, key **`.`**: the band appears (the law value) and vanishes (what ships, OFF, his 2026-07-16 ruling). Overturned this session: the rays' input is LIVE on every plain moon (the plan's 100 %-hasAtmo claim was planets-only); the game DREW the terminator band he turned off in the lab. Instrument A re-recorded at `eb58ac9` (its old record never covered `src/**/__tests__`; ⚠ nested worktrees double a bare vitest run — ALWAYS `--dir tests`). Records: PLAN § THE EJECTA RAYS, WIRED and § THE TERMINATOR, CONVERGED (EOF); backlog QB-25..28.
>
> ## ⛔⛔ 2026-09-03 (session 4, after the ship) — **THE COVERAGE AUDIT: "is all world-engine rendering in the game?" — NO, and the F-spine could not answer it because it counted ONE of the two roads.** Max's condition for starting the lighting engine. Measured against the code at `c1816e3` (§ THE COVERAGE AUDIT, MEASURED at the PLAN's EOF; evidence `WORKSTREAMS/coverage-audit-2026-09-03/`). **Road 1, pack uniforms:** 10 of 40 feature master gates published. **Road 2, the relief bake:** `writeBodyRelief`'s 5-way dispatch runs the LAB's own writers in a worker and CROSSFADES into the base field, so F4/F6/F8/F9/F10 are already in the game as baked geometry (10–103 bodies each) — five rows the spine scored ❌. **Rows corrected:** F37 aurora was flatly wrong (the game runs the lab's law, 46/124); F7 (103/124) and F17 (28/124) were stale. **What is missing is CARRIAGE, and it is ONE pack-shaped job:** thirteen master gates (`uMountainAmp`, `uChasmaDepth`, `uScarpStrength`, `uPlateauStrength`, `uTesseraStrength`, `uLavaCoverage`, `uDuneDensity`, `uDustDepth`, `uSubStrength`, `uMassWastDensity`, `uKarstDensity`, `uFacetStrength`, `uBioCoverage`) that `labCore.deriveUniforms` computes every frame and no pack forwards — a `solidRelief` deck on the `fluvialDeck` precedent (11 live inputs + 2 queue-(c) dead). **Three gaps nobody had named:** no bake crossover at all below 0.22 R⊕ (41 of 124 bodies — every small moon), `writePassiveMargins` never fires, the crater cube is all-zero on 82/124. **F1 mountains is GENERATION-blocked** — the plate path claims 0 of 124 bodies, so wiring `uMountainAmp` would light a law nothing selects (risk #13). ▶ ⭐ **MAX'S RULING 2026-09-03: "1 then 3 then 2"** — **(1) the `solidRelief` deck** (the thirteen gates; his condition), then **(2) F1 mountains: why `plates.js` claims 0 of 124 bodies** (a GENERATION question, before any F1 wiring), then **(3) the lighting engine / F52**. ⛔ Do not reorder; he was told lighting is independent and still put it third. Handoff: [`FEATURES/handoff-2026-09-04-solidrelief-deck-then-mountains-then-lighting.md`](FEATURES/handoff-2026-09-04-solidrelief-deck-then-mountains-then-lighting.md).
>
> ## ⏳ 2026-09-03 (session 4) — ⭐ **THE LAST TWO PARTIALS ARE SCOPED — F3 ejecta rays + F35 terminator gradient — AWAITING MAX'S GREENLIGHT** (`250e712` scope, `+` the review amendments; lane A, NOT pushed). Two premises overturned by measurement: **F3 is NOT queue (c)** — the airless input is LIVE on 56 of 124 corpus solid bodies (every plain moon; the PLAN's 0/800 count was planets-only, corrected 2026-08-20 and never absorbed), rays can render on 52 (the 4 others have no crater schedule), one shared law module + three names in the crater block, A/B key **`Y`**, walk = `?system=rocky-13`, the FIFTH planet's ice moons. **F35's missing 4th piece is `uTermBypass`** (a display knob) — but the game DRAWS the band on every body with air while the lab has it OFF on all 13 presets by **Max's 2026-07-16 ruling ("disable terminator gradient totally; it doesn't work … lighting engine")**; scope = converge on OFF via one declared value at BOTH lab producers (the preset dressing table + the literal) and a RULED gate policy, A/B key **`.`**, walk = `?system=rocky-2`, second planet at a crescent. Direction of convergence (OFF) is the one product call he can overrule at greenlight. Contracts: `WORKSTREAMS/wire-ejecta-rays-lab-into-game/`, `WORKSTREAMS/wire-terminator-gradient-lab-into-game/` (both `scoping`, review-amended, validated). Handoff: [`FEATURES/handoff-2026-09-03b-last-partials-scoped-greenlight-next.md`](FEATURES/handoff-2026-09-03b-last-partials-scoped-greenlight-next.md).
>
> ## ✅✅ 2026-09-03 — ⭐⭐ **THE STORM SLICE (F27/F28, `uStorm*`) IS SHIPPED** — verdict `wf_14116a31-eca` at `f9da55d` (unit PASS · integration PASS); **Max's UAT on rocky-2, his words: *"It works, red dot appears"*** — the WIRE is accepted. Contract `shipped` (+ `shipped.followUp`), FEATURES.md row added (Rule 3). **Queue (b) is EMPTY.** ▶ NEXT, per Max's 2026-09-02 ruling (wire every world-engine feature before developing any): **queue (c) as world-generation inputs allow** (`uRayBrightness ≡ 0` for F3's rays, `uFacetStrength ≡ 0` for F43, `habGate ≡ 0` for F46 — world-gen work, not wiring) **and the partials F3 (4/7) / F35 (3/4)** — each its own `dev-collab-scope` workstream in a fresh session. ⚠ QB-16 (blend) + QB-18 (ink-in-water) + QB-24 (vigor saturation) are the storm backlog, deferred by the ruling. Record: **[`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE STORM SLICE, WIRED**; handoff [`FEATURES/handoff-2026-09-03-storm-slice-verified-max-walk-next.md`](FEATURES/handoff-2026-09-03-storm-slice-verified-max-walk-next.md). Lane A pushed with this ship (see the handoff's push line).
>
> ## ✅ 2026-09-02 (session 2) — ⭐⭐ **THE RIVER ROUTER IS SHIPPED** — the live pair measured (AC-4 relief, AC-5 rivers, AC-6 lab clause), verdict PASS at `5c952e3`, **Max UAT on rocky-4's outer planet: *"The rivers are not fully developed but the wiring appears to be working here. The wiring for U is also working"*** — the WIRE is accepted. ▶ NEXT, ⭐ **MAX'S RULING 2026-09-02: *"I want to continue wiring up all the features from the world engine before we try to further develop any of them"*** — so the next workstream is the NEXT UNWIRED F-ROW (PLAN queue (b) = **F27/F28, the storm slice, `uStorm*`**; then queue (c) as world-gen inputs allow; partials F3 4/7, F35 3/4), scoped with `dev-collab-scope`. "Not fully developed" is DEFERRED by that ruling, logged in the contract's `shipped.followUp`.
>
> **Record: [`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE RIVER ROUTER, WIRED → (viii) The live pair: MEASURED** — every pixel count, the sabotage arms, the controls. Workstream `WORKSTREAMS/wire-river-router-lab-into-game/` — contract **`shipped`** (`5c952e3`; verdict `1ee0219`: unit PASS · integration PASS; AC-8 on Max's word, quoted in the contract's `shipped.uat`). FEATURES.md row added (Rule 3).
> On `feature/world-engine-production-L1` (lane A, `~/projects/well-dipper`). Handoff: [`FEATURES/handoff-2026-09-02b-live-pair-measured-max-uat-next.md`](FEATURES/handoff-2026-09-02b-live-pair-measured-max-uat-next.md). The province cube is HISTORY: shipped `ccee0d1`, Max UAT 2026-09-02 — *"it does read as a crust and coheres"*.
...
>
> **⛔ What the live check overturned — the CORPUS RECORD, not the wire.** Both corpus tests read a PLANET-CLASS moon through its wrapper entry (trap 3: an entry wrapping `planetData`), where `T_eq` defaults to 288 K, so two relict planet-moons (rocky-14 p0m0 at 409 K, rocky-15 p0m1 at 455 K) were counted wet. Fixed in both builders and re-measured: **wet 2 · relict 66 · airless 56** (routed still 68); erosion on 124 of 124; outflow live on 68; the F13 ramp saturates on 62 of 66 relict (QB-23). **The two wet worlds in the whole corpus are `rocky-2` p4 (0.446 R⊕, carbon) and `rocky-4` p5 (0.449 R⊕, carbon)** — no wet world is Earth-sized. ⚠ The same wrapper read sits in the province suite's builder: read through `planetData` fresh from the generator the two planet-moons are REFUSED by `labPipelineAdmits` ("no `_systemSeed` / no `_ordinal`") while the live game admits and bakes them, because the mount path stamps the inner record — logged for the province lane, not touched here.
> **AC-4 relief, `rocky-14` p0m0 `body.planet.64b466` (0.948 R⊕, relict), 3 radii:** ON vs OFF **124,960 px** differ, all inside the disc, max Δ 91; floor 0; gas control 0 px; sabotage (1×1 placeholder cubes at the live strength) is a THIRD state — 130k px from OFF, 132k from ON — because the crossover is strength-weighted, not a mix knob; the contract clause is amended in place. **AC-5 rivers, `rocky-4` p5 `body.planet.8ad228` (0.449 R⊕, wet), 3 radii:** `uSeaLevel` 0.0028 = the record's solved sea (not −1), `uCoastStrength` 1, the carve cube the record's 1024 px one, the ribbon a visible child at r × 1.0014 (56,028 verts); ON vs OFF **6,141 px** (4.29 % of the disc), max Δ 227, 0 outside; ribbon alone 2,103 px (visible, not buried), carve alone 4,671; sabotage 228 px from OFF, 6,174 from ON; airless control 0 px. `U` on the same wet body: 48,928 px at strength 0.385 — **both keys say something on this one world.** **AC-6 lab clause:** the lab loads clean, `route()` binds real 1024/256/256/128 cubes, the Rivers toggle shows the ribbon (8,736 px).
> **Driver pack #9** `src/worldengine/drivers/fluvialDeck.js` — F11/F12/F13/F14/F20, ten uniforms, three gates, read back by the lab; it carries ROOT-0 fix 1's TWO erosion spellings (without them the corpus reads 2 wet / 0 relict / 122 airless). **One worker message per body** carries province + relief + crater + carve + ribbon; `attachLabBake` binds them on first draw. Keys **`J`** rivers · **`U`** relief · **`V`** province.
> **VRAM** 57.3 MB per ROUTED body + 7.0 MB per further solid one, ≈ 204 MB mean / ≈ 300 MB worst per system (unchanged — routed = wet ∪ relict = 68). **Costs, live, worker, one sample each:** wet 146.8 ms (route 110.3) · relict 82.2 ms (route 62.5).
>
> **Gates:** `river-bake-host` + `driver-pack-fluvialdeck` suites green (69/69) after the corpus fix · the workflow re-run at this commit is the next step (verdict.json still reads `a737ffd`; Instrument A / B / C / citations as at `a737ffd` — re-measured by the workflow). ⛔ **ORRERY trap, sharpened:** a `frameBody` issued during the orrery ARRIVAL ZOOM (≈ 5 s after spawn) leaves the group hidden and nothing bakes — issue it again after the zoom.
>
> ### ⭐ OPEN FOR MAX
> 1. **Your walk (AC-8) — it is yours alone and no agent passes it.** In the game: `_lab.spawnProceduralSystem('rocky-4')` is Claude's route; yours is to warp to a `rocky-*` system and fly to **`rocky-4`'s outermost planet** (0.45 R⊕, the carbon world, named in the orrery) — tap **`J`** and **`U`** *while moving in*: **"do the rivers sit in their own valleys and drain into a sea that looks right, with deltas at the mouths — one coupled system, not blue lines painted on a ball?"** Its sea is small (histogram 35 % at a solved level near 0) and the world is Titan-cold; that IS the population. For the Earth-sized relief look, `rocky-14`'s big moon (relict — carve + outflow channels, no sea) is the other body.
> 1. **Nothing blocking.** Your ruling (wire everything first, develop nothing yet) is recorded; "the rivers are not fully developed" is parked in `WORKSTREAMS/wire-river-router-lab-into-game/contract.json` → `shipped.followUp` with the candidates already on the books (QB-21/22/23, the view-dependent river LOD, the small-body width law). It comes back when the wiring pass is done.
> 2. Three calls that are yours, all recorded and none of them blocking: **QB-21** — every wet world gets the same 35 % ocean, because the router's target is a lab UI constant; **QB-23** — F13's outflow ramp saturates, so 62 of 66 relict worlds render a megaflood; and **only 2 of 124 corpus bodies come out wet** under the lab's own gate — is ~1.6 % the density you want rivers at? ⭐ **And a fourth, a phone question:** ≈ 200 MB of cube maps per system on average, up to ≈ 300 on the worst — your desktop will not notice; **the phone is the only instrument.**
> 3. Push: lane A pushes are confirmed per batch — three commits from this session (`5c952e3`, `1ee0219`, + the ship commit) go up on your word. ⛔ never `master`.
