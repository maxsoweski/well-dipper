...
>
> **HEALTH:** `src/ui` **922 passed / 44 files** at `--testTimeout=60000` (was 767/35; at vitest's 5 s default ~17 contention timeouts, green alone), `src/cockpit` 698, `tests/` 20 failed across the same 8, `extract --check` 0, `NavComputer.js` 4711 (five folds: 156 / 1441 / 4355 / 4405 / 4415), `main.js` 15161 untouched. Every fix pinned by a test proved red on a named mutant. ⚠ **VERIFY-WORKSTREAM (full) running at `ab610fa` — verdict pending.** ⚠ **NO LIVE CHECK HAS RUN** — the `:5175` server was down all session; debug Chrome on 9223 is up. Pending live: frame time at design 2 PRISM zoomed fully out (the depth cues multiply the drawing ~5×), the real wheel and a real gauge drag, the ship from the running game's focus index, a real burn to a moon, and every capture for Max's eyes.
>
