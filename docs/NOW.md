...
> ## ▶ 2026-09-03 — ⭐⭐ **THE STORM SLICE (F27/F28, `uStorm*`) IS BUILT, FENCE-GREEN AND ITS LIVE PAIR IS MEASURED** — contract `verifying`; the verify workflow runs next, then `VERIFIED_PENDING_MAX`. Built `f8ea499` on lane A: driver pack #10 `stormDeck` (the writer was already shared; the lab's colour law, deck-height law and slot composer moved under src/ and the lab imports them back, byte-identical to its own pre-move output over 92 fixture rows); one pack-contract extension (rows → `vec4[8]`); A/B key `I`. Live pair on `rocky-2`: Saturnian ON/OFF 14,405 px inside the disc, 0 outside the body (1 limb px); Neptunian 5,046 px; sabotage a third state; restore exact; solid control 0; re-approach identical. Full suite: the same 20 pre-existing failures as the parent, none new. Record: **[`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE STORM SLICE, WIRED**. ⚠ QB-16 (blend) + QB-18 (ink-in-water) ship as-is by the 2026-09-02 ruling; QB-24 logged. After this: queue (b) EMPTY → queue (c) / partials F3, F35.
>
