...
> ## ▶ 2026-09-07 — **BATCH 1 STEPS 4 AND 5 SHIPPED. AC-6 IS CLOSED, BOTH HALVES, LIVE-VERIFIED.** ⭐ **THE CHROME IS NOW VISIBLE — TELL MAX TO RELOAD `localhost:5175/well-dipper/`.** Steps 1-3 were plumbing he could not see; the flight HUD and the targeting reticle are the first changes he can. ▶ **`3e9d899` step 4** — SupercruiseHud draws into the world buffer (475x240 on this window): bitmap text, four opaque cross arms around a 3-texel hole where a straddling 1-px stroke used to smear, filled two-tone bars, `SAFE TO DROP` clamped not shortened. Measured live: **zero semi-transparent pixels** in the whole canvas. ▶ **`bf2879e` step 5** — the targeting reticle and the cabin mask onto the same grid: selected-vs-tentative is now 2-vs-1 texels (before today, 3 and 4 CSS px both quantised to one block and **only alpha ever separated them**), the label goes through PixelText, and the off-screen chevron is rasterised by hand because `fill()` on a rotated path antialiases. Bracket columns measured `[-6..-3]`/`[3..6]` about the centre pixel; `bracketHalf / magnification` = exact integers; R1-R6 all pass. **Cabin cut: coverage INVARIANT across a 56x sampling range (0.5497→0.5504 from 41k to 2.3M px)** — the feared rib loss is not there — and agrees with the same-session independent ray oracle to 0.0063. ⚠ It does NOT match the build-time 0.587202 here and that is **not** attributable to the change: that constant is a 16:9 figure and this window is 1.977:1, with the gap identical at full mask resolution. ⛔⛔ **STEP 6 IS ON HOLD AND ITS GATE IS MET — THE HOLD IS A DEFECT IN ITS OWN SPEC.** The plan argues grain is its one surviving justification, then places the composite branch after the cockpit — which sits AFTER the grain block (`RetroRenderer.js` grain `:883`, cockpit `:898`). So step 6 as written buys the PALETTE and pays the whole structural price (two DOM canvases gone, shared-canvas mode in two classes, flipY/clear coin-flips, dead CSS, a broken `LabMode.js:72` registration). **Recommendation: fold it into batch 2, landing it in the same commit that moves the cockpit above the grain.** Nothing waits on it — AC-6 is closed without it. ▶ **NEXT:** Max reloads and judges the look (AC-9 is his alone), then batch 2 (cockpit panels) off the **42.84-row** panel figure. ⭐ Seven corrections to the batch plans are recorded in-tree at `docs/FEATURES/chrome-240p-BATCH-PLANS.md` §3.5 — including that step 5's `t - PX` fix as PRESCRIBED was off by one, and that step 4 needed a fourth test file (`FlightReadout.test.js`) the plan missed. Baseline unchanged all session: 20 failed / 8 files, the same eight.
