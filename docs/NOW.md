...
>
> **Record: [`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE RIVER ROUTER, WIRED → (viii) The live pair: MEASURED** — every pixel count, the sabotage arms, the controls. Workstream `WORKSTREAMS/wire-river-router-lab-into-game/` — contract **`verifying` → `verified` once verdict.json is re-run at this commit** (`liveEvidence` on AC-4/5/6 in the contract).
