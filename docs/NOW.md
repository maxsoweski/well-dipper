...
> ## ★ 2026-09-03 — **VERIFIED_PENDING_MAX `f9da55d`** — ⭐⭐ **THE STORM SLICE (F27/F28, `uStorm*`) IS WIRED: unit PASS · integration PASS (verify-workstream `wf_14116a31-eca`), AC-8 = Max's walk.** What it does to the picture: the game already placed the storms (the writer + the `aStorm` mask bake) and already carried every storm shader term, but the eight storm slots never arrived — `uStormCount` was 0 on every body. Now driver pack #10 `stormDeck` fills them from the lab's writer on every gas body; the lab imports its own colour law, deck-height law and slot composer back (byte-identical to its pre-move output over 92 fixture rows); one pack-contract extension (rows → `vec4[8]`); A/B key `I`. Live pair on `rocky-2`: the Saturnian ON/OFF moves 14,405 px inside the disc and none outside the body; the Neptunian 5,046; sabotage a third state; restore exact; solid control 0; re-approach identical. Full suite = the parent's same 20 pre-existing failures, none new. Record: **[`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE STORM SLICE, WIRED**; workstream `WORKSTREAMS/wire-storm-slice-lab-into-game/` (`verdict.json`). Handoff: [`FEATURES/handoff-2026-09-03-storm-slice-verified-max-walk-next.md`](FEATURES/handoff-2026-09-03-storm-slice-verified-max-walk-next.md). ▶ **Max's walk:** deep link `?system=rocky-2`, fly in on the Saturnian (third planet out) and tap `I` while moving in; then the Neptunian (sixth planet out); leave and return. ⚠ QB-16 (blend) + QB-18 (ink-in-water) ship as-is by his 2026-09-02 ruling; QB-24 logged. After his word: `shipped` + FEATURES.md row → queue (b) EMPTY → queue (c) / partials F3, F35, each its own scope in a fresh session.
>
