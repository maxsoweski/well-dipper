...
> ## 🔧 2026-09-02 (session 2) — ⭐⭐ **THE RIVER ROUTER'S LIVE PAIR IS MEASURED** (contract AC-4 relief A/B + AC-5 rivers A/B + AC-6's lab clause, all on clean loads through lane A's server) — rivers, carve, sea and ribbon are on a game body and differ ON/OFF on the body's pixels and nowhere else. ▶ NEXT = **Max's walk (AC-8)**, then `shipped` on his word.
>
> **Record: [`FEATURES/one-pipeline-two-frontends-PLAN.md`](FEATURES/one-pipeline-two-frontends-PLAN.md) § THE RIVER ROUTER, WIRED → (viii) The live pair: MEASURED** — every pixel count, the sabotage arms, the controls. Workstream `WORKSTREAMS/wire-river-router-lab-into-game/` — contract **`verified` — `VERIFIED_PENDING_MAX 5c952e3`** (verdict.json re-run light at `5c952e3`, run `wf_566611bf-dc2`: unit PASS · integration PASS · AC-8 deferred-to-max; `liveEvidence` on AC-4/5/6 in the contract).
...
> 3. Push: lane A pushes are confirmed per batch — say the word and this session's commits go up.
>
