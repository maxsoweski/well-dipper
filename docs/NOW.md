...
> ## ▶ 2026-09-03 — ⭐ **THE STORM SLICE (F27/F28, `uStorm*`) IS GREENLIT AND BUILDING** — Max: *"Greenlight"* on the scope at `ea18406`. Workstream `WORKSTREAMS/wire-storm-slice-lab-into-game/` (contract `building`). What it does to the picture: the game already places the storms (the writer + the `aStorm` mask bake) and already carries every storm shader term, but the eight storm slots never arrive, so `uStormCount` is 0 on every body — this wire fills the slots from the lab's writer through driver pack #10 `stormDeck`, one pack-contract extension (nested arrays → `vec4[8]`), A/B key `I`. Population read at scoping: 32/32 corpus gas bodies storm (26 warm / 6 dark; 22 pearl / 4 barge+oval / 6 scooter). ⚠ QB-16 (blend) + QB-18 (ink-in-water) ship as-is by the 2026-09-02 ruling. After this: queue (b) EMPTY → queue (c) / partials F3, F35.
>
