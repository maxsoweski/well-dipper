...
- **Objective:** with E9 (carve) disabled, two body bundles produce E6-only DEMs whose **host-field divergence** rises from ~0 (today) to above a to-be-tuned threshold — measured on the **reseed-invariant hypsometric distribution** with the master seed **and the Layer-3 seed-discriminator held constant**, so the metric is moved only by Layer 1's regime physics + Layer 2's geometry, **not** by a Layer-3 reseed (a reseed alone is "a different random map of the *same* world", not a categorically different one — §5 metric 4). For pairs that should differ in regime, the **regime-class histogram (§5 metric 3) must corroborate**. This is the pass/fail line distinguishing "different worlds" from "same world rescaled *or reshuffled*". (Same-regime pairs like frozen-vs-airless are distinguished instead by the carve axis, §5 metrics 1–2.)
...
4. **Host-field divergence — THE DECISIVE GATE (measured at a HELD seed, on the reseed-invariant component).** With the E9 carve overlay **DISABLED**, compute a distribution distance between two bundles' E6-only DEMs (`substrate.height`, `relief-substrate.js:9`), **with the master seed AND the Layer-3 seed-discriminator HELD CONSTANT across the two bundles** so that only Layer 1's regime physics and Layer 2's geometry can move it. Two sub-metrics, kept SEPARATE because they detect different things:
...
- **Objective (the pass/fail line):** **held-seed, reseed-invariant hypsometric divergence above the tuned threshold (§5 metric 4)**, with the master seed and Layer-3 discriminator held constant and E9 disabled — corroborated by a real **regime-class shift (§5 metric 3)** for cross-regime pairs — proving the FIELD diverges by physics, not the coat-swap and not a Layer-3 reshuffle. (Frozen-vs-airless and other same-regime pairs are distinguished by the carve axis, §5 metrics 1–2, plus composition-keyed layout.)
