...
**Date:** 2026-06-23
**Status:** DESIGN — multi-lens assessed + hardened 2026-06-23 (see Assessment note below); pending Max UAT.
...
> **Assessment note (2026-06-23).** A 6-agent multi-lens review (north-star fidelity / proc-gen-not-render / realizes-canonical-model / isolation / honesty-of-caveats + adversarial synthesis) returned verdict = **aligned-with-edits**. It code-confirmed ~25 citations accurate and the design faithful to all lenses, but found ONE load-bearing gap: the original decisive gate (raw host-field divergence) could be cleared by **Layer 3's seed re-key alone** — a decorrelated reseed saturates a per-cell distance with no physical change ("a different random map of the *same* world", a coat-swap-of-a-different-color). **Fix applied this revision:** the decisive gate now measures the **reseed-invariant hypsometric distribution at a HELD seed** (so Layer 1 regime + Layer 2 geometry must carry it), corroborated by the regime-class histogram **for cross-regime pairs only** (same-regime pairs like frozen-vs-airless are distinguished by the carve axis, not the field), with Layer 3's reseed demoted to a secondary metric and the early-exit hardened against reseed-only passes. Minor citation/clarity fixes (uvFlux `:170`, the eps `(3+NU)·0.5` constant, doc paths, L4 three-edit-sites note, the dev-collab-scope-not-warranted statement) also applied.

> **Build-revision note (2026-06-23, TDD build).** The 5-layer build *empirically corrected the decisive gate again.* The hardened gate above made **held-seed hypsometric** the load-bearing metric — but a 15-seed sweep during the L3/L4 build showed the cross-regime **hypsometric distance is seed-fragile** (for some master seeds two different-regime bodies have nearly-identical height *distributions*; 6/15 seeds failed). Height-*value* distributions don't reliably separate regimes — the divergence is **spatial/structural**. So the decisive gate was rebuilt on the metrics that are BOTH reseed-invariant AND robustly physics-carried: **PASS iff a body pair diverges on ≥1 of {tectonic regime (L1, `regimeHistogramDistance`>0.2) · hydrology/liquid-stability (L4, `|liquidStability|`>0.3) · fluvial carve (L4, `|carveFraction|`>0.05)}.** All three are invariant to a Layer-3 reseed, so "a different random map of the same world" still cannot pass (NULL: identical bundle → 0/0/0 → fail, *structurally*). **Directional anisotropy (L2, `directionalAnisotropy`) and held-seed hypsometric are now REPORTED corroboration, NOT gate terms** (anisotropy flips with regime sign → redundant with the regime term; hypsometric → too noisy). This *preserves the assessment's principle* (reseed-invariant + physics-carried + not reseed-foolable) with metrics that hold up empirically. Measured at lock: terrestrial-vs-europa regime 0.81; europa-vs-lava (same-regime) carried by hydrology 1.0 (regime only 0.18); terrestrial-vs-lava all three; NULL 0/0/0. §1/§5/§9 below reflect this; the detailed hypsometric prose in §5 metric 4 is retained as historical rationale but is **superseded** by this note.
...
- **Objective (per the Build-revision note above):** at one master seed, two body bundles are **categorically different** iff they diverge on **≥1 robust, reseed-invariant, physics-carried axis** — **tectonic regime** (`regimeHistogramDistance` > 0.2, L1) **OR** **hydrology / liquid-stability** (`|liquidStability|` > 0.3, L4) **OR** **fluvial carve** (`|carveFraction|` > 0.05, L4). All three are invariant to a Layer-3 reseed, so a "different random map of the *same* world" cannot pass, and an identical bundle scores 0 on all three (NULL → fail). **Directional anisotropy (L2)** and the **held-seed hypsometric distance** are *reported corroboration*, not gate terms (anisotropy is redundant with the regime sign; hypsometric is empirically seed-fragile). This is the pass/fail line distinguishing "different worlds" from "same world rescaled *or reshuffled*".
...
1. **The host FIELD is byte-identical in layout across bundles.** E6 seeds noise as `alea(seed + ':e6:' + epoch.name)` and `alea(seed + ':e6plateau')` (`relief-e6-tectonic.js:72-74`) — the bundle is **excluded** from the seed. The crustal-thickness blob is seeded `alea(seed + ':crust')` (`relief-base-step.js:46`) — also bundle-blind. The grain is **latitude-only** (`writeGrain` over `latDegOfRow`, `relief-e6-tectonic.js:34-46`). So only the scalar `baseAmp` and `radialStrain*` vary; the LAYOUT is fixed. The `relief-presets.js` BUILD-INTENT block (the `Deliberate non-goals` section, ~lines 18–20) already concedes this: *"Same seed + different preset = IDENTICAL landform layout, only rescaled in height."*

2. **The one qualitative lever is damped to inertness.** `radialStrainMag` is capped at `* 0.001` (`relief-base-step.js:38`), making `eps ≈ 5e-4·mag` in stress units (the coupling at `relief-e6-tectonic.js:21` is `eps = sign · mag · (3 + NU) · 0.5 ≈ 1.625·mag` with `NU = 0.25`, then damped by the `* 0.001`) against a per-band despin stress span of the same `(3 + NU)` order Layer 1 re-bases onto. That is ~1000× too small to cross an Anderson regime boundary (`relief-e6-tectonic.js:25-27`), so the contraction-vs-expansion regime **never flips** — `steeredNoise` (`relief-e6-tectonic.js:61-67`) stays a ridge/groove micro-texture inversion on identical blobs.
...
> **Tuning discipline.** Every constant named below (`REGIME_GAIN`, the host-field-divergence threshold, terrestrial-bundle numbers) is **TO-BE-TUNED-IN-LAB-then-locked** — set in the harness so the divergence metrics clearly separate the bundles, then frozen. None of the values here is presented as calibrated. **⚠ Tuning-circularity risk:** tuning `REGIME_GAIN` and the divergence threshold against the *same* fixed bundles they must discriminate fits the gate to its own test set. Mitigation: before freezing any value, validate it against (a) an **identical-bundle null run** (must score ~0) and (b) a **reseed-only run** (same bundle, different seed — this sets the floor the threshold must clear), and ideally a **held-out bundle**.
...
- **Faithful derivation:** the SIGN is already correctly derived and varies per body — `expansionDrive = clamp01(log10(1 + tidalHeat) / 2)`, `contractionDrive = clamp01(0.4 + 0.6*age) * clamp01(surfaceGravity / 1.5)`, `radialStrainSign = contractionDrive >= expansionDrive ? +1 : -1` (`relief-base-step.js:35-37`). Keep that. Only the magnitude→eps mapping changes. Express `eps` as a fraction of the despin stress span so it is regime-relevant *by construction*:
  ...
  `eps` bounded to `< span` keeps the Melosh latitude bands as the substrate — strain biases *which* bands are scarps vs grabens rather than collapsing all bands into one regime. This is exactly P2's documented intent: the same drivers under different sign select different feature sets (`docs/FEATURES/planet-visual-features.md:143` P2 drivers `D16 cooling→contraction` / `D2 ice-shell extension`; `:220` F5 lobate-scarp/wrinkle-ridge (compression) vs horst-and-graben (extension); `:219` F4 tectonic graben/chasma (extension)). **No renderer change is needed** because the divergence is produced entirely in the generation layer: `steeredNoise` — a generation-layer function that writes `substrate.height` via `runE6` (`relief-e6-tectonic.js:91`) — already swaps ridges vs grabens on `regime === NORMAL` (`relief-e6-tectonic.js:66`), so a regime flip auto-swaps morphology in the field the preset-blind renderer merely expresses.
...
- **Gate metric:** the **secondary** divergence (at a *different* seed, minus the held-seed baseline — §5 metric 4) rises → composition-keyed DEM layouts differ between bundles. ⚠ **A reseed moves the per-cell metric trivially**, so L3's contribution is the SECONDARY metric, **not** the decisive held-seed gate; L3 must not be what carries the pass. → **Build requirement:** make the discriminator **toggleable** (a flag) so the verifier can measure held-seed baseline (discriminator OFF) vs reseed lift (discriminator ON).
- **Passing proves:** composition-keyed layout (a rocky and an icy world don't share blob/lineament placement at one seed). **What escapes the coat-swap is L1's regime + L2's geometry carrying the held-seed gate — not L3's reseed** (a reseed alone is "a different random map of the *same* world").

> **EARLY-EXIT RULE (cheap-signal stop).** If, **with the seed held constant**, Layers 1–2 cannot move the **held-seed hypsometric divergence** (§5 metric 4, load-bearing component) above the reseed floor for the cross-regime pairs **AND** Layer 1 cannot produce a real regime-class shift (§5 metric 3), **STOP** — even if a Layer-3 reseed *would* clear the raw threshold (a reseed-only pass is a reshuffle, not re-physics, and does not count). That is the cheap signal that the relief + fluvial vocabulary is too thin to carry categorical divergence, and a genuinely **new process** (breadth — e.g. impact cratering E8a, aeolian E10, cryosphere E11) is needed before more field-shaping. Do not death-spiral on re-keying.
...
- **Faithful derivation — copy the canonical production formula verbatim; do NOT invent physics.** Production already has the canonical gate: `liquidStability = clamp01(retentionGate * volatileGate * tempWindow)` (`planet-lod-lab-core.js:561`), the AND of three soft gates. Mirror it:
  ...
  - **Wire into E9** (one conceptual lever — *whether/how-strongly E9 carves* — but **three edit sites**; the Layer-4 TDD must cover all three so the carve gate is not satisfiable by wiring only one): in `runE9`, early-return the incision loop when `liquidStability <= 0`, scale `erodibility *= liquidStability` (replacing the hardcoded path at `relief-e9-hydrology.js:108`), and derive `targetFrac` from `liquidStability` instead of the hardcoded `0.4` (`relief-e9-hydrology.js:115`). This is **analogous in ROLE** to the `rockyCrust` silicate gate at `relief-base-step.js:28` — both gate a process on composition — though a **different lever class** (`rockyCrust` continuously scales E6 *amplitude*; this gates E9 *on/off + strength*). It realizes Appendix A's intent (`docs/FEATURES/planet-visual-features.md:400` F-gradational row marks **lava `–`** / not-applicable; `:233` F11 rivers gated to terrestrial/ocean/ice/carbon).
...
4. **Host-field hypsometric divergence — ⚠ SUPERSEDED to "reported corroboration" by the Build-revision note (2026-06-23).** *The build found this metric seed-fragile across regimes; it is NO LONGER the decisive gate. The decisive gate is `regime OR hydrology OR carve` (see the Build-revision note + §9). The prose below is retained as historical rationale.* It is still measured and reported (with the master seed AND the Layer-3 discriminator HELD CONSTANT) as a corroborating signal. With the E9 carve overlay **DISABLED**, compute a distribution distance between two bundles' E6-only DEMs (`substrate.height`, `relief-substrate.js:9`). Two sub-metrics, kept SEPARATE because they detect different things:
   - **Load-bearing — Wasserstein/KS distance on the hypsometric height histogram.** *Reseed-invariant*: it moves only when the height **distribution** changes — a real regime/geometry change (scarp-skewed vs graben-symmetric, different lineament spectrum). Under a pure reseed it stays at the finite-sample floor. **This is the pass/fail component.**
   - **Diagnostic only — per-cell RMS height difference normalized by relief range.** *Reseed-sensitive*: a pure reseed decorrelates the noise stream and saturates this term **with no physical change**. Report it for insight, but it **must NOT certify the gate** — alone it proves only "a different arrangement", not a different world.

   **Why held-seed (the coat-swap-of-a-different-color guard).** Layer 3 re-keys the noise/crust seeds; a reseed produces fully decorrelated noise, which saturates the per-cell RMS and nudges even the hypsometric distance off the *identical-bundle* floor by a finite-sample amount — i.e. "a different random map of the *same* world", NOT the categorical divergence this design targets. Holding the seed constant strips the reshuffle out so the gate certifies **physics-routed field-shaping**. Layer 3's reseed contribution is real and worth keeping (composition-keyed layout — a rocky and an icy world shouldn't share blob placement), but it is reported as a **SECONDARY metric** (divergence at a *different* seed, minus the held-seed baseline), never as the decisive gate. → **Build requirement:** the Layer-3 discriminator must be **toggleable** (a flag) so the verifier can measure the held-seed baseline (discriminator OFF) separately from the secondary reseed lift (discriminator ON).

   **Pass/fail line.** PASS = held-seed **hypsometric divergence above the to-be-tuned threshold**. **Corroboration (not a universal AND-gate):** for any bundle pair that *should* differ in tectonic regime — e.g. a contraction (silicate, sign +1) vs an extension (icy/molten, sign −1) bundle — the **regime-class histogram (metric 3) must also show a real THRUST-dominant-vs-NORMAL-dominant shift**; if it doesn't, the hypsometric move is suspect and that pair FAILS. For **same-regime pairs** (e.g. europa vs lava, both extension — see §3 Layer-1 caveat), the host-field will legitimately be near-identical; their categorical distinctiveness rests on the **carve axis (metrics 1–2, Layer 4)** plus composition-keyed layout, **NOT** on host-field divergence — do not force metric 4 to separate them. The threshold is **TO-BE-TUNED-IN-LAB-then-locked**, set just above the **reseed floor** (two different seeds, SAME bundle) — **not** the identical-bundle floor — so physics must clear what a mere reseed already produces. **Validate every locked value against an identical-bundle null run (~0) and a reseed-only run (the floor) before freezing** (guards the tuning-circularity risk, §3).

**EARLY-EXIT RULE (restated as the verifier's stop condition):** if, **with seeds held constant**, Layers 1–2 cannot clear the held-seed hypsometric threshold for cross-regime pairs AND metric 3 shows no real regime shift, STOP field-shaping and escalate to a new process (breadth) — the cheap signal that relief+fluvial alone is too thin. **A pass that appears only when the Layer-3 reseed is active is a reshuffle, not categorical divergence — it does NOT satisfy the gate.**
...
- **`uvStripFactor` dropped in the retention reconstruction.** Production's `computeAtmosphere` includes a UV-sputter term needing `uvFlux = luminosityRel / orbitAU²` (`PhysicsEngine.js:170`; the `uvStripFactor` expression that consumes it is at `PhysicsEngine.js:180`); the relief bundle carries no `luminosityRel` / `orbitAU`. The Layer-4 reconstruction **drops `uvStripFactor`** — a **documented simplification**, not a silent one. It may mis-gate borderline bodies and is not bit-faithful to production; the bundles' clearly-temperate/clearly-airless presets are well inside the gate's margins, so this is acceptable for the lab. Flag if a borderline bundle is ever added. **Consequence (final-review finding, 2026-06-23):** with `uvStripFactor` dropped AND given the 5 shipped presets' masses/temps, the Jeans **retention term always evaluates `retained=true`** (λ_N2 ≈ 46–209 ≫ 6, even europa) — so for the current preset set `liquidStability` is carried by `tempWindow × volatileGate`, not the retention term. The retention gate is **nonetheless verified LOAD-BEARING, not dead code**: a constructed temperate, volatile-rich, too-low-mass bundle (`massEarth 0.005, radiusEarth 3.0, T_eq 290`) yields λ_N2 ≈ 0.35 → `retained=false` → `liquidStability = 0`, while an Earth-mass control with identical T/volatiles stays at 1.0 (test: `Layer 4 — retention gate is load-bearing`). So D6 will correctly screen a future low-mass/hot body that temp/volatile alone would wrongly pass.
...
- **Isolated lab, renderer untouched, additive on `master`.** Zero production edits. No render hacks, no tacked-on visual features — the renderer is preset-blind by construction and MUST stay that way. Production L1 wiring is a separate high-blast-radius branch (INDEX §1 NEXT). **`dev-collab-scope`/`contract.json` is deliberately NOT used here:** this build imports zero production modules (it copies formulas and cites the source in comments — the same pattern already used for `tidalHeat`), touches only `relief-*` + the harness + the test file, and is additive on `master` — low blast radius, and THIS spec is the design contract. (The high-blast-radius production-L1 port is the thing that will need `dev-collab-scope`, on its own branch.)
...
- **Objective (the pass/fail line — per the Build-revision note):** a body pair passes iff it diverges on **≥1 robust, reseed-invariant, physics-carried axis: regime (`regimeHistogramDistance` > 0.2) OR hydrology (`|liquidStability|` > 0.3) OR carve (`|carveFraction|` > 0.05)**, computed by `relief-slice.divergenceReport`. Proven categorically different, not the coat-swap (amplitude — killed by z-scoring) and not a Layer-3 reshuffle (all three axes are reseed-invariant; NULL identical-bundle scores 0/0/0 → fail, structurally). Directional anisotropy (L2) + held-seed hypsometric are reported corroboration. Locked measurements: terrestrial-vs-europa regime 0.81; europa-vs-lava hydrology 1.0 (regime 0.18, carried by hydrology); terrestrial-vs-lava regime+hydrology+carve; NULL 0/0/0.
