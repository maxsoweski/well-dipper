...
- **No `Date.now` / `Math.random` in derivation code.** The grain path must use only `stressAtLat`/`writeGrainSphere`/`seededUnitVec3`/sin-hash. The GUI `randUnitVec3` (`world-engine-lab.html:2390`, `Math.random`) must NEVER be the grain source.
...
The carve already lowers `h` (`world-engine-lab.html:424` — dossier corrects the "cosmetic" framing). The real gaps:
