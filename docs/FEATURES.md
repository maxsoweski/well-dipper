...
| Rivers — the lab's router WIRED INTO THE GAME (F11/F12 + F13/F14/F20 via driver pack #9 `fluvialDeck.js`; relief + crater cubes on every procedural solid body, carve cube on routed ones, ribbon + solved sea on wet ones, one worker message per body; keys `J` rivers / `U` relief) | F&F-MVP | **shipped-confirmed** (2026-09-02) `5c952e3` — verdict unit + integration PASS (`1ee0219`); live pair measured on rocky-14's moon (relief, 124,960 px) and rocky-4's outer planet (rivers, 6,141 px; ribbon visible; sea at the solved level). Max UAT on rocky-4's outer planet: *"The rivers are not fully developed but the wiring appears to be working here. The wiring for U is also working"* — the wire is accepted; **"not fully developed" is the open follow-up, his specifics pending** | — | [WORKSTREAMS/wire-river-router-lab-into-game](WORKSTREAMS/wire-river-router-lab-into-game/) |
...
| **Star↔planet barycentre (the star's own wobble)** — the one real-world barycentre WD does not model. Planet↔moons is already universal (`main.js:11267`, no gate) and binary stars orbit a shared empty point (`:7543`, `:11244`), but NOTHING ever moves a star because of its planets; a single star is pinned at the origin permanently. ⭐ Max 2026-08-19: **consider building BEFORE the rotor-fuel minigame** — if you harvest energy by dipping into a well, where the well's centre actually IS stops being cosmetic, and a wobbling star makes the well move. | GAME | proposed | — (physics is nearly free: `barycentreOffset` already generalises to "a body plus things orbiting it") | `FEATURES/moon-formation-handoff-2026-08-18-b5.md` §12 |
