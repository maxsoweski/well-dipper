  ...
  arc: {
    ...
    darkClipGuard: {
      ...
      toleranceFrac: {
        ...
        status: 'GUESSED',
        rationale:
          'A per-capture growth bound: post-flip darkClipFrac must not EXCEED the re-baselined darkClipFrac1 by more than this fraction of disc pixels newly clipping to black (which would erase arc asymmetry). bake-attenuation-model.json.shadowRebaselineProtocol step 4 explicitly defers this number to read-gate-thresholds.json (S0.6); it cannot be derived from S0 alone because darkClipFrac1 is not measured until the S1 flip.',
        resolutionPath:
          'At the S1 re-baseline seam (before any S2/S3/S4 capture), measure darkClipFrac1 and its per-reroll sampling spread σ across seed 1 + 2 re-rolls at the staged light; set toleranceFrac = max(k·σ (k=2), dither-noise floor). Replace this GUESS with that derived value, then FREEZE and hold (do not re-baseline per capture — that would launder a regression).',
      },
