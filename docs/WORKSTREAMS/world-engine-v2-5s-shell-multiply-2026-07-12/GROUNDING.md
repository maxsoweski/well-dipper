...
Diag returned (`:369-373`): `U, regime, cellId, cellCount, stressTensile, thetaTraj, lineamentNode, chaosMask, reliefStress, w0, t_hat, phi0, meanEdgeAngle, relaxPasses` — read live by `shellProbe()` (world-engine-lab.html:6096) and by the headless structure suite (tests/worldengine-base-shell-structure.test.js).
...
Idiom (grep-verified, world-engine-lab.html):
