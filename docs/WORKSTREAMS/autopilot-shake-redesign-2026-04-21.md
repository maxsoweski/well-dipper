...
- **Orbit-phase wobble-squashing code.** Max's "weird wobbly stuff when we get to the planet" is explicitly NOT scoped as an AC in this round. Working hypothesis: the orbit wobble is a symptom of the mistimed decel impulse (AC #8) — a decel event that fires at `travelComplete` leaves residual impulse-train tail extending into the approach + orbit windows, which would read as "wobbling in orbit." Working-Claude runs a telemetry capture (AC #9) at Sol over a full tour after the AC #8 fix lands and analyzes the `shakeMag` time-series during the sustained-orbit window. If `shakeOffset` samples during orbit are `[0,0,0]` (AC #9's verification point c), the orbit wobble was indeed a symptom of #8 and is now gone. If `shakeMag` is non-zero during sustained orbit, that is a **round-4 follow-up bug** — a new workstream or amendment, not a patch slipstreamed into this round. Do NOT preemptively scope orbit-wobble-squashing code into this round.
