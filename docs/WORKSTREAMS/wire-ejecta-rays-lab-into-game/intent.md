...
(`scoping-corpus-2026-09-03.json`; 21 of the 24 seeds produce bodies — rocky-5, -8 and -18 generate
no planets — 156 bodies): **56 of the 124 solid bodies — every plain moon —
...
uniform with a live input, two constants. **The VALUE is > 0 on 56 of 124 solid bodies; rays can
actually RENDER on 52 (41.9 %) and at a crater density above 0.01 on 39 (31.5 %)** — rays multiply
the crater host, and four airless moons have no crater schedule at all (`CRATERS_OFF`: rocky-13 p3m0
and p4m0, rocky-14 p3m0, rocky-19 p0m0). The moons are where the real Solar System paints its rays
(Tycho and Copernicus on the Moon, Kuiper on Mercury, the bright-ray craters of Callisto and Rhea).
**One thing the population is NOT: per-body.** 19 distinct ray values over the 56, and every plain
moon of one system reads the identical value — the generator's airless erosion is the SYSTEM age
alone (`PhysicsEngine.js:823-825` fed the system age at `MoonGenerator.js:300`), and its 0.3 cap
bounds the whole population to [0.70, 1.0] by construction. That is a world-generation row, logged
below; the wire carries the law as it is.
...
| `uRayCount` = 6, `uRaySharp` = 8 | lab constants (`world-engine-lab.html:1175-1176`; the only two F3 knobs with NO producer in `deriveUniforms` — measured absent on 18/18 driver presets) — already the shared bag's defaults (`uniforms.js:178-179`) | how many spokes each crater throws and how crisp they are |
...
**Where they will render** (airless AND cratered — `craterDensity × craterRelevance > 0.5`, by size):
`rocky-13` p4m3 ice 1.06 R⊕ (density 1.0, Dchar 209 km), p4m5 ice 0.96 R⊕ (1.0), p3m2 ice 0.62 R⊕
(1.0), p4m4 ice 0.54 R⊕; `rocky-10` p4m1 ice 0.48 R⊕ (1.0). ⚠ The two BIGGEST airless moons in the
corpus cannot show a ray: rocky-13 p3m0 (volcanic 1.27 R⊕) has no crater schedule and p3m1 (rocky
1.14 R⊕) has density 0.0002 — every rocky/volcanic moon of that system is bare, every ice moon is
saturated. `rocky-2`'s two moons are 0.027 and 0.019 R⊕ — too small for the walk.
...
- On an airless, cratered moon in a procedural system the *"bright rays read as discrete radial
  spokes from young craters"* (card §6 — which judges them *"at full-disk distance (distance ≈ 20)"*;
  this wire is gated at 3 radii and RECORDS one pair at ≈ 20), *"present only on airless"* bodies and
  *"absent under any atmosphere or high erosion"*, brightening *"only the SUNLIT hemisphere"* — on
  every planet with air, nothing.
- Each body from its own record (its air, its erosion), the same on every return — with the
  measured caveat that the generator gives every moon of one system the same erosion, so
  differentiation is BETWEEN systems (19 values over 56 moons), not within one.
...
1. **The law's home is a new shared module** `src/worldengine/base/ejectaRays.js` exporting
   ...
   (the line is cited by symbol). The crater driver block (`craterDeck.js:116-197`, shared by
   `craterDeckPack` for gas and `rockySurfacePack` for everything else) reads the CONDITION directly
   — `rayBrightnessOf(condition)`, the same shape as its `rel = craterRelevanceOf(condition)` at :109
   — and emits the three names beside `uEjectaStrength`. ⛔ NOT through `craterUniformsFrom`: its
   frozen `CRATERS_OFF` return has no such key (54 corpus bodies would throw at the write), and the
   ray law is a pure condition function that must not inherit the crater schedule's fired branch.
   `uRayBrightness` joins neither LAB_BINDING (the apron's precedent) — `state.rayBrightness` keeps
   its single writer at :2042. No class guard: on gas the law itself returns 0.
...
6. **The live subjects are `rocky-13`'s big CRATERED airless moons — the FIFTH planet's ice moons
   p4m3 (1.06 R⊕, density 1.0) and p4m5 (0.96 R⊕, 1.0)**, each pre-flighted headlessly
   (`craterRelevanceOf === 1`, `density ≥ 0.5`, `rayBrightnessOf > 0`) before a pair is shot; a
   subject failing the pre-flight is REPLACED, not attributed. The air control is any planet of the
   same system; the gas control its giant if it has one. One extra pair is recorded at ≈ 20 radii.
7. **The PLANs' stale CLAIMS are annotated in place, line-neutral:** the queue-(c) sentence at :574
   and the queue list at :133 of `one-pipeline-two-frontends-PLAN.md`, and :398 / :403 of
   `lab-pipeline-into-game-PLAN.md` (the plan of record), all absorbing the 2026-08-20 correction.
   ⛔ The `conditionFromPlanet.js` SPELLING is left alone — 19 PLAN lines carry it, three are the
   rename's own record, and `tools/port-uniform-delta.mjs` pins it as a citation spelling.
...
- **Two world-generation rows:** (1) airless erosion is the system age alone, so every plain moon of
  a system carries identical ray brightness (19 values over 56 bodies) and the 0.3 cap floors the
  population at 0.70; (2) a terrestrial plain moon's atmosphere record is `{ color, strength }` with
  no physics (`MoonGenerator.js:217`) — airless to the adapter, air-bearing to the erosion law. Zero
  such moons in this corpus. Both logged for the generator, not touched.
...
- **`craterUniformsFrom`, `CRATERS_OFF`, the crater schedule, the apron law — untouched.**

## Risks named up front

- **Rays follow craters.** The ray term is multiplied by the crater host; a body the crater
  schedule leaves bare shows no rays however bright the gate (42 of 124 solid bodies, 4 of the 56
  airless). Hence the pre-flight in decision 6: with it, a null on a live subject IS the wire.
