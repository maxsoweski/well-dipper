...
| `S.zoomGaugeRect` | LAB (design 2 SYSTEM, INSIDE the map pane at its right edge — a press must pass the host's `onMap` test at :4401 to reach the grab) | `null` (default in state.js, cleared by `resetPicks`); `{ x, y, w, h }` = the VERTICAL track's rect in texels; the mark sits at t = (ln zoom − ln 0.3) / (ln 5 − ln 0.3), t = 0 at the BOTTOM (y+h), t = 1 at the TOP (y) | DRIVER (AC-6): `zoomGrab(x,y)` → true inside the rect, design 2, level 4; `zoomDragTo(py)` → exp(ln 0.3 + t·(ln 5 − ln 0.3)) with t = clamp((y+h−py)/h, 0, 1), or `null` with no rect. HOST folds: `_zoomDrag=false` default beside `_gaugeDrag` (:156); :4405 `this._zoomDrag = !!(this.viewMode==='bars' && drv.zoomGrab?.(p.x,p.y))`; :4355 `if (this._zoomDrag) { const z = drv.zoomDragTo?.(p.y); if (z != null) this._systemZoom = z; return; }`; :4415 `this._zoomDrag = false`. (Fixed 2026-09-20 after wave 1b; the wave 2a prompt carries the same text.) |
...
| `S.hoverCalloutRect` (wave 1b) | LAB, `hoverCallout` at its draw site; `null` when no callout drew (cleared at the head of both `drawDesign*`) | `{ x, y, w, h }` or `null`; default `null` in state.js (DRIVER, wave 2a) | nothing yet; the callout is never under the pointer that summoned it, so no press lands on it. Rule (after the wave 1b finding): a callout draws ONLY when the hover point (`S.hover.sx, sy`) lies inside the map pane — a rail-row hover paints none. |
| belt rows' `isKuiper` | DRIVER, `buildBodies` (state.js ~:645) copies `!!b.isKuiper` from the generator's belt (wave 2a) | boolean | LAB `beltLabel` (wave 1b) reads it when present, else falls back to geometry (the belt beyond every planet is the Kuiper belt). |
