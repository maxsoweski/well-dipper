...
(world-engine-lab.html:1933-2760) into a shared module wholesale. The adversarial agent was told to
...
| `radius-live-feed-fence.test.js` pins literal lab bytes in-range | :676, :681 | **confirmed** — those byte-strings are at world-engine-lab.html:2283 and :2406 |

## The five gate-breakers

1. **Citation fence.** `world-engine-lab.html` is a `CITE_SOURCE`. Deleting 828 lines from 1933 orphans
...
- **`preset: null` zeros all 13 feature-relevance gates.** `world-engine-lab.html:1986-1989` does
...
- **`drawPresetRotation` is lab-local** (`world-engine-lab.html:878`, keyed on preset NAME). Skipping it
