...
> Max's in-thread scoping calls this session (full-driver-set; D16 age surfaced first). The "Why we
...
volatiles, tidal heating, age) into the plate writer so the *same* archetype becomes a **continuum** —
heavy worlds sit flatter, volatile-rich worlds drown more continent, tidally-heated worlds churn more
plates, old worlds read their age. It is a second-order multiplier on top of the proven writer (lower
...
- **Crank a driver and the world changes the way the physics says it should** — heavier gravity →
  ...
  plates; older → reads its age — each in one consistent, documented direction, not a random wobble.
...
- **Age actually feeds the world** — D16 is surfaced into the terrestrial presets (a real value, not
  silently defaulted) and reaches the driver→tune mapping.
