...
|---|---|---|---|---|
| 1 (D1) | `RIM_HEIGHT_FRAC ≈ 0.04` (§4) | `RIM_HEIGHT_FRAC = 0.20` | A rim at 4% of the bowl depth is visually inert; real fresh craters have rim height ≈ 0.04·D over depth ≈ 0.2·D ⇒ rim/depth ≈ 0.2, and A is the (normalized-height) bowl depth. calibration-pinned profile-shape constant (§9 adjudicable). | AC-UAT legibility ↑; AC-CHANNEL two-signed unaffected (rim still > 0). |
| 1 (D2) | unbounded `+=` superposition (§2a) | soft tanh saturation `CRATER_SAT_N = 0.5` applied to the accumulated field | M-m4 legibility check: raw `+=` churns to −1.55 (7.5× a basin depth) at the low-g/high-age corner (inside the AC-LAB demo window). tanh caps deepest overlaps to ±0.5 (empirical crater saturation), preserving sign + node ordering (the #6 editor still thresholds floors). | AC-MULTIPLY/POWERLAW/DISTINCT unaffected (sign + ordering preserved); AC-UAT legibility ↑. |
| 1 (D3) | AC-POWERLAW fits the new preset's stamped population (~2887 @ native, §2b) | fits the writer's `drawPowerLaw` SFD sampler at `N_FIT = 16000`/seed | The differential dN/dlogD LSQ slope is Poisson-noise-biased shallow at ~2887 (sparse large-D bins clip at count≥1) → some seeds drift just outside −1.8. §2b explicitly sanctions "raise N_CRATERS_REF for the TEST POPULATION". The SFD is a sampler property (slope = what AC-POWERLAW asserts); the stamp count is a separate MULTIPLY concern (AC-MULTIPLY). At N_FIT the slope converges to −B_SFD every seed (−2.04…−1.97, R²>0.997). | AC-POWERLAW closed robustly; stamp count unchanged (N_CRATERS_REF 1800). |
| 1 (D4) | wall ramps floor → 0 at the rim crest, then rim = +RIM_HEIGHT_FRAC·A (§2a zones) | inner wall ramps −A → +RIM_HEIGHT_FRAC·A (the 0-crossing sits inside the wall; crest at the crater edge) | The plan's literal zones leave a value discontinuity at the crest (wall→0, rim jumps to +peak). Ramping the wall straight to the crest is continuous AND makes every single crater individually two-signed (bowl + raised rim) — the cratered-silhouette requirement. | AC-CHANNEL two-signed robust even for an isolated crater. |
| 1 (D5) | AC-MULTIPLY "measure craterField count/size directly" (§2c) | `writeBombardment` returns `carrier` (idiom parity); AC-MULTIPLY reads the exported `craterSchedule(condition)` `{nCraters, sizeMul}` directly | "Directly from the summed field" is ambiguous (count is not recoverable from a saturated field); the writer's scheduled `{nCraters, sizeMul}` IS the direct-writer-metric (route-independent), and the tanh saturation (D2) would muddy a field-derived count. | AC-MULTIPLY closed via the writer's own schedule (non-strict monotone). |
| 2 (D6) | `compositeMargins` stays module-internal (§4 does not name an export) | `export function compositeMargins` (byte-inert export added) + slice-2 tests live in a NEW file `tests/worldengine-v2-5-preset-composite.test.js` | The required §7-SLICE2 "composite value-identity on non-target worlds" test needs a direct handle on the pure helper (it is module-local; route() is its only runtime caller). Export is purely additive — no export-surface guard exists (checked), route() calls it identically, zero behavior/byte change. New test file keeps the slice-2 integration ACs off the slice-1 data-AC file. | None: AC-PRESET + composite value-identity closed via the exported helper; AC-ZERO-CLOBBER unaffected (full-suite failure signature byte-identical to the slice-1 baseline). |
