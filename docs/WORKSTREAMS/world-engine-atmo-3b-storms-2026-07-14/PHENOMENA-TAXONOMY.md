...
> **Status:** ✅ RATIFIED 2026-07-14 (designDecision 4 build-sequencing gate CLEARED — slices P/V may build against this doc). All recommendations ratified; two riders below carry to BUILD-PLAN.
> **Ratification record (Max, 2026-07-14, verbatim):** *"1. Good 2. Good; A is fine but also let's consider the wide variety of theorized possible planets 3. I don't know...I just want lots of variety to be possible here, driven first by physics/astronomy, but then enriched further 4. That's fine 5. Rec 6. Rec 7. Rec; good to commit"* — numbering maps to the presentation order given to Max: 1→Q1 · 2→Q2 · 3→Q3 · 4→Q4+Q7 (merged nod) · 5→Q5 · 6→Q6 · 7→Q8.
> **Rider A (Q2, working-Claude interpretation — flag at BUILD-PLAN):** sub-call (a) answered: deliberately bland Uranus is acceptable. Sub-call (b) (distinct Uranian preset vs Neptunian variant) left open, enriched with *"consider the wide variety of theorized possible planets."* BUILD-PLAN default: realize Uranian as a Neptunian-regime **variant** (variety-through-parameters over another frozen preset better serves the theorized-planet ask); broader theorized-giant coverage routes to the derive-not-freeze increment's agenda. Lenses may overturn toward a distinct preset with cause.
> **Rider B (Q3, working-Claude interpretation — flag at BUILD-PLAN):** no literal count chosen — *"lots of variety... driven first by physics/astronomy, but then enriched further."* The rec's mechanism stands (declared per-regime canonical N, GUI-tunable), with N a physics-grounded default plus a plausible tunable range, NOT a frozen literal; per-seed N variation belongs to derive-not-freeze.
