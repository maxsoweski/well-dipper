...
1. Build **E1**, a derived regime selector: per-body drivers (D1–D16) → geodynamic regime. Physics-derived at **three** regime-diagram *edges* (dead lid, heat-pipe, icy conductive-vs-convective); the **hot-surface stagnant edge is outcome-agreed but data-placed** (surface-temperature anchored), mechanism contested (Lenardic stress-reduction vs Noack/Breuer healing — different intermediate-case predictions; see §2.3); **seeded, literature-bounded** pick in the genuinely multistable middle (mobile/episodic/stagnant), per `condition-to-regime-research.md`. Archetype is **demoted** from load-bearing input to an **emergent LABEL** (the spine §4b principle — locked on paper, never built).
...
| **Exogenic overlays** (orthogonal, not absorbed) | P-IMPACT (bombardment → V2-5); sculpting (#7 → V2-8, with weathering E8b + hydrology E9 folding into V2-8 — no separate increments) | overprint any endogenic base |
| **P-ATMO** (parallel track) | climate #3/#3a/#3b, emission | atmosphere is its own process family |

**E1** is the selector over all of them. It emits `{compositionClass, geodynamicRegime, label, L, Φ, V, n, m_hp, e1Seed, positionWithinRegime}`. Writers branch on these coordinates, **never on a label**. The migration conformance oracle is **writer-equality** — `writerUnder(e1) === writerUnder(PRESET_ARCHETYPE)`, minus the enumerated allow-list (§5.2) — NOT label-equality: distinct archetypes legitimately share a writer (Lava/Magma/Io all → magmatism, while E1's regime reads "heat-pipe/weak-lid"), so label-equality would systematically fail volcanic presets. AC-CONFORMANCE(FINE) (§5.3) is a stricter tier atop this same oracle, not a third definition. The label is diagnostic output only, never an input.

**[RESOLVED-BY-SYNTH — D1-risk (exogenic omission)]:** The "one root + three lid-couplings" vocabulary describes only the **endogenic** layer. Exogenic overlays (bombardment, sculpting, weathering, hydrology) and the P-ATMO track are **orthogonal overlays that are NOT absorbed into the lid processes** — they remain first-class program goals (§3). The epoch/host-editor (#6 → V2-7) is deliberately NOT in the overlay list: it is an editor over the pilot's persistent channels (`plumeField/provinceMap/datumStack/coronaState`) with hard dependencies on them — a third category, neither endogenic base nor orthogonal overprint. "All ROADMAP goals under the new architecture" explicitly includes them; the architecture is not silently narrowed to endogenic worlds.

**[RESOLVED-BY-SYNTH — D3-MF1 (label carve-out)]:** Authored-label archetypes with **no distinguishing driver signature** — crystal/geometric, technogenic — are **permanently outside condition-derivation**. E1 excludes them by an **explicit label carve-out**, not by hoping a scalar separates them. Carbon is derivable only if the C/O>1 branch beats the density→rocky branch (flagged, §6 R-exotic).
...
- The *shipped* magma driver is **`magmaThermal H` = clamp01(0.5·clamp01(rawTidal)+0.5·(1−age/10))** (magmatism.js:97-103), whose tidal term is **raw and saturating**, NOT the tanh-compressed `thermalState`. The tidal terms coincide only where tidal≈0, and the age terms use different normalizations (`age/10` with inner clamp01 vs adaptL0 `ageNorm`) — the two quantities are not interchangeable at any clean reference point. Reusing `thermalState` as Φ would silently regress or double-drive the validated #4-MULTIPLY plume-count/height response.
- `thermalState` is **production-dormant** on the sphere path (baseStep's only non-test caller is the `worldengine-fieldviz.html` viz harness; sphere-path relief callers import `relief-base-step.js`, a different file), so "zero-clobber via verbatim reuse" was moot anyway.
...
|---|---|---|---|---|
| **V2-0** | **L0 plumbing + baseStep scalar extraction.** Finish surfacing D12/ecc/D13/D16/metallicity (data-only). **[RESOLVED-BY-SYNTH — D3-risk]:** "wire dormant `baseStep` into the sphere path / reuse `thermalState`,`shellThickness`,`rawTidal` verbatim" is **not achievable** — `baseStep` exports only `makeBaseStep`, a **grid** op; those quantities are internal locals. Instead: **refactor baseStep to export pure per-body scalar helpers**, guarded by its existing test suite so extraction is drift-safe (⚠ `worldengine-fieldviz.html` also imports baseStep — a non-test caller OUTSIDE that guard; keep it working). This activates never-integrated code, not a live path. **Two plumbing deliverables added (BU-M1/M2, both prerequisite to a buildable V2-1):** (i) **extract `DRIVER_PRESETS`** from `planet-lod-lab.html` (~L2641) into an importable presets module — today tests string-scrape the HTML and V2-1's conformance oracle has no headless preset source; (ii) **thread the full body condition-vector** through `route()`→`writeBodyRelief` (composition/density, age, radius, eccentricity/raw-tidal Io-ratio, shellThickness — all exist in `_fp` but stop at the lab today). Byte-safe: `driversToTune`/`magmaDriversToTune` default `?? D_EARTH` and ignore unknown fields. | M | — | E1 gets real D-slots + reusable scalar derivations + a headless oracle source |
| **V2-1** | **E1 regime selector (SHADOW mode).** Emit `{compositionClass, geodynamicRegime, label, L, Φ, V, n, m_hp, e1Seed, positionWithinRegime}` onto `state` + a live probe. Dispatch still reads `PRESET_ARCHETYPE`. **Oracle = `writerUnder(e1) === writerUnder(PRESET_ARCHETYPE)`** over the **15 archetype-mapped presets** (`DRIVER_PRESETS` holds 17 entries — Mars and Hot Jupiter carry driver data but no archetype mapping; they join the oracle at V2-3), **minus an enumerated allow-list of intentional reroutes** (V2-3). | M–L | V2-0 | The whole condition-first architecture, zero-clobber; executes spine §4b on paper |
...
| **V2-3** | **Flip dispatch to E1's derived `{compositionClass, geodynamicRegime}`** — never `E1.label`; the label stays diagnostic-only (§1 invariant) — once oracle green **and every enumerated misroute adjudicated** (below); retire `PRESET_ARCHETYPE` as dispatch key. Add **Mars archetype MAPPING + routing** (driver data already in `DRIVER_PRESETS`); resolve **sub-Neptune/Neptunian** short-key collision (⚠ radius hazard: demoting Neptunian→gas-giant flips seeded radius [2.5,4.0]→[6.0,14.0] = Jupiter-sized Neptunians; add a `ScaleConstants.js` ice-giant range or keep the key); fix **Frozen(airless)→dead-lid** misroute. `PRESET_ARCHETYPE` survives as fallback oracle until the retirement trigger — **post-V2-3 verify-workstream green + Max UAT** — and is **deleted only after each misroute is explicitly resolved**. | S–M | V2-1, V2-2 | Type-as-derived-label live |
| **V2-4** | **Shared-substrate pass (was #5.5) — ALL FIVE fields.** (d) SP-STRESS-FABRIC; (b) `carrier.sediment`/`carrier.accommodation` host (new channels, not the E9-owned `maturity`/`baseLevel`); **(a) passive continental margins; (c) history-tied E12-province (random-seed association-test must FAIL); (e) E2-figure (oblate/triaxial/Roche/despun-fossil-bulge — must ORIGINATE `w0` from drivers, not accept it authored).** Fine print restored from v1: accommodation is **SINK-RANKING only** — no CYCLE-1 mass-conservation claim (the volumetric budget belongs to #6/V2-7). | M–L | V2-2 | De-risks #6/#7/#8; **provides the E2-figure that V2-7 CYCLE-2 depends on** |
...
| **V2-6** | **Atmosphere track (P-ATMO) — POINTER ROW.** `ATMOSPHERE-PLAN.md` (same dir) continues **unchanged as a first-class parallel sub-plan**: 9 increments, 2 shipped (#3a bands/jets, emission v1), **7 remaining** (#3b vortex/storm; emission v2 aurora/lightning/airglow + magnetic-dipole driver; brown-dwarf/directly-imaged; lava rock-vapour; terrestrial climate substrate→E9; Mars thin-CO₂; Pluto/Triton sublimation). This row adds only the v2 touchpoints: wire `writeClimateE5Sphere` game-side; **derive-not-freeze MULTIPLY** (`shellDepthFrac`/`internalHeat`/`dissipation`, R-atmo). *Parallelizable with V2-2..V2-5.* | **XL** (sub-plan total) | V2-1 (classification plumbing only), #3a (shipped) | All ATMOSPHERE-PLAN unlocks: giant/hot-Jupiter/Neptunian storms, brown-dwarf coverage, rock-vapour, Mars + Pluto/Triton atmospheres; precip feed for #7 |
| **V2-7d** | **SP-LID-DISRUPTION shared abstraction (BUILD).** Generalize basal-upwelling→quasi-circular lid deformation from its two structurally different seeds (`shellRelief` STEP-2 space-filling Voronoi convection tessellation; `stagnantLid` sparse rejection-sampled coronae with analytic profiles) into one owned module. **CUTTABLE:** cutting it strikes the cantaloupe-silicate unlock from V2-7 and the diapir-grooved-coronae option from #4.5 — the #4.5 geometry choice is then forced to block-jumble (the pick itself stays Max's, §7a). | M–L | V2-2 | V2-7 cantaloupe-silicate payoff; #4.5 diapir option |
| **V2-7** | **Epoch / host-editor model (was #6, LOCKED).** 2–4 **discrete painted epochs** (byte-identical at `epochs=1`); editor-on-host over `plumeField/provinceMap/datumStack/coronaState`. **Depends on the V2-4 E2-figure field (CYCLE-2 figure↔grain).** Fixed-point cross-tier solver stays the flagged open problem — and carries #6's v1 solver requirements explicitly: **surface relaxation is Jacobi, not Gauss-Seidel**, and the **volumetric mass budget** (material moved between provinces) is owned here, not by V2-4 (see V2-4 fine print). | L–XL | V2-2 (host), **V2-4 (figure)**, V2-5 (editor exemplar), **V2-7d (cantaloupe only)** | Frozen-then-pierced; cantaloupe-silicate payoff (requires V2-7d) |
...
| **V2-9** | **Exotics + technogenic + Tier-5 (was #8, split).** 9a: carbon/crystal as E1 Stage-A terminals + small writers + #4.5 block-jumble primitive; 9b: technogenic + rings/weathering/palette/magnetosphere **+ E13 transient events + E14 inhabitation** (all six v1 Tier-5 overlays restored — cutting any is a Max call, not a synthesis default) via epoch. 9b ACs: **palette DERIVED from province** (the anti-bag-of-overlays guard; needs V2-4's history-tied E12-province); rings/E4 carry the v1 `moons:[]` system-graph prerequisite (unbuilt). | XL (cuttable) | V2-1, **V2-4 (province)**, V2-7 | Closes toward 11-of-11 |
...
**Critical path:** V2-0 → V2-1 → V2-2. **Three pre-code gates block V2-2** (R-phantom): delegable #1 (`L` functional form — the response space *consumes* `L` but cannot *define* it), delegable #2 (`localYield(L,i)`), and the **interpenetration-statistic definition** (§5.4 #2 / §7 pre-code gates).
...
| **#4-MULTIPLY** | **DISSOLVED into the Φ axis.** **[RESOLVED-BY-SYNTH — D2-MF7]:** its elongation **MAGNITUDE rides Φ, NOT L** (Φ ≠ the shipped kernel's internal thermal `H` — §2.3; the anchor keeps `H` unchanged); its elongation **DIRECTION is a writer-private seeded fissure axis** (`alea('magma:grain:')`, derived — *not* read from `carrier.grainAngle`, because a volcanic body's grain is zero/latitude-binary). This axis **stays disjoint from SP-STRESS-FABRIC** or it clobbers the weak anchor and is physically wrong. |
...
| **#4.5 exotic-shattered** | **SPLIT.** Diapir-grooved-coronae branch → covered by the future SP-LID-DISRUPTION family; Miranda block-jumble → small V2-9a primitive. The blocked geometry decision shrinks (still a Max call). Do **not** write a 4th regime constant (`verify.js:39` asserts `sub.regime` ∈{0,1,2} — the **substrate** field, not `carrier.regime`). |
...
| brown-dwarf / directly-imaged | none (zero home in any prior map) | P-ATMO — ATMOSPHERE-PLAN #5 (self-luminous S≈0, L/T cloud-clearing) | Stage-A terminal | **V2-6 sub-plan #5** |
...
| `shellRelief.js` #1 | **P-LID-ICY sibling, whole.** Convection-cell partition → **seed for SP-LID-DISRUPTION (V2-7d — build, not reuse).** `REGIME_WEIGHTS` → proof-of-concept response-table *shape* only. Tidal-lineament half stays sibling-local. |
...
**New files:** `src/worldengine/base/e1Regime.js` (`computeE1(conditionVector)`) and `src/worldengine/base/lidResponse.js` (`writeLidResponseSphere(...)`, the router + mixed-interior composition; imports both corner writers as expression kernels). `writeLidResponseSphere` **emits a per-node `primitiveId`** (`Int32Array`; precedent: `magmatism.js:220` `plumeId`) — the instrument AC-MIX-DISCRETE and the interpenetration statistic are measured on.
...
**[RESOLVED-BY-SYNTH — D3-MF1 (subtractive gate)]:** During migration the gate is **SUBTRACTIVE**: `isUnbrokenLidPath(e1)` requires `{composition ∈ rocky/magma-ocean}` **AND** `{heat-pipe edge OR hot-surface-stagnant edge}` **ONLY**. `unbroken-lid-generic` rocky bodies (Mars, and anything else currently despun) **stay on the despun fallback** until a dedicated increment absorbs them — so the flip clobbers no shipped despun world. (Contract-author note: TWO despun destinations exist — `shellRelief`'s 'eyeball-despun' for **locked** bodies, `path:'shell'`, vs the final zonal fallback for **unlocked** bodies, `path:'despun'`; real Mars is unlocked → final despun. Do not conflate.) Authored-label exotics are excluded by the §1 label carve-out.
...
2. **Conformance oracle** — over 15 presets, `writerUnder(e1) === writerUnder(PRESET_ARCHETYPE)` **except the enumerated allow-list**. **[RESOLVED-BY-SYNTH — D2-MF8]:** the allow-list is a **hard-block adjudication list** of known-wrong routings that must be *fixed, not matched*: (i) **Frozen(airless)→'ice'** (a dead-lid airless body must route dead-lid, not to the icy-shell writer); (ii) ~~Neptunian/Sub-Neptune short-key collision~~ **[AMENDED 2026-07-11 — superseded by the V2-1 EMPIRICAL oracle]:** the shipped conformance oracle (`tests/worldengine-e1-conformance-oracle.test.js` — the authoritative allow-list) pins the divergence set as exactly **{Frozen(airless), Eyeball(locked temperate)}**: Eyeball resolved today-wins (eyeball-despun), locked-awareness added at V2-3. **Neptunian/Sub-Neptune is writer-EQUAL under both dispatch schemes and must NOT be on the divergence list** — its short-key collision + radius-range hazard stands as a real V2-3 *taxonomy* task (§3.1 V2-3 row), not a writer misroute. `PRESET_ARCHETYPE` is **not deleted until each is explicitly resolved** — "oracle green" ≠ "E1 correct."
3. **Flip** when oracle green + allow-list adjudicated. `isVolcanicPath`/`isStagnantLidPath` survive as fallback oracle until the retirement trigger — **post-V2-3 verify-workstream green + Max UAT** ("one release" has no referent in a lab) — then delete.
...
| **AC-MIX-DISCRETE** | **every node's height derives from exactly one primitive, keyed by the emitted `primitiveId`** — measured on `primitiveId`, NOT by height-array inspection (within-province BOUNDED-CONTINUOUS legitimately produces heights indistinguishable from forbidden blends); province boundaries sharp (anti-mush) |
...
| **R-disrupt** | SP-LID-DISRUPTION is **not** existing shared machinery (Voronoi cells vs sparse corona pool — structurally different). §5 cantaloupe-silicate payoff books work on machinery that must first be **built**; risks the metasystematicity collapse the task warns against. | Funded as owned **cuttable** increment V2-7d; cantaloupe demo design-forward until it lands | Technical (V2-7d) |
...
| **R6-solver** | #6 fixed-point cross-tier solver still **unmechanized**; epoch cross-tier cycles blocked. "Bounded relaxation passes" ≠ "fixed-point cycle resolution." v1 requirements carried explicitly: surface relaxation is **Jacobi, not Gauss-Seidel**; the **volumetric mass budget** is #6-owned (V2-7 row). | Do-not-contract flag stands (V2-7 open problem) | Max (defer or fund research) |
...
| **R-marspreset** | Mars falls-out is validated at V2-2 but the Mars **preset** only exists at V2-3; the **Tharsis-volcanism integration checkpoint** (§5.4 #3 — the "acid test" framing is retired; it is not a falsification) uses a **hand-set D-vector** until then. | §5.4 uses a hand-set vector explicitly | Technical |
...
|---|---|---|
| **1. `L` (lidStrength) form + Earth calibration** | `clamp01(f(ρgz)·μProxy(V, T_surf))`, **non-monotonic in T_surf**, anchored so `L(MAGMA_REF)<L(Venus)` — NOT the Earth mid-mobile point | **LOAD-BEARING, blocks the weak↔strong axis.** No `τ_y=180 MPa` constant; Earth is a calibration point, not a threshold. Must place both hot Venus and cold Mars high (R-L). Explicit modeling fork to resolve in this gate (SH-F5): route Mars to high-`L` via lid strength (forces the non-monotonic `L`) OR via **low Φ** — the brief reads Mars as LOW-vigor, which would leave `L` monotonic; pick one and record the choice |
...
| **4. Φ size-proxy form** | Radiogenic-scaled-by-mass + d³ (`shellThickness`) + raw-tidal; expose raw Io-ratio in the drivers bundle | Must separate small-cold Mars from Earth-sized Venus (R-Φsize). ⚠ **Coherence risk (SH-F2):** `shellThickness` currently serves triple duty — lithosphere thickness *z* (in `L`), icy shell thickness *D* (shell-Ra), rocky mantle depth *d* (Φ's d³) — three physically distinct thicknesses (~30× apart on Earth; the brief derives rocky *d* from mass/gravity and keeps them separate). Confirm semantics + distinct transforms for *z* vs *D* vs *d* BEFORE treating d³-from-`shellThickness` as a vigor derivation; R-Φsize's outcome test will NOT catch the conflation |
...
| **6. Heat-pipe threshold** | Raw pre-`calibrateTidal` Io-ratio, ~0.4–0.5 as an **order-of-magnitude peg** (≈ the brief's ≳1 W/m² against Io's ~2.2; model/epoch-dependent, not calibrated precision), exposed as tunable | Do NOT threshold the visual-tuned `calibrateTidal` knee |
...
**Pre-code gates (THREE):** delegable #1 and #2 are unwritten research, and the **interpenetration statistic** (§5.4 #2) is a named-but-undefined measurement — all three **block V2-2 coding** (R-phantom). Everything in §1–§5 is buildable once they are pinned.

> **ALL THREE GATES RESOLVED 2026-07-03** (adversarially verified; every quoted number reproduces from the committed scripts): `gate-1-L-lidstrength-form-DESIGN.md` (SH-F5 fork → non-monotonic `L` via two monotonic mechanisms: cold-thick `z` limb + hot-dry `muProxy` limb; `z` ≠ `shellThickness`), `gate-2-localyield-DESIGN.md` (sharp per-center boolean, `Ybase = Y0·exp(Y_K·L)`; 400-seed Monte-Carlo: Venus 0, Tharsis 1–3, compound minority band reachable), `gate-3-interpenetration-statistic-DESIGN.md` (`Π = C·F` with companion `M ≤ 0.70` guard; validated 100%/0% over 80 synthetic worlds × 2 meshes). **Findings that amend signed text (recorded in the briefs, not edited into §1–§5 — Max reviews at V2-2 scope):** (i) §2.3's "`n` carries the Mars/Venus separation" is falsified — the pierce split is carried by `Ybase(L)` + `L_STRONG` routing (gate-2 §8); (ii) §5.1's router needs a tidal-shoulder rule (`pure-strong = L≥L_STRONG AND rawTidal<SHOULDER_LO≈0.15`) or the `m_hp` seam cliffs (gate-2 PG-5); (iii) §5.4 #1 (wet-stagnant) is NOT closed by the gates — it needs E1's seeded-`'stagnant'` pick to set an **effective `L`** in the strong-mixed band (gate-2 §4; carried into V2-1 scope).
