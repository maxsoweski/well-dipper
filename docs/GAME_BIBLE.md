...
**What the drive is working against — the ether.** The gravitational field of a system is conceived in-fiction as an **ether-like medium** the ship cuts across during motion — a nod to the Victorian-era "luminiferous ether," the imagined substance through which things were once thought to travel. Smooth motion glides through the medium; the drive maintains neutrality easily. But a fast change in *speed* — not direction — produces a friction-like pileup against this medium, and that friction is what the shake renders visually. Accelerating hard is the ship **pushing INTO** the medium from rest — waves build against the hull as it breaks through, then release. Decelerating hard is the ship **slamming into the wall** of accumulated medium ahead of it — the impact hits first, then rings out. The two produce visually asymmetric shake patterns for this reason: they are different physical events against the same medium. Pure direction change at constant speed does not shake, because the ship is not changing its relationship with the medium's density — it is simply redirecting through it.

**Rendering shape — sustained tremor, not discrete bounces.** The friction against the medium renders as a **high-frequency small-amplitude sustained tremor** during the sharp-motion window, not as a discrete 4–5-bounce impulse train. The player's felt experience is aircraft turbulence: a subtle 1–2 second judder in the camera while the drive is cutting across a density spike in the medium, ramping in as the spike begins and ramping out as it passes. The accel/decel asymmetry from the prior paragraph is preserved as the **shape of the amplitude envelope** over the tremor's duration — accel crescendos-then-fades as the ship breaks through, decel impacts-then-decays as the ship slams in — not as counted bounces. The carrier is tremor; the envelope is the asymmetric story. The shake fires while the ship is *in* the sharp-motion window, not at the boundary that ends it: the felt moment is "I am currently experiencing turbulence," not "I just arrived and now the ship is ringing out." Primary rendering surface is **camera rotation** (small pitch/yaw/roll offsets applied to camera orientation), not camera position — translating the camera moves every framed object, which reads as the scene bouncing rather than the cockpit juddering. Rotational shake leaves static background geometry fixed in world-space; only the ship's heading jitters, which is the cinematic reading of "the pilot is holding a shaky helm."

**Arrival-compensation — why braking happens at the destination, not halfway.** A realistic reaction drive has to begin deceleration halfway to its destination — the physics of integrating mass over distance require it. The player's gravity drive does not. Because the drive warps the local gravitational field rather than pushing against conventional inertia, it can **brake sharply at arrival** — pulling the local field into a decelerating well right as the ship settles in against its target. The ship rides neutrally inside that field the whole way in, then the field steepens in the final moment. STATION arrival therefore feels like a *hard settle* — the ship arrives smoothly and then the drive snaps the field — not a gradual slowdown distributed across the approach. This is why the decel-side compensation shake belongs at the moment of arrival (the settle into orbit), not at the halfway point of transit or at the start of the approach close-in: the friction-against-ether event is the arrival event itself, because that is where the drive exerts its compensation impulse.
...
**Precision substrate (Shipped 2026-05-01):** Float32 (~7 sig figs) limits ship-scale precision when world coordinates grow large. Symptom first observed 2026-04-16 (Portal B's 100 m post-warp offset dissolved into FP noise once camera was more than ~10 scene units from origin); escalated to *blocking V1 warp* on 2026-05-01 when a session-accumulated worldOrigin of ~32,000 scene units made `camera.position.addScaledVector(forward, dt × ship-scale-speed)` round to zero every frame and the warp's FOLD/ENTER camera motion never happened. **Solved** by world-origin rebasing per `docs/PLAN_world-origin-rebasing.md`: every frame past `REBASE_THRESHOLD_SQ = 100²`, subtract the camera's position from every top-level scene child + tracked controller cache, accumulate the offset in `worldOrigin`, reset the camera to `(0, 0, 0)`. Implementation: `src/core/WorldOrigin.js`. Workstream: `docs/WORKSTREAMS/world-origin-rebasing-2026-05-01.md`. Telemetry-equivalence harness: `tests/refactor-verification/world-origin-rebasing.html`. Sub-object precision (cockpit / hull detail at <0.5 m relative to ship) remains future work — needs a ship-local nested coordinate frame inside the rebased world frame, on the agenda for the on-foot / cockpit-interior workstreams.
