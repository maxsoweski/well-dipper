...
| `lab-surface-ratchet`<br>`tests/fixtures/lab-surface-baseline.mjs:1` | Shrink-only ratchet over three measured sets from `world-engine-lab.html`. Growth blocks; shrinking is legal. | Source-level scan | **NONE** — entries added by hand, in the same commit, with a reason |
...
⛔ **`tests/fixtures/giantdeck-preset-baseline.mjs` is un-regenerable.** Its stated command names `scratchpad/capture-giantdeck-baseline.mjs`; that script is **not on disk** (`ls scratchpad/ | grep capture-giantdeck` → nothing). It sliced two live regions out of a pinned git blob of `world-engine-lab.html @ 4e864bc` and ran them with `new Function`.
...
| **LAB-PROCEDURAL-200** | `lab-procedural-0…199`, null ctx | 200 systems, 64 binary; **846** planets actually swap (asserted `tests/material-parity-list.test.js:303`; 341 before the Step 10a registration widened the predicate to `!== 'gas'`), 663 moons on them — the suite's own moon pins at `tests/material-parity-list.test.js:325-326` are knowingly stale at 228/456 while the moon-formation window is open | `material-parity-list` |
...
`planet-lod-CHARTER.md:1` — why the LOD lab exists, the standing north star, and ⭐ **the lab ≠ the game BY DESIGN**. If a handoff and this charter disagree on the FRAME, the charter wins. · `planet-visual-features.md:1` — the L0→L1→L2 causal model and the 18 type presets as driver bundles. · `world-engine-architecture-spine.md:1` — the story-engine frame in Max's words: what renders IS the body's history, so the engines' dependency order is time's arrow. ⛔ Self-described as a brainstorm, not a build. · `lab-vs-game-renderer-divergence.md:1` — why two renderers exist, and that it is intentional, not drift. · `binary-planets-scoping-2026-08-17.md:9` — a binary pair can be generated with correct barycentric physics and drawn to the correct SEPARATION **today** with zero renderer changes, if the companion is delivered through `planets[i].moons[]` with the full separation in `orbitRadiusScene`. ⛔⛔ **But its "drawn correctly" claim FAILED UAT on the first shipped pair, 2026-08-18 — see its §9.** The primary is nailed to the barycentre instead of circling it (`r1 = 5.53` primary radii unbuilt on `wd-10`), and one ring is drawn around the primary rather than two around the empty point, so the pair reads as a planet with a big moon. The fix is at `main.js:11201` INSIDE the write, plus the moon-ring source at `:11265-11271`.
...
⚠ Two known-live errors inside `one-pipeline-two-frontends-PLAN.md` itself, filed at `step6-parity-ledger.md:232` by a lane that could not make the edit: §12.4's committed eyeball seed does not swap, and §6b's five-row loss table is scoped to the gas branch while the swap is not. **The PLAN is knowingly wrong on those points until someone edits it** — under Rule 9's line-count constraint.

---

## 7. Known open defects in this layer

Each is recorded in-tree with a citation. ⭐ **Items 1 and 2 were FIXED on 2026-08-18** and are kept here with their resolution, because the decision behind item 2 is a design ruling that exists nowhere else.

1. ✅ **FIXED `df78785` — binary-star mass ratio disagreed with the gravity model.** `docs/FEATURES/binary-planets-scoping-2026-08-17.md` §1. Procgen ROLLED *q* off a dice table while `GravityField.js:83,99` derived masses from `radiusSolar^1.25` — median disagreement 1.22×, max 8.83×, barycentre misplaced up to 77.8 primary radii. Now recomputed from the actual radii after the secondary's ±15% jitter, via `physicalStarMassSolar` (`PhysicsEngine.js:1207-1229`).
   ⛔ **Why the guard, and do not "simplify" it away:** a bare `Math.pow(radiusSolar, 1.25)` ratio ships a visible regression. Procgen cannot roll `'D'`, but a HYG catalog star normalising to `'D'` arrives via `ctx.starTypeOverride` with no `companionSpec`, so the procgen roll runs; `STAR_PROPERTIES.D.radiusSolar = 0.01` gives 0.0032 M☉ against a real ~0.6, flinging *q* to 81.36 and **inverting the pair** at two named, visitable stars (HD 15634 and one unnamed HYG entry).
   ⚠ Consequence deliberately left open: for those `D` systems *q* now disagrees with `GravityField`, which still uses the wrong 0.0032 M☉. Closing it means changing GravityField's mass law, which moves shipped authored systems — its own workstream.
   ⚠ The brightness clamp at the same site is **required, not cosmetic**: once *q* is real it exceeds 1.0 in 27.5% of binaries (peak 1.652) and nothing downstream caps `starBrightness2` — the shaders floor `starLight` but never ceiling it, and the only ACES curve (`RetroRenderer.js:788`) is gated behind the cockpit pass. It is an EXPOSURE choice, not physics: in all 288 affected systems both stars share a spectral type, so the unclamped value is within ~1.2% of the true luminosity ratio and clipping costs a mean 17.1% ratio error. Max UAT-passed the clipped look 2026-08-18.
   `star2` remains a hardcoded singleton (155 occurrences, 17 files) — unchanged and still true.
2. ✅ **FIXED `d26971d` — binary-star orbits ran 41–948× faster than Kepler** (median 283.4×). The speed was computed from `binarySeparation`, the MAP-UNIT quantity the code's own comment labels "map/HUD only", and never passed through `orb()`/`ORBIT_REALISM_FACTOR`. Now `keplerOrbitSpeed(binarySeparationAU)` — the same call the authored branch already made. `StarSystemGenerator.js:19-25` records the planet-side version of the identical bug.

   ⭐⭐ **THE DESIGN RULING THIS DOC EXISTS TO CARRY — nothing else in the repo states it.** Correct physics makes a binary pair **visually static at 1× celestial time**. Measured live on `wd-272` (M+M, 0.294 AU): star A moves **0.0004° in six seconds at 1×**, versus **4.29° at 10000×**. A full revolution is 58.2 days at 1×, ~8.5 minutes at the slider's ceiling.
   **Max ruled 2026-08-18: KEEP IT.** Rationale: it matches what the authored real systems (Sirius, Alpha Cen, Procyon) have always done, and `celestialTimeMultiplier` reaches ±10000× (`index.html:178`, mapping `sign(pos)·10^(|pos|/10)`) for anyone who wants to watch a pair turn. ⛔ **Do not "restore visible motion" as a bug fix.** A future reader seeing motionless suns is looking at an intended outcome. If it is ever revisited, the honest option is a *deliberate, documented* binary time factor — not a return to the map-unit number.
   ⚠ **Residual, knowingly left in place:** `keplerOrbitSpeed` carries no √(M₁+M₂) term, so the pair sits at 0.139×–1.644× of the true two-body rate, systematically by class (O+O 0.150, M+M 1.501) — hot blue pairs are the most sluggish when they should be the fastest. Not fixed *because* circumbinary planets (`:554`) share the same omission, which makes `P_planet / P_pair = (a_p/a_b)^1.5` currently EXACTLY right; adding the mass term to the pair alone would break a correct number. One coordinated system-wide change, filed at `docs/FEATURES/moon-formation-channel-model-PLAN-2026-08-15.md:73`.
...
6. **Nothing on the NAV screen moves — planets or moons.** `binary-planets-scoping-2026-08-17.md` §3a. `NavComputer._systemData` is the generator's output (`:135`); the scene copies `orbitAngle` **by value** (`main.js:7719`) and the only two angle advances (`:11198`, `:11243`) write the scene copy, so nothing is written back. Both `p.orbitAngle` (`:2509`) and `moon.startAngle` (`:2567`) are generation-time values. Fixing it is the nav-screen rework, not a patch.
   ✅ **The moon DISTANCE half was fixed in `7814088`.** The old form multiplied by `moonOrbitWorld` and divided by it in the same expression, so the real orbit radius **cancelled exactly**, leaving `(baseR + 6 + 4m)/projScale` — distance by INDEX. `_renderPlanetDetail` (`:3066`) disagreed with the system view about where the same moon was. Now one shared `moonBandRadius` helper, with `NavComputer.moonBandRadius.test.js` guarding the cancellation.
   ⚠ §3a of that doc records being wrong about this **twice** — draft 1 called the radius cosmetic without knowing why, draft 2 "corrected" it to "uses real data" by reading the intent comment and stopping two lines above the cancellation. **A comment stating what code intends is not evidence of what it computes.**
...
Nothing in this file was verified by running the test suite at the time it was written — a dev server was live on this tree and that session was read-only. The instrument readings quoted came from `docs/FEATURES/moon-formation-handoff-2026-08-17.md`, a snapshot.
⭐ **UPDATE 2026-08-18:** `npm run check:instruments` has since been run at `1def6da` — **all four green** (A per-test-ID, B body-identity, C shipped-uniform + citations), with `known-failures.json` re-recorded for two added test files and the 24-entry failing set unchanged. Still **run it yourself** before trusting a reading; this line will go stale too.
