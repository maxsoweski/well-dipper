...
**New uniforms** (insert GLSL decls after the F47 `uMachWindowDensity` line, world-engine-lab.html:200; THREE entries after :3984; per-frame writes after :6869):
...
1. `world-engine-lab.html:200` (after `uniform float uMachWindowDensity;`) — add the 4 `uniform float uCity*` + 1 `uniform vec3 uCityColor` decls.
2. `world-engine-lab.html:912` (after `const int PROV_MACHINE = 43;`) — `const int PROV_CITYLIGHTS = 44; // F48 — neutral (civilization coverage, not geology; FROST-row, like bioMats F46 / machine F47)`.
3. `world-engine-lab.html:973` (after the `PROV_MACHINE` arm) — `else if (fid == PROV_CITYLIGHTS) { f = gProvince.z; fl = 1.00; }`.
...
5. `world-engine-lab.html:3633` (immediately AFTER the F47 `machC` block, before the F30 lightning comment) — INSERT the new `vec3 cityC = vec3(0.0); if (uCityMaturity > 0.0) { … }` block (the key GLSL below).
6. `world-engine-lab.html:3895` — extend the final composite: `... + bioC + machC + cityC, vec3(1.0)) ...` (add `+ cityC` to the existing `gl_FragColor` min()).
7. `world-engine-lab.html:3984` (after the `uMachWindowDensity: { value: 0.5 },` THREE entry) — add the 5 `uCity*` uniform-object entries (`uCityColor: { value: new THREE.Color(0.95,0.75,0.3) }`).
8. `world-engine-lab.html:4596` (after `machWindowDensity: 0.5,`) — state-init defaults: `cityLightsEnabled: true, cityMaturity: 0.5, cityIntensity: 0.9, cityScale: 2.8, cityCoastBoost: 1.6, cityColor: [0.95,0.75,0.3],`.
9. `world-engine-lab.html:4879` (after the F46 `fEnv.add(state,'bioMatsEnabled')` line) — GUI: F48 is pure-emissive like F46, so it lives in the **Envelope `fEnv` group** (NOT `fExoticGroup` — that was F47's home because F47 has a relief channel). Add the city sliders + the load-bearing enable literal (see GUI bindings below).
10. `world-engine-lab.html:6434` (after `machine: fMachine,` in `featureFolders`) — `cityLights: fCity,` (`fCity` = the folder created at step 9 if a sub-folder is used; if the sliders go directly into `fEnv`, map `cityLights: fEnv` exactly as `bioMats: fEnv` does at :6433).
11. `world-engine-lab.html:6869` (after the `uMachGlowColor.value.setRGB(...)` writer line) — per-frame writes (the 5 uniforms; `uCityMaturity.value = state.cityLightsEnabled ? state.cityMaturity : 0.0` is the sole gate; `uCityColor.value.setRGB(...)`).
...
**Solo plumbing:** none needed — `setFeatureEnables` (world-engine-lab.html:6455) and the per-folder solo button (:6479) iterate `Object.keys(FEATURES)` generically. Registering step 12 + mapping step 10 makes `window._lab.solo('cityLights')` work automatically (the §5 recipe).
