...
- Status: VERIFIED_PENDING_MAX (lab, 2026-06-10) · ⭐ **CONVERGED IN THE GAME 2026-09-03** — the 4th piece was `uTermBypass` (a lab display knob, now written as display policy); the real finding was that the game DREW the band (`columnFraction × 0.15` on every body with air, from 2026-08-21) while the lab had it OFF on every preset since Max's 2026-07-16 ruling. One declared constant `TERMINATOR_ENABLED = false` (`solidOptics.js`) is now read by the game's RULED gate policy AND both lab producers; the lab's retyped width law at :2505 is deleted. **Max's game-side UAT 2026-09-03 on rocky-2 (A/B key `.`, half phase): *"Terminator is there (but looks really bad as usual). This has been in the game for weeks at this point though, what's the change"* → on being shown the change IS the removal: *"1 yes"* — SHIPPED `2e60453`, the band OFF in both front-ends.** Its LOOK is F52 / the lighting engine's, parked by his 2026-09-02 ruling; turning it back on is one constant. Workstream `wire-terminator-gradient-lab-into-game`.
