...
1. **planet-lod-lab.html:187** (GLSL uniform decls, after the F46 `uBio*` block at :183-188) — add the 9 `uMach*` uniform declarations.
2. **planet-lod-lab.html:901** (after `const int PROV_BIOMATS = 42;`) — add `const int PROV_MACHINE = 43; // F47 — neutral (engineered overlay, not geology)`.
3. **planet-lod-lab.html:961** (provinceWeight if-chain, after the `PROV_BIOMATS` arm at :961) — add `else if (fid == PROV_MACHINE) { f = gProvince.z; fl = 1.00; }`.
...
5. **planet-lod-lab.html:3026** (analytic relief stage, immediately after `shatterCombiner(vPos, h, grad);`) — call `machineRelief(vPos, h, grad); // F47 panel bevels onto grad (ADDITIVE, F19 contract; gated by uMachCoverage × provinceWeight(PROV_MACHINE))`.
...
12. **planet-lod-lab.html:6313** (`featureFolders` map, after `bioMats: fEnv,`) — add `machine: fMachine,` so solo/filter/relocate-enable plumbing picks it up (the `setFeatureEnables`/`window._lab.solo` path at :6334/:6358 then works for free).
13. **planet-lod-lab.html:6736** (per-frame writer, after the `uShatSubAmt` write :6736) — add the writer block:
```js
...
- `machSeamWidth` 0.06 (range 0.01–0.15) — matched to the ~500px posterized-disc seam scale (the F45 `shatBorderWidth` 0.04 precedent at planet-lod-lab.html:6237).
...
- **Tweaks applied** (planet-lod-lab.html state-init defaults; both within existing GUI slider/picker ranges, no range edits needed):
