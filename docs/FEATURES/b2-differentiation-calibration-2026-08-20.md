...
| 19 non-gas planet-class | **19 / 19** | the same legacy `Planet` program — src/main.js:7731 `        const planetMoon = new Planet(scenePMData, pmStarInfo);` where `scenePMData` is a spread of `moonData.planetData`, so the baked palette carries through | **14 move — REAL TODAY** |
