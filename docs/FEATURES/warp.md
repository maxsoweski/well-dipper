...
- **FOLD** — portal appearance now works at any test-start position. Diagnosed 2026-05-01: the 2026-04-18 "appearance works okay today" claim was correct only at near-origin test positions (worldOrigin ≈ 0). At session-accumulated worldOrigin of ~32,000 scene units, `camera.position.addScaledVector(forward, dt × ship-scale-speed)` (FOLD's per-frame camera motion of ~5e-10 scene units) was rounding to zero every frame because float32 precision at coord 32,000 is ~4e-3 — seven orders of magnitude too coarse. Camera never approached portal; portal stayed open at sub-pixel apparent size; full 5+ s of FOLD + ENTER showed only the fullscreen hyperspace shader streaks with no portal mechanism visible. **Fixed** by world-origin rebasing (Shipped 2026-05-01 per `docs/WORKSTREAMS/world-origin-rebasing-2026-05-01.md` and `src/core/WorldOrigin.js`): camera stays near rendering origin, ship-scale per-frame motion stays above precision, portal is visible at all worldOrigin magnitudes. Verified across two test-start positions (Sol origin + worldOrigin ≥10,000 scene units) per the workstream's recording ACs.
- **ENTER** — **threshold-crossing now fires at all worldOrigin magnitudes** (post-rebasing 2026-05-01). The `OUTSIDE_A → INSIDE` traversal-mode transition was previously stuck at `OUTSIDE_A` for the full ENTER phase because the camera physically never approached the portal plane (same float32 precision root cause as the FOLD entry above). Telemetry confirms transition at FOLD progress ~0.9997 / start of ENTER post-rebasing. Separately, the 1–2 second freeze at the initial slowdown — a load-timing problem (asset loading happens during a continuous-motion phase) — remains open; tracked by `docs/WORKSTREAMS/warp-phase-perf-pass-2026-04-20.md`, NOT addressed by rebasing.
...
- **`docs/WORKSTREAMS/warp-phase-perf-pass-2026-04-20.md`** — **Active 2026-04-20.** Diagnose + fix the three phase-window perf hitches Max observed in the un-do recording (FOLD acceleration chug, ENTER → HYPER threshold-crossing freeze, EXIT fly-out stutter). Director-bundled because the phases are contiguous and the root causes likely overlap (shader compile / allocation churn / velocity discontinuity). Attacks §"Current state snapshot (2026-04-18) ENTER" — the 1–2 s freeze named as the primary V1 blocker — and closes the motion-continuity gap the un-do recording's AC #5 marked PARTIAL. Principle 6 scrutiny: Chrome DevTools trace before any fix; separable diagnosis + fix commits; canvas-path recording as the Shipped gate. If H1 (sync GPU work at ENTER) fires, the implementation vehicle is `docs/PLAN_transition-freeze-fix.md` Phases 1 + 2.
- **`docs/WORKSTREAMS/world-origin-rebasing-2026-05-01.md`** — **Active 2026-05-01.** Implement world-origin rebasing per `docs/PLAN_world-origin-rebasing.md` to fix the float32 precision-ceiling regression diagnosed this session: per-frame camera motion during FOLD/ENTER (≈5e-10 scene units) is seven orders of magnitude below float32 precision at world coord ~32,000 (≈4e-3 scene units), so `addScaledVector` rounds to zero every frame and the camera never approaches the portal. The warp state machine fires correctly; the camera doesn't move. Bible §10 Scale System "Precision ceiling (deferred work)" graduates to Shipped. Verification splits by contract: telemetry-equivalence on autopilot + body positions (refactor surfaces), canvas-recording at Sol origin AND a position ≥10,000 scene units from origin (warp authored experience). Also addresses two Max session observations — idle portal "follows the player around" (should be world-fixed) and Stage 2 alignment slerp not visibly reorienting camera before warp init.
