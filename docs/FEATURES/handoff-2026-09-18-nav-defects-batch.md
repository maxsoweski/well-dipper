...
| `docs/WORKSTREAMS/nav-defects-batch-2026-09-18/verdict.json` (if the verify workflow wrote it) | verify-workstream FULL over the 22 ACs at `b3f7737` — one commit behind the home-test fix; the delta is one adapter field + one lab line, pinned by 14/14 design cases. |
