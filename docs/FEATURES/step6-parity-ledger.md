...
|---|---|---|
| **How much of the game's material varies from body to body** | **45 of 72** uniforms ⭐ **71 → 72 AT B2P, 2026-08-20** — `uPosterizeLevels`, the colour quantum. The DENOMINATOR moved and the numerator did not, deliberately: it is one global display value, identical on every body, so it enters the declared set without entering the varying one. Pinned at tests/material-parity-list.test.js:406 `expect(LEDGER.gameSize).toBe(72);`. | 37 of 71, as measured then — a historical column, left at the count the material had that day |
...
| swapped bodies with ≥1 moon | **420** of 846, **663** moons — ⛔ measured here, NOT pinned: tests/material-parity-list.test.js:325 `expect(CENSUS.withMoons).toBe(228);` is left stale and red on purpose, because the moon population is live under the open formation window (34b502d) and re-recording it here would absorb another lane's defect into this one. That pin is B7's | 228 of 341, 456 moons |
...
|---|---|---|---|
| P-01 | `starColor1` `starBrightness1` | accepted-loss | `[lab-lacks-it]` The lab material declares **no star-colour uniform of any kind** — zero of its 356 names contain `star` or `Star`. Every one of the 341 swapped bodies renders under implicit white light; measured, every primary in the corpus is non-white, so this is visible on 341/341. No lab mechanism exists: closing it is a shader change, not a mapping line. |
| P-02 | `lightDir2` `starColor2` `starBrightness2` | accepted-loss | `[lab-lacks-it]` `uLightDir2` does not occur anywhere in `LAB_SHADER_CORPUS`. 125 of 341 swapped bodies sit in a binary; they lose the second star's diffuse term outright. `starColor2`/`starBrightness2` land in `lost` (non-zero) on exactly those 125 and in `lostAtZero` on the other 216 — the split IS the binary population, which is the control that the bucket assignment is reading real values. |
| P-03 | `shadowMoonCount` `shadowMoonPos` `shadowMoonRadius` `shadowPlanetCount` `shadowPlanetPos` `shadowPlanetRadius` `starPos1` `starPos2` | accepted-loss | `[lab-lacks-it]` No lab uniform name contains `hadow`; `uShadow` does not occur in the corpus. 228 of 341 swapped bodies carry 456 moons between them, and every writer of these is guarded (`main.js:9865`/`:9847`) so the loss throws nothing. Transits and planet shadows stop being drawn on swapped bodies. |
...
| P-18 | `lightDir` `planetRadius` `time` `uPosterizeLevels` | carried | The four that genuinely survive (three until B2P, 2026-08-20), each by a named mechanism and each verified to VARY across bodies on the post-swap material: `lightDir`→`uLightDir` and `planetRadius`→`uBodyRadius` are written by `buildLabPlanetMaterial`, and `time`→`uTime` by the per-frame seam `updateLabPlanetMaterial`. ⭐ **`uPosterizeLevels` JOINED THIS ROW AT B2P, 2026-08-20, AND IT IS THE ONE SUBJECT HERE THAT DOES NOT VARY ACROSS BODIES — deliberately.** It is the colour quantum: a global DISPLAY setting, not a per-body derived quantity, so sameness across bodies is the feature and not a collapse (`port-uniform-delta.mjs` files it UNWATCHED for the same reason). The mechanism this row demands is named, and it is stronger than the rename the other three rely on: `buildLabPlanetMaterial` substitutes THE SAME uniform object for the lab's differently-spelled `uLevels`, so the post-swap material does not receive a copy of the value — it holds the identical object the pre-swap material held, and one assignment moves both. ⛔ Which is exactly why the name still lands in `lost`: the SPELLING leaves the material and the QUANTITY does not, which is §2's `carried` in its own words. |
<!-- /LEDGER-CH1 -->
...
`tests/moon-lab-mount.test.js`, and `uPosterizeLevels` (B2P, 2026-08-20) in `tests/posterize-levels-wiring.test.js` — rather than inferred from a name.
