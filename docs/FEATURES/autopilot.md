...
- **CRUISE-entry gate — dot-gate primary, fixed-duration timeout fallback.** lhokon completes when `dot(camera_forward, normalize(new_target.current_position − camera.position)) ≥ 0.9999`, OR when 1.5 s has elapsed (whichever fires first). The dot-gate is the normal exit; the timeout is a safety net against degenerate-geometry hangs (near-antiparallel start/end directions). If the timeout fires under realistic playable conditions (Sol tour, normal speeds), it is a flag for §"Failure criteria" review — same shape as the §A4 discriminant-fallback flag on §CRUISE.
- **Smoothing curve.** Cubic ease-out on lerp progress (matches the prior in-CRUISE smoothing curve; only the phase site changes). Normalized lerp from start direction to end direction; end direction is re-derived each frame to track an orbiting new body.
