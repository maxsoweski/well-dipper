...
- **2026-08-09 — Camera aim choreography DEFERRED by Max, and made a requirement for screensaver MVP.** `CameraChoreographer`'s critically-damped aim spring was surfaced to Max during unrelated world-engine work; he had not known it was a developed feature. Ruling, verbatim: *"This is functionality that I do want to build into the future of autopilot, but need to think about how exactly to do that… as a description of feature and some basic functionality, I think this is good, but we need to defer this. I will choose [when] to pick it back up. But let's say that this is a requirement for the MVP screen saver mode shipping."* Three consequences, recorded because each is easy to lose: **(1)** the mechanism is **committed but dormant** — it does not execute at HEAD and nothing a player does reaches it, so it is neither shipped nor lost; **(2)** resumption is **Max-gated by name** — no agent picks this up on its own initiative, and "it's cheap now that we're in the file" is not a reason to; **(3)** it is now a **ship-gate on screensaver-mode MVP**, which is the first time this mechanism has a deadline attached to it. See §Deferred — camera aim choreography below for the full state of the code, the three things reviving it would first have to repair, and the ⚠ architectural consequence of the screensaver requirement.
...
## Deferred — camera aim choreography (`CameraChoreographer`) — MAX-GATED, REQUIRED FOR SCREENSAVER MVP

**Status: built, committed, dormant, never through UAT. Deferred 2026-08-09 by Max. Resumption is his call and nobody else's. It is a ship-gate on screensaver-mode MVP.**

### The beat

- **As a viewer, I want the camera to hold a subject steadily while the ship moves past it — easing its aim rather than snapping — so that a close pass reads as a cinematographer's shot and not as a lurch.**
- **AC:** the aim point eases toward its target with no overshoot and framerate-independent timing; a close pass produces no visible swing; phase changes between framing beats are blended, not cut.

### What exists today

`src/auto/CameraChoreographer.js` decides **where the cinematic camera looks** — aim only. It never moves the camera and never calls `lookAt` itself; it hands one world-space point to `FlythroughCamera`, which does. Position authority lives elsewhere.

The mechanism is a **critically-damped spring on a ship-relative aim offset**, using Daniel Holden's closed-form `spring_damper_exact` integrator, so the feel is identical at 30 fps and 144 fps. `zeta` is hardcoded to 1.0 — it eases in and **never overshoots**, which is the change's most characteristic signature and a deliberately different feel from an underdamped Cinemachine-style rig. On top sit a ~1.8 s linger on the body just departed and a forward "lean" 35 % toward the next body during CRUISE.

It replaced three failed attempts at the same problem. Cycles 1–3 capped the camera's angular rate in degrees per second; all three failed, and the prior-art survey found rate-capping is not how real camera systems solve this. The spring targets the actual failure — a small sideways motion at short range is a large angle change, worst exactly at closest approach.

### ⛔ Three things reviving it must repair FIRST

1. **It never executes.** `SHIPS_ENABLED = false` (`src/main.js:171`) → no ships spawn → `focusShip()` early-returns → `navSubsystem.beginMotion` is never called (its only live call site is `src/main.js:8880`, inside that function) → `flythrough.active` stays false → `FlythroughCamera.update` never runs → the choreographer never ticks. There is no flag on the choreographer itself. Any claim that it "changes flight feel" is false at HEAD.
2. **Its intended successor path is dead.** `setShip()` is meant to select the 2026-04-25 V1 collapse (camera-forward === ship-forward, no linger, no pan-ahead). That branch reads `motionFrame.cameraLookDir`, which came from the retired `AutopilotMotion`; `NavigationSubsystem`'s frame does not carry it. Execution falls through to the legacy framing-state path **under a comment reading "Should not happen."** So a naive revival runs the mechanism the record retired, not the one the redesign specified.
3. **`MIN_TARGET_DISTANCE` is mis-sized, and its own comment says the opposite.** It is 2.0 scene units, calibrated against degenerate clusters observed at 0.30–0.46. Real hold distances are 0.02–0.1 for planets. Its guards therefore fire **every frame on every real target**, not "rarely" as documented. Benign in direction (the push preserves direction exactly) but it means the guard is doing structural work nobody sized it for.

Also: **zero tests reference this module** — measured, not estimated. The cycle-1/2/3 work was judged by violation counts against recorded `.webm` flights; the cycle-4 spring was never put through that instrument before the workstream closed. `TARGET_HALF_LIFE_SEC = 0.35` is a seed justified by a Blue-Danube phrasing argument, never validated; the workstream's own budget allowed two tuning passes and closed before either ran.

### ⚠ What the screensaver requirement implies, stated now rather than discovered later

The screensaver **is** the HELM hands-off autopilot tour (`src/flight/flightModes.js`, `tour` mode — *"system tours every body (screensaver)"*). Today that tour runs on `SupercruisePilot` with the camera **welded to the ship by HeadMount** — first-person, no independent aim axis at all. So "camera auto-look is required for screensaver MVP" is not a tuning request against the shipped path; **it requires the screensaver to grow a cinematic aim axis it does not currently have.** That is the real cost of the requirement, and it is larger than reviving the module. The two camera authorities cannot currently fight — `scActive` and `flythrough.active` are mutually exclusive by an else-chain — but that is because one of them is dead, not because anyone reconciled them.

### The record says this was already retired once

`docs/WORKSTREAMS/autopilot-live-feedback-2026-04-24.md` marks this loop **superseded on 2026-04-24**, retired by reframe rather than patched, after Max watched a much simpler Elite-Dangerous-style stub (`screenshots/max-recordings/stub-saturn-v4-2026-04-24.fixed.webm`) and said *"that works very well."* It records the implementation as preserved in `git stash@{0}` — ⚠ **it is not there**; that stash is unrelated July world-engine WIP. The code lived in the working tree for ~106 days and was committed in `92614e5` to end a dirty tree, not to ship a feature; that commit says so in its own first line. **Code wins on what exists; the doc wins on what the intent was. Nobody has reconciled them, and that reconciliation is the first thing the revival owes.**
...
- **`docs/WORKSTREAMS/manual-nav-autopilot-toggle-fix-2026-05-02.md`** — **Active — scoped 2026-05-02.** Lifecycle fix at the manual-nav ↔ autopilot toggle boundary plus a focused code-review pass over adjacent lifecycle plumbing. Lands against §"Manual override — two-layer architecture" and the §"Failure criteria" entries *"Manual override snap-stops the ship"* (today's failure mode is the inverse — release doesn't happen at all) and *"Autopilot-on-then-off-then-on auto-resumes"*. Three user-visible bugs (`autopilotMotion.isActive` never releases / manual flight gate dead / Q half-engages on residual state) traced in the diagnosis at `screenshots/diagnostics/manual-autopilot-toggle-2026-05-02/DIAGNOSIS.md`; root cause is `stopFlythrough()` missing `autopilotMotion.stop()` and the warp-arrival handoff at `main.js:6796` starting `autopilotMotion` without checking `_autopilotEnabled`. Code-review surface (ACs #6–#11) explicitly part of scope per Max's direction so the fix doesn't paper over deeper rot.
