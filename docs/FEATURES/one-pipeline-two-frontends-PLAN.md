...
**They already share the derivation core.** `src/worldengine/port/conditionFromPlanet.js:24` and `planet-lod-lab.html:174` import `deriveConditionVector` from the *same* file, `body-condition-vector.js`. There is one condition engine, not two. [VERIFIED — read both import lines.]

What is actually duplicated is the two *ends*:

**(a) Upstream — two constructors for the engine's input.** The game builds an `fp` literal at `conditionFromPlanet.js:119-137`; the lab builds one from `DRIVER_PRESETS`. Fed the same nominal body they disagree by 3–6× on temperature (Venus 737 K lab / 2345 K game; Jovian 125 / 776) and by 5–7 orders of magnitude on tidal heat (Europa 137 / 0.0019). Two causes, both verified:
- The game double‑applies a greenhouse correction — `T_eq: surfaceTemperatureOf(d.T_eq ?? 288, atmosphere?.pressure)` at `conditionFromPlanet.js:131`, a fit calibrated on Earth 1 bar and Venus 92 bar (`:37-52`) and then handed a giant's 1000 bar, where it multiplies by 6.207×.
- The port reads `d.tidalHeat` (via `baseStep.js:26-33`) while `PlanetGenerator.js:796` emits `tidalHeating`, so the real value is dropped and a hardcoded 1 M☉‑at‑1‑AU fallback substituted. Measured over 161 generated planets: within 2× of the true value for **5.6%** of them, median 75× off.
...
**The cost is not effort — it is drift.** Three laws have already diverged while someone was actively trying to keep them in sync:
...
**One correction to a claim you will see repeated.** The port does **not** drop `massEarth` and `surfaceHistory` from its `fp` — both are there, at `conditionFromPlanet.js:121` and `:135`. [VERIFIED by reading lines 119–137.] It drops **three** things: `tidalHeat` (name mismatch), `magneticField` and `metallicity` (never set, though `body-condition-vector.js:156-157` declares them as vector keys). And `surfaceHistory` *goes in* and is *not emitted* — the loss is in the vector, not the adapter. Both statements about `surfaceHistory` in the recon are true; they describe different halves of the same pipe. Fix the vector, not the fp.
...
- Row status derived from `docs/FEATURES/planet-lod-campaign-tracker.md:56-105`, `docs/FEATURES/cards/PROFILES.md:35-49`, and a mechanical name‑intersection of `Planet.js:1548-1717`'s 71 uniforms against `planet-lod-uniforms.js`'s 351 (27 shared names).
...
- **(c) Wire‑and‑it‑renders‑nothing until world‑gen work lands** — ~8 features whose inputs are degenerate today: `uRayBrightness ≡ 0` because `hasAtmo` is true on 100% of bodies; `uFacetStrength ≡ 0` because `conditionFromPlanet.js:102` only nulls atmosphere when `retained === false`, which never happens; `habGate ≡ 0`; `airlessnessOf ≡ 0`. **These must not be measured through the renderer.**
...
- **C.** `tools/port-uniform-delta.mjs`: resolve the **27 uniform names shared** between `Planet.js`'s production material and `planet-lod-uniforms.js` over ≥300 generated bodies, and emit min/median/p95/max deltas against a committed capture.
...
### Step 1 — Widen the condition contract, additively, with provenance · **M** · deps: Step 0

**What.** Two files, no law changes.

1. In `src/worldengine/base/body-condition-vector.js` (still at the repo root at this point), **emit** `surfaceHistory` — it is already handed in at `conditionFromPlanet.js:135` and silently dropped by the vector's return literal (`:109-158`).
2. In `conditionFromPlanet.js`, forward `magneticField`, `atmosphere.color`, `habitability`, `axialTilt` (**convert to radians at the seam** — the game stores 0.41 for 23.4° at `SolarSystemData.js:180`; do not pass through), and `radiusEarthCanonical` distinct from the drawn radius.
3. **Do NOT forward `metallicity` here.** It lands in Step 5. Reason below.
4. Replace the atmosphere sniff at `conditionFromPlanet.js:101` — `if (!phys) return gameAtmosphere; // already engine-shaped` — with a **positive shape validation** (require `retained` or `pressure` to be present). `MoonGenerator.js:192-196` emits a visual‑only `{color, strength}` that passes the current check and yields a condition whose atmosphere is **truthy** and whose pressure is **undefined**, i.e. "has air" for truthiness gates and "vacuum" for every pressure gate.
5. Emit `_provenance` — for each of the 13 inputs, `'measured'` or `'defaulted'`.
...
**Files.** `src/worldengine/port/conditionFromPlanet.js`, `body-condition-vector.js`, `tests/port-condition-contract.test.js`.
...
- **Instrument C: zero delta on all 27 shipped uniforms.**
- **The giant‑driver triple** `{internalHeat, shellDepthFrac, dissipation}` is byte‑identical over 200 generated bodies. *This is the assertion that actually means "additive."*
- `_provenance.massEarth === 'measured'` on a generated planet, `'defaulted'` on `{radiusEarth: 0.273}` alone.
- `atmosphereFromPlanet({color, strength})` no longer returns a truthy atmosphere. **Note:** measured 177/177 generated planets carry `{color, physics, strength}` and 0 lack `.physics`, so this change is moon‑only. If this gate *does* go red, it is a real regression, not expected churn.
...
- A **committed delta table** over ≥300 bodies: min/median/p95/max for `rawTidalIoRatio`, `surfaceGravity`, `T_eq`, and the four bakes `PlanetGenerator.js:753-755` writes onto `planetData` (`landPalette`, `iceness`, `lavaGlowColor`, `lavaCrustColor`), plus the crater uniforms.
- **Instrument B stays GREEN** — no new draw was added.
- **Instrument A** shows only tests named in the commit message, each re‑blessed in its own follow‑up commit. Not a blanket re‑record.
...
- **Queue (c) — the ~8 features that would wire correctly and render nothing.** `uRayBrightness ≡ 0` (F3 rays, because `hasAtmo` is true on 100% of bodies), `uFacetStrength ≡ 0` (F43, because `conditionFromPlanet.js:102` only nulls atmosphere when `retained === false`, which never happens), `habGate ≡ 0 ⇒ uBioCoverage ≡ 0` (F46), `airlessnessOf ≡ 0`, `erosionOf` never returns 0. These need **world‑generation** work, not wiring. They must not be measured through the renderer.
