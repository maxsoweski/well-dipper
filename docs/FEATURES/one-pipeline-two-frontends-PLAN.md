...
- The port reads `d.tidalHeat` (via `baseStep.js:26-33`) while `PlanetGenerator.js` emits `tidalHeating` (grep `tidalHeating, // D12`), so the real value is dropped and a hardcoded 1 M☉‑at‑1‑AU fallback substituted. Measured over 161 generated planets: within 2× of the true value for **5.6%** of them, median 75× off.
...
| **D — dead gate** | a gate the step *names* cannot fail on its own declared subject: vacuous, self‑referential, subject outside its watched set, **or the next declared step's own move can be written past it** | **BLOCKS** |
