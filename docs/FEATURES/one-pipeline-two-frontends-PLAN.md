...
|---|---|---|
| **symbol only, no line** | ``PlanetGenerator.js `baseColor: palette.base` `` | `src/worldengine/port/conditionFromPlanet.js` (all of it) and the record‑literal / bake region at the bottom of `PlanetGenerator.generate`. Every step in this plan adds lines to both, so an integer written there is born with a half‑life of one step. ⭐ **EXTENDED 2026‑08‑08 to `tests/radius-live-feed.test.js` and `tests/radius-live-feed-fence.test.js`, on measured evidence rather than by analogy.** Step 3 rewrote both (+409 and +463 lines), and four refs from *this document* into them broke inside a single round — then broke a SECOND time, in the same session, when round 2's fix lanes moved the same anchors again. `const REQUIRED_CARRIERS` travelled :139 → :187 and `const BOOT_SEED` :234 → :300 **while agents were editing concurrently**, so a line written at the top of a lane is stale before its own commit lands. These files now satisfy the exact test this row already states — every step that touches them adds lines to them — so they take the same exemption for the same reason, and §11.2 requires closing the class rather than repairing the four instances a third time. |
...
**Part of it already exists and already works.** src/main.js:2411 `async tryLabShader(index = 0) {` puts the lab's real material, off‑thread‑compiled, on a real generated game body, and the per‑frame seam is wired at both ends — src/objects/Planet.js:1928 `updateLabPlanetMaterial(mat, {` feeds the clock and the object‑space light, src/rendering/objects/BodyRenderer.js:211 `updateLabPlanetMaterial(surface?.material, { distanceRadii, cameraWorldPos, mesh: surface });` feeds the LOD ramp. A previewed body therefore animates and ramps with no further work. ⛔ **Everything below this line does NOT exist at `9b33264` and is written here as work, never as a hook to call.** Measured, not assumed: `grep -rn "previewPack\|swapLedger\|shotState\|restoreGameMaterial\|freezeFrame\|cameraPose" src/` returns zero hits, `scripts/shot-diff.mjs` does not exist, and `writePackUniforms` arrives with Step 5a.

**E‑1. The registry — and what it does NOT buy.** src/rendering/ShaderWarmup.js:224 `mesh.material = material;` drops the previous material and keeps no reference, so today the swap is destructive and the only revert is a page reload. Keep `{mesh, prevMaterial}` and add `_lab.restoreGameMaterial({ body })`. ⛔ **The OFF twin it produces exists only for MANUALLY swapped bodies** — Steps 5, 9 and Step 10's hand‑picked moon. It does **not** exist on the automatic path: 6e keeps the legacy branch behind an off‑by‑default flag, so with the flag ON the lab material is selected at material‑creation time and the legacy `GAS_BODY` material for that body **is never constructed**. `prevMaterial` is undefined and there is nothing to restore. At Step 12 the branch is deleted from source outright. **On the automatic path the OFF twin is a flag flip plus a reload at a restored pose** — `spawnProceduralSystem(seed)` is deterministic, so with `setCameraPose` the pair is two page loads at one pose, and the determinism assertion (same seed ⇒ same body list, same radii) is part of the caption.

**E‑2. NAME‑keyed addressing, everywhere.** ⭐ **Index keying produces a false PASS, so no E hook takes an index as its subject.** Two index spaces are live at once: src/main.js:2700 `selectBody(kind, idx, moonIdx) {` indexes `system.planets` in sim order, while the scene walk at src/main.js:2420 `if (o?.material?.uniforms?.noiseScale && o.geometry) {` filtered by src/main.js:2422 `if (owner.startsWith('body.planet.')) surfaces.push(o);` indexes a traversal — and PLAN.md:441 `it never excluded *all* moons` records that this filter already admits planet‑class moons, because they are named `body.planet.*`. In any system carrying one, the camera frames body X while the preview drives body Y. Worse, Step 10 widens the prefix to `body.`, and src/objects/Moon.js:42 `noiseScale: { value: d.noiseScale },` means plain moons pass the uniform test too — so `surfaces[i]` for a given planet **shifts at Step 10**, and "re‑run Step 6's shot line verbatim" at Step 12 lands on a different body, obtains the expected null on a body nothing touched, and passes. Every hook therefore takes `{ kind, p, m }` resolved through `system.planets[p]` / `.moons[m]` down to `.surface` — the space `selectBody` already uses — and **prints the resolved mesh name**; an index is a discovery convenience that returns a name. Steps 7 and 12 re‑run **against the recorded body NAME**, never against a recorded integer. The walk's own stabilisation is still needed (the lab material declares `uNoiseScale`, not `noiseScale`, so a swapped body drops out of its own list) and rides along with Step 10's prefix widening.

**E‑3. The drive.** `_lab.previewPack({ kind, p, m, pack })` computes `pack(condition, ctx)` for **that body**, writes through Step 5a's `writePackUniforms` **under the GAME display policy**, and `setAttribute`s `aBand/aShear/aMush` itself — src/rendering/LabPlanetMaterial.js:54 `geometry.setAttribute(name, new THREE.BufferAttribute(new Float32Array(count), 1));` deliberately never overwrites an existing attribute, which is correct and means the preview must fill them. ⛔ **Values come from the pack's return object only, and are printed beside the frame** (scoped per §12.2). Reaching the condition needs a back‑link that does not exist: src/objects/Planet.js:1568 `const condition = conditionFromPlanet(d);` is a local, and `grep -n userData src/objects/Planet.js src/rendering/objects/BodyRenderer.js` returns **zero** — the walk yields a bare mesh from which `d` is unreachable. Two lines in src/objects/Planet.js:1548 `_createSurface() {` fix it for every future instrument, not only for E.
...
**Channel 1 — the live uniform diff, which sees the alias shape (C20).** Because E‑1 holds both materials **on the same body**, `_lab.swapLedger()` is a pure two‑object diff into three buckets: **LOST** (present and non‑zero before, absent after), **OFF‑BY‑DEFAULT** (absent before, present‑at‑zero after), **CARRIED** (present both). src/objects/Planet.js:1616 `uLimbMix: { value: LIMB_MIX },` falls out as LOST; planet-lod-uniforms.js:40 `uLimbStrength:   { value: 0.0 },` falls out as OFF‑BY‑DEFAULT; planet-lod-uniforms.js:425 `uPolarStrength:   { value: 0.0 },` likewise. **§6b diffs two FILES and was populated by reading; this diffs two LIVE MATERIALS and is populated by running.** ⭐ And it fires at **Step 5's** hand‑picked preview, one step before Step 6 ships the swap, so the losses are printed **before** Max is shown anything.

⚠⚠ **Whether C19 and C20 are losses AT ALL is not settled by a declared default, and this section must not pretend otherwise.** Both uniforms are *enable‑gated*, not merely zero‑defaulted: planet-lod-lab.html:4995 `uniforms.uLimbStrength.value = state.limbEnabled ? state.limbStrength : 0.0;   // ✓ enable gate` and planet-lod-lab.html:5174 `uniforms.uPolarStrength.value = state.polarVortexEnabled ? state.polarStrength * state.featureRelevant.polarVortex : 0.0;   // ✓ enable gate × per-feature relevance hard-gate (Thread B idiom) — zeros Mars leak (polar vortex authored for gas giants, not terrestrial)`. Max's **ruling 4** at PLAN.md:625 `for the game — CONFIRMED.` puts every one of the 49 checkboxes ON in the game, so a port honouring it opens both gates and the two rows stop being regressions. **A gate is only half a feature, though, and the two rows differ on the other half — measured, not reasoned.** `state.polarStrength` has exactly ONE producer in the entire lab, planet-lod-lab.html:1911 `state.polarStrength = _pl.strength;`, which is **inside the fenced `applyStormState`** — so `gates = ALL_ON` opens a gate onto a value the game has no way to compute, and **C19 survives the ruling.** `state.limbStrength` is produced at planet-lod-lab.html:2001 `state.emissive = u.emissive; state.specStrength = u.specStrength; state.limbStrength = u.limbStrength;` and boosted at planet-lod-lab.html:2453 `if (_thickHaze) state.limbStrength = Math.min(1.0, state.limbStrength * 1.3);`, **both inside `applyDrivers`** — so the value exists on both sides and **C20 is contingent on one line of the port's uniform mapping**, which makes it the *"two names for one value"* class §2 already records happening four times, not a feature nobody can compute. ⛔ **The consequence for this section is procedural: no step's pre‑declared claim (§12.5 fact 1) may name a loss from a document.** It names whatever `_lab.swapLedger()` reported as LOST or gate‑shaped OFF‑BY‑DEFAULT **on that run**, before Max looks. The pre‑declaration stays; its content comes from the machine. A caption written ahead of the ledger is the same defect as §6b's hand list, one instrument later.
...
5. **The swap ledger and the gating uniforms are printed beside the image**, and `labShaderReport().fps` is read **first** — a backgrounded window throttles rAF to ~1 Hz while `document.hidden` still reports false, and every per‑frame verdict then reads as "not wired" (src/main.js:2491 `const fps = Math.round(frames / ((performance.now() - t0) / 1000));`).
...
| **8b** | **ROI: rim‑annulus.** The limb, not the surface, on the ~3.5% of moons already reaching `Planet.js`. Indicative (a 1/√AU proxy, directional only): a 5.21 AU sub‑neptune‑class moon 1.80→3.50 and cream→blue; weathered palette ≤0.02/channel on 6 of 7; `icenessOf` 0 on 7 of 7 both sides. **Say that before he looks.** | Revert src/generation/MoonGenerator.js:278 `const pData = PlanetGenerator.generate(rng, 1.0, planetData.sunDirection, zones, planetType);` and re‑shoot — **two loads, one restored pose**, since that file has no `import.meta.hot`. | ⛔ The seed comes **from the committed delta table's top movers**, never from a guess — a random seed has no planet‑class mover and produces a green, pretty, meaningless pair. |
...
| **10** | **ROI: disc.** One hand‑picked plain moon, addressed **by name**, stops being `Moon.js`'s generic‑simplex ball; then every moon in the system at once. | **The per‑class quad**: {rocky planet, gas giant, plain moon, planet‑class moon} from one seed. Any two pixel‑identical ⇒ the degenerate failure this step must not ship — which no single screenshot can show. **Instrument D as PLAN.md:339 `Instrument D (build it here):` defines it stays primary** (`window.onerror` + `unhandledrejection`, ≥120 frames, rAF advanced) — ⛔ never `runIntegrationSuite`. | ⛔ **Intra‑step ordering:** the two unguarded frame‑loop writes land **before** the filter widening. src/main.js:9888 `mMat.uniforms.shadowPlanetPos.value.copy(entry.planet.mesh.position);` is unguarded and the sibling at src/main.js:9878 `if (pmu?.shadowPlanetCount) {` records this exact bug already stopping the render loop permanently **while the caller returned success**. Preview first, guard second = a frozen frame and an `ok: true`. |
