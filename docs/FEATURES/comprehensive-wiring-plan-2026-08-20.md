...
| **Route (iii) for every wiring extraction** — extract each law into a condition-shaped module both sides import | Only route with shipped precedent, three-way: `surfaceMaterial.js`, `bombardment.js` and `atmosphereOptics.js` are each imported by `world-engine-lab.html`, by `src/objects/Planet.js` AND by a driver pack. `docs/FEATURES/pack-authoring-path.md` documents it; `tests/lab-surface-ratchet.test.js` enforces it shrink-only. | **B3** |
...
docs/FEATURES/lab-features-not-yet-wired-2026-08-20.md:91 `| **F13** | Outflow channels | **(p)** ⭐ |` — world-engine-lab.html:2166 `state.outflowDensity` → **1 distinct / 100.0% zero** today.
...
`grep -rn` over `src/` — they are inline JS inside `world-engine-lab.html`, outside the module tree and
...
`state.featureRelevant.mountains`, recomputed at `world-engine-lab.html:1987-1988` as
