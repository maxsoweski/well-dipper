...
7. **Defaults under-read at build time** — verify routinely walks them up (F49 `ecuCanyonDepth` 0.45→0.70; F45 `shatSubAmt` 0.4→0.7). The verify agent edits `state.*` defaults in planet-lod-lab.html + widens any GUI slider whose good value sits at an edge.
