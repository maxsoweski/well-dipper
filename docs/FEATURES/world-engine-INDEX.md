...
5. **Composition-weighted greenhouse (τ), NEW 2026-08-14** — the adapter's grey-greenhouse (`src/worldengine/port/conditionFromBody.js:65` `export function surfaceTemperatureOf(T_eq, pressureBar) {`) computes `T_surf = T_eq·(1 + 0.75τ)^0.25` with `τ = 0.84·P^1.124`, and **τ is a function of PRESSURE ONLY**. Two bars of CO₂ and two bars of N₂-O₂ therefore receive an identical lift, which is physically wrong — CO₂ absorbs far more per bar. Composition is already in scope at this seam but is used only as a GATE (`compositionClass` decides *whether* a body gets a correction, i.e. the no-surface domain), never as a STRENGTH. ⚠ The fit itself is sound and should not be replaced casually: it is anchored on Earth (1 bar, 255→288 K) and Venus (92 bar, 232→737 K) and validated against four bodies that were NOT fitted (Mars +0.1%, Titan +3.7%, Moon 0%, Europa 0%) — so any composition term must reproduce those five. Noticed on `wd-614` planet 1 during break B7's UAT: 2.12 bar of N₂-O₂ lifting 254.77 K → 319.20 K (+64.42 K, 115 °F). ⛔ NOT a defect and NOT scoped — the current model is a deliberate, documented approximation. Max asked for it to be filed 2026-08-14; he has not asked for it to be built.
6. **Type → derived-label demotion** — long-horizon target: `_pickType` becomes a seed/label; palette/atmo/feature-set derive from drivers + fields (replace `rendersOn` allowlists with driver-threshold gates).
