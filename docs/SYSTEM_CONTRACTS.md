...
**Implementation shape:** camera / ship-mesh accepts an **additive shake input**. The shake magnitude is computed from motion discontinuity, not authored per-phase-transition. Three specifics:
...
- **Decel-impulse trigger phase is the `approaching → orbiting` phase transition (arrival moment), NOT the `traveling → approaching` boundary.** This is the implementation correlate of Bible §8H's *arrival-compensation* clause: the gravity drive brakes sharply at the destination, not halfway through transit, so the friction-against-ether event belongs to the ship's settle into STATION orbit — not to the start of its approach close-in. Accel-impulse trigger remains the `idle/descending → traveling` onset (ship begins accelerating from rest). The two impulses are thus anchored to **phase-boundary events**, each sign-discriminated by its own boundary: `+1 sign` (accel, crescendo-then-fade) at travel-onset; `-1 sign` (decel, impact-then-decay) at orbit-settle. Implementations detect the orbit-settle boundary by tracking the prior-frame `MotionFrame.phase` and firing on the frame where prior = `'approaching'` and current = `'orbiting'`; no new one-shot field on the navigation subsystem is required.
...
- **Shake magnitude is bounded relative to the body being framed, not expressed in scene-unit absolutes.** Sol orbit distances span a ~666× range from innermost moon (`orbitDistance ≈ 0.06`) to star (`orbitDistance ≈ 40`); a scene-unit-absolute amplitude that reads as a bounded bob at the star reads as a view-flipping ±80°+ lurch at a moon. The shake's peak amplitude is therefore computed as a bounded fraction of a per-target framing scalar — the implementing workstream picks which scalar (`orbitDistance`, `bodyRadius`, or a live angular-framing query), documents the pick, and holds the bound. The scale factor is **frozen at impulse onset** alongside the shake axis, the sign, and the onset time: a tour-leg transition mid-ringout does not re-scale an in-flight impulse train (each impulse is one event, not a continuously-retuned envelope). Scaling applies to the **whole envelope sequence** — peak and every subsequent impulse amplitude multiply by the same frozen scale factor — so the log-decay ratios between impulses (`a_n / a_0`) are preserved across body classes. Verification must be ratio-based: scene-unit magnitude alone (e.g., `shakeMag` in isolation) is not sufficient to evaluate this invariant; the verification scalar is `shakeMag / <frozen onset-scale>` bounded within the workstream's chosen fraction range across all body classes in a multi-body stress-test.
