...
// AC1 (rivers-dendritic-drainage): extracted verbatim from world-engine-lab.html
...
export const HEIGHT_GLSL = /* glsl */ `
      ...
      uniform vec3  uLightDir;  uniform vec3  uLightDir2;  uniform vec3  uStarColor1;  uniform vec3  uStarColor2;  uniform float uStarBrightness1;  uniform float uStarBrightness2; uniform vec3  uStarPos1;  uniform vec3  uStarPos2;  const int MAX_SHADOW_MOONS = 6;  uniform int uShadowMoonCount;  uniform vec3 uShadowMoonPos[6];  uniform float uShadowMoonRadius[6];  const int MAX_SHADOW_PLANETS = 2;  uniform int uShadowPlanetCount;  uniform vec3 uShadowPlanetPos[2];  uniform float uShadowPlanetRadius[2];  float sphereShadow(vec3 fragPos, vec3 starPosition, vec3 casterPos, float casterRadius) { vec3 toStar = starPosition - fragPos; float distToStar = length(toStar); vec3 rayDir = toStar / distToStar; vec3 oc = casterPos - fragPos; float tca = dot(oc, rayDir); if (tca < 0.0) return 1.0; if (tca > distToStar) return 1.0; float d2 = dot(oc, oc) - tca * tca; if (d2 >= casterRadius * casterRadius * 1.3) return 1.0; return smoothstep(casterRadius * 0.85, casterRadius * 1.15, sqrt(d2)); }  float totalShadow(vec3 fragPos, vec3 starPosition) { float shadow = 1.0; for (int i = 0; i < MAX_SHADOW_MOONS; i++) { if (i >= uShadowMoonCount) break; shadow *= sphereShadow(fragPos, starPosition, uShadowMoonPos[i], uShadowMoonRadius[i]); } for (int i = 0; i < MAX_SHADOW_PLANETS; i++) { if (i >= uShadowPlanetCount) break; shadow *= sphereShadow(fragPos, starPosition, uShadowPlanetPos[i], uShadowPlanetRadius[i]); } return shadow; }      // ⭐⭐ B4-2 - F52's ARITHMETIC, FORWARDED FROM THE GAME (ledger P-03). sphereShadow is src/objects/Planet.js:209-221 CHARACTER-FOR-CHARACTER once whitespace is normalized - the early-outs, the 1.3 slack factor on the radius-squared test and the 0.85/1.15 penumbra are the game's constants, not re-derived ones, and tests/lab-shader-shadows.test.js compares the two token streams rather than trusting this sentence. totalShadow is src/objects/Planet.js:224-241 with SIX names re-spelled (shadowMoonCount -> uShadowMoonCount, ...) and nothing else changed: same caps, same break-on-count loop shape, same multiply order moons-then-planets, which is what keeps the product's float rounding in the game's order rather than a re-associated one. ⛔ THE OPERANDS ARE OBJECT SPACE / uBodyRadius, NOT WORLD - see the note at src/worldengine/shaders/uniforms.js:137 for why the lab fragment cannot have a world-space position at all and why the similarity-invariance of this function is what makes the substitution legal. ⚠ DIVISION BY distToStar IS UNGUARDED, EXACTLY AS THE GAME LEAVES IT: a caster loop only runs when a count is non-zero, and a fragment on the body surface coincides with the star position only if the star is inside the body. Forwarding the law verbatim includes forwarding what it does NOT check. ⛔ RIDES THIS LINE by the handoff gate "every citation-bearing file N added / N deleted" (handoff-2026-08-21-B3-B4-parallel.md:89) - a NEW line here shifts symbol-anchored citations and reds them as some other block's failure. FRAGMENT BLOCK ONLY - the vertex shader reads none of them, which leaves LAB_VERTEX_SHADER's four reads of position at four.      // object-space light dir, then B4-1's star set (ledger P-01 / P-02). ⛔ RIDES THIS LINE by the handoff gate "every citation-bearing file N added / N deleted" (handoff-2026-08-21-B3-B4-parallel.md:89) - a NEW line here shifts symbol-anchored citations and reds them as some other block's failure. FRAGMENT BLOCK ONLY - the vertex shader reads none of them, which also leaves LAB_VERTEX_SHADER's four reads of position at four; a fifth makes deriveTapVertex THROW (tests/instrument-tap-fence.test.js).
      ...
      void ecuRelief(vec3 pos, inout float h, inout vec3 grad){
        ...
        if (cov <= 0.0) return;
        vec3 ddx = dFdx(pos), ddy = dFdy(pos); vec3 crC = cross(ddx, ddy); float sAC = dot(ddx, ddx) + dot(ddy, ddy); float dAC = sqrt(max(sAC * sAC - 4.0 * dot(crC, crC), 0.0)); float fwB = max(sqrt(max(0.5 * (sAC - dAC), 0.0)), sqrt(max(0.5 * (sAC + dAC), 0.0)) * 0.125);   // ⭐⭐ ANISOTROPIC FOOTPRINT, 2026-08-26 — THE THIRD AND LAST FORM OF THIS LINE, AND EACH EARLIER ONE IS KEPT IN THIS COMMENT BECAUSE EACH LOOKED RIGHT AND WAS MEASURED WRONG. (1) max(max(fwidth(p.x), fwidth(p.y)), fwidth(p.z)) — an L-INFINITY norm over OBJECT axes; a max() creases where the winning term changes and three axes crease a sphere into an octahedron. Max saw it and called it a diamond. (2) max(length(dFdx), length(dFdy)) — the OpenGL mip rho; still a max(), so still creased, just along SCREEN axes. (3) sqrt(length(cross(ddx, ddy))) — sqrt of the footprint AREA; genuinely isotropic, and Max confirmed the diamond became a CIRCLE. But an isotropic measure throws away the detail an ANISOTROPIC footprint still resolves, which is why he then said it was too aggressive and blended badly from the middle of the disc out toward the limb. ⭐ THIS FORM IS THE MINOR SINGULAR VALUE of the footprint Jacobian, capped at 8x anisotropy — what anisotropic texture filtering uses. sigma1*sigma2 = the parallelogram area and sigma1^2+sigma2^2 = the Frobenius norm, so both singular values fall out of one quadratic with NO max() in the smooth part; the only max() left is the 8x cap, which engages solely at grazing angles where the silhouette hides it. MEASURED by tools/across-disc-lod-probe.mjs on Europa at 4 radii: the area measure holds 3 octaves at the disc centre and drops to 2 by 0.85 of the disc — one visible RING — while the minor axis holds 3 octaves all the way out to 0.97. ⭐ CALIBRATION-PRESERVING, like the two before it: at the disc centre face-on the footprint is square, sigma1 == sigma2, and this returns exactly what all three earlier forms returned. Only the off-centre behaviour changes.
