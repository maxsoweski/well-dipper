...
export const HEIGHT_GLSL = /* glsl */ `
      ...
      void ecuRelief(vec3 pos, inout float h, inout vec3 grad){
        ...
        if (cov <= 0.0) return;
        float fwB  = sqrt(length(cross(dFdx(pos), dFdy(pos))));   // ⭐⭐ ISOTROPIC FOOTPRINT, 2026-08-26 — THE DIAMOND MAX SAW. This was max(max(fwidth(p.x), fwidth(p.y)), fwidth(p.z)); fwidth is abs(dFdx)+abs(dFdy), so that was an L-INFINITY norm over the three OBJECT-SPACE axes. A max() CREASES wherever the winning term changes, and three axes crease along planes that cut a sphere into an octahedron — a diamond, locked to object space, so it rides the planet as it turns. ⛔ max(length(dFdx), length(dFdy)) — the OpenGL mip-selection rho — IS NOT THE FIX AND WAS TRIED AND MEASURED: it is still a max(), so it still creases, just along SCREEN axes instead of object axes. MEASURED by tools/footprint-anisotropy-probe.mjs, swing around a constant-foreshortening ring at 0.9 of the disc: old form 41.7 percent, rho 29.9 percent, THIS FORM 0.1 percent (float noise). ⭐ sqrt of the parallelogram AREA is the side of the equivalent SQUARE footprint. Area is invariant under rotation of the screen axes and there is no max() left to crease, which is why it is the only one of the three that is flat around a ring it has no physical reason to vary around. ⭐ CALIBRATION-PRESERVING: at the DISC CENTRE face-on — the convention src/worldengine/port/craterUniforms.js states for its own floor — dFdx and dFdy are orthogonal and equal, so this returns exactly the old value and every threshold keeps its meaning. Only the off-centre anisotropy goes.
