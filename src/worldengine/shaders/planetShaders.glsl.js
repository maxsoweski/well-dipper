...
export const LAB_FRAGMENT_SHADER = /* glsl */ `
      ...
      void main(){
        ...
        float diff1 = max(dot(shadeN, uLightDir), 0.0);  float diff2 = max(dot(shadeN, uLightDir2), 0.0);  float shadow1 = totalShadow(vPos, uStarPos1);  float shadow2 = totalShadow(vPos, uStarPos2);  vec3 starLight = uStarColor1 * diff1 * uStarBrightness1 * shadow1 + uStarColor2 * diff2 * uStarBrightness2 * shadow2;  float diff = diff1 * uStarBrightness1 * shadow1 + diff2 * uStarBrightness2 * shadow2;   // ⭐⭐ B4-1 (ledger P-01 / P-02) — the GAME'S OWN LAW, forwarded from src/objects/Planet.js diff1/diff2/starLight/diffuse. ⛔ RIDES THIS LINE by the handoff gate "every citation-bearing file N added / N deleted" (handoff-2026-08-21-B3-B4-parallel.md:89) - a NEW line here shifts symbol-anchored citations and reds them as some other block's failure. ⭐⭐ diff STAYS A FLOAT ON PURPOSE. It is read as a GATE a dozen times below (the nightMask for bio/city/ecumenopolis, step(0.0001, diff) for carbon sheen / facets / sunglint, the lit dayside gate) and as a MAGNITUDE in rayField, machine metal and the F34 limb's (diff + 0.15). Widening it to vec3 would silently re-colour the limb and every night gate - a different block's pixels. The game solves this the same way and for the same reason: a coloured starLight AND a scalar diffuse, side by side. ⛔⛔ THE SHADOW SENTENCE THAT USED TO BE HERE IS NOW FALSE AND IS REPLACED, NOT LEFT STANDING. B4-1 wrote "NO SHADOW FACTORS HERE ... every shadow term is therefore 1.0 and DROPS OUT"; B4-2 put the factors in. ⭐ THE SHADOW HALF (ledger P-03): shadow1 and shadow2 are the game's own src/objects/Planet.js:484-489 expression, with vPos where the game reads vWorldPos and uStarPos1/2 where it reads starPos1/2 - the lab fragment has NO world-space varying and provably cannot be given one (a fifth read of bare position in the vertex shader makes deriveTapVertex throw), so the casters are transformed INTO this fragment's object space at the seam instead. sphereShadow is similarity-invariant, which is what makes that legal; the measurement behind that word is at src/worldengine/shaders/uniforms.js:137. ⚠ INVARIANT UP TO FLOAT ROUNDING, NOT BIT-IDENTICAL - nothing here claims the lab and the game evaluate this to the same bits. ⭐ STILL THE PRE-B4-2 LINE AT THE DEFAULTS: uShadowMoonCount and uShadowPlanetCount default to 0, both of totalShadow's loops break on the first iteration, both factors are exactly 1.0, and the expression is arithmetically the one B4-1 left. ⭐ AT THE FACTORY DEFAULTS THIS IS ARITHMETICALLY THE LINE IT REPLACES: uStarBrightness1 is 1.0 (every primary StarSystemGenerator draws is a literal 1.0), uLightDir2 is the zero vector so diff2 is 0, and diff reduces to diff1. On a BINARY the scalar gains the second star's term - which is the game's law, not a lab liberty.
        float ambient = 0.035;
