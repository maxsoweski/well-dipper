...
export function makeUniforms(WORLD_LIGHT) {
  return {
      ...
      uLightDir:     { value: new THREE.Vector3().copy(WORLD_LIGHT) }, uLightDir2: { value: new THREE.Vector3(0, 0, 0) }, uStarColor1: { value: new THREE.Color(1, 1, 1) }, uStarColor2: { value: new THREE.Color(0, 0, 0) }, uStarBrightness1: { value: 1.0 }, uStarBrightness2: { value: 0.0 }, uStarPos1: { value: new THREE.Vector3(0, 0, 0) }, uStarPos2: { value: new THREE.Vector3(0, 0, 0) }, uShadowMoonCount: { value: 0 }, uShadowMoonPos: { value: [new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3()] }, uShadowMoonRadius: { value: [0, 0, 0, 0, 0, 0] }, uShadowPlanetCount: { value: 0 }, uShadowPlanetPos: { value: [new THREE.Vector3(), new THREE.Vector3()] }, uShadowPlanetRadius: { value: [0, 0] },   // ⭐⭐ B4-2 — F52 TRANSIT + PLANET SHADOWS (parity-ledger P-03), and THE SPACE IS THE WHOLE DESIGN. These eight are the 1:1 counterparts of the game's starPos1/starPos2/shadowMoonCount/Pos/Radius/shadowPlanetCount/Pos/Radius (declared src/objects/Planet.js:107-116) - but they are OBJECT space divided by uBodyRadius, where the game's are WORLD space. ⛔ THE LAB FRAGMENT SHADER HAS NO WORLD-SPACE VARYING AND CANNOT BE GIVEN ONE. The game's shadow signature is sphereShadow(vWorldPos, starPos1, casterPos, casterRadius) - four world-space vectors. Adding a vWorldPos varying needs a FIFTH read of bare position in LAB_VERTEX_SHADER, and tests/instrument-tap-fence.test.js makes deriveTapVertex THROW on exactly that, killing the AC-SAMPLER instrument. So the CASTERS move into the fragment's frame instead of the fragment moving into theirs. ⭐ THAT SUBSTITUTION IS LEGITIMATE BECAUSE sphereShadow IS SIMILARITY-INVARIANT, AND THAT WAS MEASURED, NOT ASSUMED: transforming fragPos, starPosition and casterPos by one rotation+translation+uniform-scale and scaling casterRadius by the same scale moved the returned factor by at most 2.5e-12 absolute / 1.5e-11 relative over 200,000 random float64 samples, 3,043 of them inside the smoothstep penumbra (the only branch that computes). ⚠ THAT IS FLOAT ROUNDING, NOT ZERO - this is an equality up to rounding, and in float32 GLSL the rounding is larger. Nothing here claims bit-identity with the game's world-space evaluation. ⚠ UNIFORM SCALE IS AN ASSUMPTION: a non-uniformly-scaled body mesh would break the invariance, and the seam reads ONE component of getWorldScale. ⛔ THE DEFAULTS ARE THE PRE-B4-2 IDENTITY: both counts are 0, so totalShadow's two loops break immediately and return 1.0, every shadow factor drops out of the lit expression, and no star position is ever read. Only a body whose seam is fed a real caster list moves a pixel. ⛔ RIDES THIS LINE by the handoff gate "every citation-bearing file N added / N deleted" (handoff-2026-08-21-B3-B4-parallel.md:89) - a NEW line here shifts symbol-anchored citations and reds them as some other block's failure.   // ⭐ B4-1 — STAR COLOUR + THE SECOND STAR (parity-ledger P-01 / P-02). ⛔ RIDES THIS LINE by the handoff gate "every citation-bearing file N added / N deleted" (handoff-2026-08-21-B3-B4-parallel.md:89) - a NEW line here shifts symbol-anchored citations and reds them as some other block's failure. ⭐ THE DEFAULTS ARE THE PRE-B4 IDENTITY, and that is the whole safety story: white × 1.0 is exactly the implicit light the lab shader already rendered under, and a ZERO uLightDir2 makes dot() zero, so the fragment's new `starLight` collapses to vec3(diff) and its scalar `diff` collapses to the single expression it replaces. Only a body BUILT with a real starInfo moves a pixel. ⚠ CARRIED AT CONSTRUCTION, NOT AT THE SEAM: star colour has no per-frame writer anywhere in the engine — it is written once, from StarSystemGenerator.js:484-489 into the Planet/Moon constructors — so there is nothing per-frame to widen. uLightDir2 is the opposite and IS re-copied every sim tick, so the seam re-reads it.
