...
// from src/main.js:7764 `const planetMoon = new Planet(scenePMData, pmStarInfo);` and not through
...
export const PACKS = Object.freeze([
  ...
  GIANT_SURFACE_ENTRY, FLUVIAL_DECK_ENTRY, STORM_DECK_ENTRY, SOLID_RELIEF_ENTRY,   // ⭐⭐ solidRelief (pack #11, 2026-09-04) RIDES THIS LINE TOO, appended after stormDeck for the same citation reason. ⛔ IT MOVES NO BODY BETWEEN MATERIALS: its predicate is `compositionClass(condition) !== 'gas'`, character-identical to rockySurface's, solidOptics', solidFeatures' and fluvialDeck's, all four of which already claim every body it claims, so the `packs.length > 0` admission term cannot flip and no census is re-pinned. ⛔ AND IT COLLIDES WITH NOTHING: all twenty-three names it writes (the F1/F4/F5/F6/F8/F15/F16/F18/F19/F21 families — uMountainAmp, uOrogenyStrength, uOrogenyAxis, uChasmaDepth, uChasmaCount, uChasmaAxis, uScarpStrength, uScarpStyle, uScarpAxis, uPlateauStrength, uTesseraStrength, uTesseraAxis, uLavaCoverage, uLavaActivity, uLavaAxis, uSubStrength, uKarstDensity, uKarstMaturity, uDuneDensity, uDustDepth, uMassWastDensity, uRepose, uLdaFat) were written by NO pack before this commit — MEASURED at the parent in tests/fixtures/solidrelief-pack-drivers-baseline.json, whose `newNamesAlreadyWritten` is empty over 156 corpus bodies x 18 presets, and re-asserted by name lookup over real pack outputs in tests/driver-pack-solidrelief.test.js. ⚠ IT DECLARES ELEVEN GATES, more than any other entry, because the lab has eleven independent checkboxes over these features and they do not switch together; `gatesFor` answers each `GATE_RULINGS[g] ?? true`, so all eleven default ON and none is ruled off today. ⭐ stormDeck (pack #10, 2026-09-03) RIDES THIS LINE TOO, appended after fluvialDeck for the same citation reason; its predicate is character-identical to giantDeck/limbDeck/polarDeck so it moves no body between materials, and it writes five names (uStormPosSize, uStormParams, uStormColor, uStormAux, uStormCount) no pack wrote before — asserted by name lookup over real pack outputs in tests/driver-pack-stormdeck.test.js. // ⭐ fluvialDeck RIDES THIS LINE, APPENDED NOT INSERTED — this file is line-cited (limbDeck.js:173 and fourteen others point INTO it by line), so a new line here reds fifteen citations as some other block's failure. The same discipline the two-import line 59 keeps. ⛔ IT MOVES NO BODY BETWEEN MATERIALS: its predicate is character-identical to rockySurface's, solidOptics' and solidFeatures', all three of which already claim every body it claims, so the `packs.length > 0` admission term cannot flip and no census is re-pinned. ⛔ AND IT COLLIDES WITH NOTHING: all ten names it writes (uFluvialActivity, uFluvialDepth, uFluvialMeander, uSeaLevel, uLiquidMask, uDeltaDensity, uCoastStrength, uStrandStrength, uOutflowDensity, uOutflowActivity) were written by NO pack before this commit — asserted by name lookup over real pack outputs across a generated corpus in tests/driver-pack-fluvialdeck.test.js, not by reading `applies` lines side by side. ⚠ IT IS PACK #9, NOT #8, AND THE NUMBER WAS CORRECTED BEFORE REVIEW RATHER THAN LEFT AMBIGUOUS: `giantSurface` above already holds `PACK #8` here and at giantSurface.js:3, so the first draft of this entry shipped a colliding label. Renumbering giantSurface would rot its symbol-anchored citations; renumbering OURS costs nothing, so ours moved. Every `#9` in fluvialDeck.js, in this line and in world-engine-lab.html's neutralised block is this pack; every `#8` anywhere is giantSurface's.
]);
