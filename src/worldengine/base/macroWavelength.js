...
const U_IO = calibrateTidal(1);   export const MAX_FINER_THAN_IO = 1.5;   const R_IO_RATIO = K_MACRO_R_IO / K_MACRO_R;   const SHORT_FLOOR = R_IO_RATIO / MAX_FINER_THAN_IO;   const SHORT_C = (1 - SHORT_FLOOR) / (R_IO_RATIO - SHORT_FLOOR) - 1;   // ⭐⭐ THE EXTRAPOLATION BOUND, 2026-08-26, AND IT EXISTS BECAUSE MAX'S EYE SUPPLIED A CRITERION THIS FILE DID NOT HAVE. Flying the lab at game parity he said the terrain "seems to delete any sense of scale because it's so fine". MEASURED at the old unbounded ceiling: lambda/R = 0.0133, i.e. SEVENTY-FIVE macro structures across a body radius, and 5.1x finer than Io — the FINEST body in this file's own calibration table. On Lava that made the LARGEST terrain feature 27 km on a 2032 km body. That is sandpaper, not geography, and no amount of LOD filtering rescues it because nothing downstream can make a feature bigger than the law asks for. ⛔ THE OLD SENTENCE 'Above Io this law IS extrapolation. It is bounded, it is monotone, and it is declared here' WAS TRUE ABOUT MONOTONICITY AND FALSE ABOUT BOUNDEDNESS — the hyperbolic form tends to ZERO, so lambda/R had no floor at all. ⭐ THIS FORM ADDS THE FLOOR WITHOUT LOSING WHAT THE OLD ONE PROTECTED. It is still exact at BOTH anchors (1 at u=0, K_MACRO_R_IO/K_MACRO_R at u=1) and still strictly monotone, so the 67 hottest plain moons keep 67 distinct values — it ASYMPTOTES to the floor rather than clamping onto it, which is the whole reason a clamp was refused here. ⚠ MAX_FINER_THAN_IO IS THE ONE NUMBER TO TURN and it is a LOOK call, not a physics one: it says how far past the finest REAL body the law may extrapolate. 1.5 is the deliberate default — it keeps some room past Io without leaving the neighbourhood of the calibration set. Raise it for finer hot worlds, lower it toward 1.0 to forbid extrapolation entirely.
