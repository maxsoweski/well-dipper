...
export function deriveUniforms(drivers, qualityTier = 1.0) {
  ...
  const frostPermanence = frostCondensationT > 0 ? clamp01(1.0 - smoothstep(0, 60, T - frostCondensationT)) : 0;   const frostMaxCoverage = clamp01(smoothstep(0.05, 0.4, volatileFraction)) * frostPermanence;   const frostLatChill = 0.60 / (1.0 + Math.pow(Math.max(pressure, 0) / 0.015, 0.225));   // ⭐ TWO LAWS RIDE THIS LINE, AND THEY RIDE IT ON PURPOSE: thirteen citations point INTO this file below here (labCore.js:985/:996/:999/:1009/:1045/:1064/:1108/:1124/:1140/:1177/:1200/:1227/:1329), so the edit is LINE-COUNT-NEUTRAL — the :1124 import convention, applied to declarations. (1) frostPermanence — THE BUDGET'S MISSING TEMPERATURE TERM. Before this, the budget was purely how much water a world has: MEASURED identical at 0.802 from 100 K to 1200 K at the same volatileFraction, so a 700 K world with volatiles scored the same budget as a 200 K one and was saved only by the shader's localT test. Permanent ice needs the volatile to STAY solid, so the budget falls as the world warms past its own species' freeze point, over 60 K (water ⇒ gone by 333 K). ⛔ IT IS EXACTLY 1 AT AND BELOW frostCondensationT, WHICH IS THE WHOLE OF THE COLD-WORLD REGRESSION ARGUMENT: every body at or under its freeze point multiplies by 1 and is byte-identical. MEASURED over 200 seeds: of 118 bodies more than 10 K below their freeze point, ZERO move; only the 12 within 10 K of the line move, which is the marginal class, not the frozen one. ⚠ 60 K IS A WIDTH, NOT A THRESHOLD — ask which way it moves as T falls (last session's rule): it CLOSES toward 1, it does not open. (2) frostLatChill — the equator→pole temperature falloff, DERIVED, replacing the flat 0.35 that shaders/uniforms.js:268 labelled "lab knob". A thick atmosphere redistributes heat and flattens the gradient; a bare rock cannot. Three-point fit to real bodies: airless Moon (pole/equator ≈ 0.41 ⇒ chill 0.60), Mars at 0.006 bar (≈ 0.33), Earth at 1 bar (250 K pole / 300 K equator ⇒ 0.168). Venus at 90 bar lands 0.074 against a near-isothermal reality — an overestimate, and harmless, because 90 bar worlds are centuries above any freeze point. ⛔ THE FLAT 0.35 PUT A 293 K WORLD'S SEA-LEVEL SNOWLINE AT 26° LATITUDE, which is the defect Max walked into: 56% of the surface inside the snowline on a room-temperature ocean world.
  ...
  return {
    ...
    tempEq: T, frostLatChill, frostPermanence,                 // Cryo step 2 — T_eq passthrough for the shader localT field (uPlanetTempEq). ⭐ frostLatChill AND frostPermanence RIDE THIS LINE for the same line-count reason as their derivation above. frostLatChill is now DRIVEN per body (uFrostLatChill, ex-lab-knob); frostPermanence is _derived-only — it is already folded into frostMaxCoverage and is returned so a test can tell a budget of 0 that is BONE-DRY from one that is TOO WARM, which the folded product cannot.
