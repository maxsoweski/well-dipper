...
import { makeUniforms } from '../worldengine/shaders/uniforms.js'; import { POSTERIZE_LEVELS } from './posterizeLevels.js'; export const TERM_BYPASS = false; export const LIMB_BYPASS = false;   // ⭐ 2026-09-03 (F35) THE TWO BYPASS DEFAULTS RIDE THIS ROW, beside the one display-policy value this file already owns. They are DISPLAY POLICY, not pack drivers: `uTermBypass` / `uLimbBypass` let the band and the rim skip the posterize quantizer, and no per-body law produces them — a knob with no per-body producer is not a law. Their one declared value lives here, the LAB imports both back into its state literal (world-engine-lab.html:1143-1144) and its per-frame writer stays a relay from state, so the constant is the single SOURCE and the writer is not a second author. ⛔ `false` IS THE LAB'S OWN DEFAULT and both uniforms already default to 0 (uniforms.js:43, :51), so the write below is byte-inert on both front-ends today. ⛔ RIDES THIS LINE: this file carries symbol-anchored citations far down it and a new import row shifts every one of them. ⚠ THE LINE NUMBER IS DELIBERATELY NOT WRITTEN HERE — as a bare colon-plus-number it is parsed by the citation fence as a reference to `uniforms.js`, the file this same line imports from, and reported PAST EOF. The identical trap is recorded at this file's last line. // ⛔ B2P RIDES THIS LINE: this file is line-cited, so a new import line moves every ref below it.
...
export function buildLabPlanetMaterial(opts = {}) {
  ...
  const uniforms = makeUniforms(light); uniforms.uLevels = POSTERIZE_LEVELS; uniforms.uTermBypass.value = (opts.termBypass ?? TERM_BYPASS) ? 1 : 0; uniforms.uLimbBypass.value = (opts.limbBypass ?? LIMB_BYPASS) ? 1 : 0;   // ⭐ 2026-09-03 (F35 AC-0) THE BYPASS PAIR RIDES THIS LINE, for the same reason `uLevels` does: this is the file's display-policy seam, the one place a value with no per-body law is allowed to reach a material. It gives the GAME material its declared value where it had none. ⚠ RECORDED, NOT CLAIMED AWAY: `uTermBypass`'s only consumer (planetShaders.glsl.js:958) sits INSIDE the `uTermStrength > 0.0` block at :953, so under the 2026-07-16 ruling it cannot move a game pixel — F35's 4/4 records a wired PRODUCER, not a rendering path, and it becomes reachable with no further change the day Max rules the band back on. ⛔ `opts.x ?? CONST`, not `opts.x || CONST`: `false` is a meaningful value and a parameter seam that could not carry it would leave the constant untestable (the 'flip the const in a test copy' trap, handoff-2026-09-03b §2 trap 4). // ⭐ B2P — SUBSTITUTE the shared object for makeUniforms' private one. Nothing here edits the lab shader or the uniforms.js default (still 6.0); this only makes the value REACHABLE. Once the lab flag flips, 846 planets and 632 moons render through THIS program, whose uLevels no pack writes — leave this out and the setting evaporates exactly when the world engine becomes visible.
