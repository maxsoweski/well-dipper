...
const DEFAULTS = {
  // Visual
  renderLines: 240, skyRenderLines: 240, pixelScale: 3, skyPixelScale: 1, posterizeLevels: 31,   // ⭐ renderLines/skyRenderLines SUPERSEDE pixelScale/skyPixelScale, 2026-09-05: resolution is a LINE COUNT now, because the era's fixed quantity was the scanline count and a divisor made vertical resolution a function of window size (renderLines.js). ⛔ THE LEGACY KEYS STAY AND ARE NOT DELETED — they are the migration's input (a stored value <= 16 is a divisor, > 16 is lines, unambiguous by magnitude) and Max's stored settings are read, never reset. His 4.5 on a 1130-tall window is 251 lines, which snaps to the 240 he already said he prefers.   // ⭐ B2P: the colour quantum every body/ring/moon/asteroid shader quantises to. ⭐ 31 IS MAX'S RULING, 2026-08-21, AND IT IS A SOURCED NUMBER RATHER THAN A TASTE PICK: `levels = N` yields N+1 values per channel, so 31 gives exactly 32 = 5 bits, which is the framebuffer depth of the N64, the PlayStation AND the Saturn alike (RGB555 / RGBA5551). Max asked for "that era's aesthetic" and this is that era's number. ⛔ IT WAS 6 UNTIL NOW (2.81 bits) — 4.57x fewer values per channel than any of those machines and 95.5x fewer RGB triples, i.e. the old default was a sub-3-bit look, not a fifth-generation one. An absent key falls through the DEFAULTS merge; a STORED 6 from before this change is preserved as-is, which is the flightControlType precedent below in its own words.
  starDensity: 18000,
