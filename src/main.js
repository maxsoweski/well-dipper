...
retroRenderer.renderLines = migrateToRenderLines(settings.get('renderLines'), settings.get('pixelScale'), window.innerHeight); retroRenderer.skyRenderLines = migrateToRenderLines(settings.get('skyRenderLines'), settings.get('skyPixelScale'), window.innerHeight); settings.set('renderLines', retroRenderer.renderLines); settings.set('skyRenderLines', retroRenderer.skyRenderLines); retroRenderer.resize();   // ⭐ 2026-09-05: RESOLUTION IS A LINE COUNT NOW, and resize() is what derives the magnification and pushes it to BOTH shared uniforms — so the two setDither/setSky calls that used to be here are gone, not dropped (renderLines.js, RetroRenderer.resize). ⛔ THE settings.set CALLS ARE THE MIGRATION LANDING: they persist the converted value under the new key so the conversion happens ONCE, while leaving the legacy pixelScale/skyPixelScale keys in storage untouched. Max's settings are read, never reset — his 4.5 becomes 240p, the mode he said he prefers. // ⭐⭐ THE 2026-07-30 FIX ABOVE WAS ONLY HALF-APPLIED, AND THE OTHER HALF IS THIS CALL. Assigning the field restores the SETTING; it does not restore the PICTURE. `pixelScale` is not read anywhere per-frame — it is consumed once, in `resize()` (RetroRenderer.js:811), to ALLOCATE the render targets — so without a resize the constructor's hard-coded 3 stays on the GPU and the comment above describes a bug that was still live: stored 5, field 5, sceneTarget still 735x377 at 2205 wide, measured in the running game 2026-09-06. It only self-corrected on the next window resize, which is why it read as fixed. ⛔ THIS MATTERS RIGHT NOW, not academically: Max is choosing a render resolution by eye, and the defect makes the game boot showing a DIFFERENT resolution from the one the slider claims — i.e. it silently invalidates the very judgement the slider exists to support. ⛔ RIDES THIS LINE: main.js carries ~700 line-anchored citations.
...
window._lab = {
  ...
  async previewPack(args = {}) {
    ...
    else if (entry) { gates = drivers.gatesFor(entry, drivers.GATE_POLICY_RULED, drivers.GAME_GATE_RULINGS); gatesSource = `gatesFor(PACKS['${entry.name}'], RULED) — Max ruling 4 (ALL_ON) minus the names he has since ruled on individually: ${JSON.stringify(drivers.GAME_GATE_RULINGS)}`; }   // ⭐ 2026-09-04 GAME_GATE_RULINGS, not GATE_RULINGS: this preview reports what the GAME writes, and since the development gates landed the two maps differ — GATE_RULINGS is the LAB's (convergence only). Printing the lab's map here would have made the provenance string quietly describe the wrong front-end.   // ⭐ 2026-09-03 RIDES THIS LINE — the preview now names the policy the GAME actually runs (drivers/index.js:404 takes gatesFor's default, which is RULED), and prints the rulings map so the provenance string says WHICH decisions moved rather than just naming a policy. ALL_ON is still reachable and still means all-on; asking for it here would have made this string a report about a policy no writer uses.
...
// ── Boot flow: black ORRERY/HELM chooser → intro logos → title → music → warp ──
// (boot-flow corrected 2026-06-27.) The cold-open is the BLACK ORRERY/HELM chooser
// (the repurposed #splash-screen, splashActive=true). Picking a station records it
// (_pendingBootMode) then runs the ORIGINAL begin flow UNCHANGED: startIntroSequence
...
// Wire the two BLACK chooser buttons (ORRERY / HELM). Each picks its station then
// launches via _pickBootMode → startIntroSequence (the original intro → title →
// music → warp). stopPropagation so the click doesn't also hit the splash-background
// "press anything" dismiss (which would default to ORRERY).
{
...
function populateSettingsUI() {
  ...
  el.querySelectorAll('[data-setting]').forEach(input => {
    ...
    if (input.type === 'checkbox') {
      ...
      // ⭐ SELECTS GET THEIR READOUT TOO, 2026-09-05. This branch used to return without touching the
      // sibling label, which was invisible while every select was a bare enum. It stopped being
      // harmless the moment Resolution became a select: its whole job is showing the derived buffer
      // ("240p · 468×240"), and Max asked for exactly that — "I do want to know the numbers".
      const selLabel = input.nextElementSibling;
      if (selLabel?.classList.contains('setting-value')) selLabel.textContent = formatSettingValue(key, value);
    } else {
...
function applySettingChange(key, value) {
  switch (key) {
    ...
    case 'skyRenderLines':
      ...
      // size compensation) AND resized (bgTarget), because doing one without the other gave either
      // 3x-bigger stars — the bug Max rejected — or a coarser buffer nothing drew smaller for. resize()
      // now does both, from one derivation, so they cannot come apart (RetroRenderer.resize).
      retroRenderer.skyRenderLines = clampRenderLines(value);
      retroRenderer.resize();
      break;
...
function simStep(deltaTime) {
  ...
  if (system) {
    ...
    for (let i = 0; i < system.planets.length; i++) {
      ...
      const pu = pMat.uniforms;   { const sc = entry.planet._shadowCast; if (sc) { sc.starPos1.copy(_star1Pos); sc.starPos2.copy(_star2Pos); sc.moonCount = Math.min(entry.moons.length, 6); for (let m = 0; m < sc.moonCount; m++) { sc.moonPos[m].copy(entry.moons[m].mesh.position); sc.moonRadius[m] = entry.moons[m].data.radius; } let sIdx = 0; if (i > 0) { sc.planetPos[sIdx].copy(system.planets[i - 1].planet.mesh.position); sc.planetRadius[sIdx] = system.planets[i - 1].planet.data.radius; sIdx++; } if (i < system.planets.length - 1 && sIdx < 2) { sc.planetPos[sIdx].copy(system.planets[i + 1].planet.mesh.position); sc.planetRadius[sIdx] = system.planets[i + 1].planet.data.radius; sIdx++; } sc.planetCount = sIdx; } }   // ⭐⭐ B4-2 (ledger P-03) — THE SAME CASTER SELECTION, WRITTEN INTO THE BODY INSTEAD OF INTO A UNIFORM BAG. ⛔ THIS IS A SECOND COPY OF THE LAW TWENTY LINES BELOW, AND THAT IS A DELIBERATE TRADE, NOT AN OVERSIGHT. The obvious de-duplication is to make the guarded block below read from sc — but tools/port-uniform-delta.mjs pins SIX of those lines by symbol as its UNWATCHED evidence, two of them (:11392, :11393) quoting the whole assignment expression including its right-hand side, so rewriting them to read from sc would red the citation fence and the failure would read as some other block's. Duplicating the law and FENCING THE AGREEMENT is the cheaper risk: tests/lab-shader-shadows.test.js runs one system through both paths and asserts the record and the game bag select the same casters, so a future edit to either copy alone goes red. ⛔ UNGUARDED ON PURPOSE, and it is not the hazard the guards below defend against: _shadowCast is a plain field the Planet constructor always creates, not a uniform that may be missing, so there is no material on which this can throw. The if (sc) is for a test double or an older cached Planet, not for a material. ⚠ AND THAT GUARD ONCE HID A REAL BUG IN THIS VERY BLOCK: the constructor line was first written INSIDE a trailing comment, so _shadowCast was never created, every body took the if (sc) false branch, and the whole feature was a silent no-op that reddened nothing. Only a source-text assertion caught it. ⭐ WHY HERE AND NOT IN THE GUARDED BLOCK: a lab material declares none of the game's shadow names, so every guard below is FALSE on it and the whole feature leaves silently. That is the exact shape of the loss P-03 recorded. ⛔ RIDES THIS LINE — a new line here shifts every symbol-anchored citation below it in this file, including the six this block is cited by.
      ...
      for (const moon of entry.moons) {   { const msc = moon.isPlanetMoon ? moon.planet?._shadowCast : moon._shadowCast; if (msc) { msc.starPos1.copy(_star1Pos); msc.starPos2.copy(_star2Pos); msc.moonCount = 0; msc.planetPos[0].copy(entry.planet.mesh.position); msc.planetRadius[0] = entry.planet.data.radius; msc.planetCount = 1; } }   // ⭐⭐ B4-2 (ledger P-03, moon half) — ONE FILL SITE FOR BOTH ARMS, above the branch rather than inside either of them. A moon's only eclipse caster is its parent planet, in both arms, so the branch below is about WHICH UNIFORM BAG to write and not about which casters exist — putting the record fill above the branch is what stops those two questions from being conflated a second time. ⛔ DELIBERATELY OUTSIDE THE MOON-SHADOW-WRITE SENTINEL PAIR. tests/moon-shadow-write-guard.test.js slices the text between those two markers and COMPILES IT as a standalone function; a statement in there referring to moon would be evaluated against that harness's scope rather than this one, so the fence would start testing a different program than the one that ships. ⭐ moonCount IS SET TO 0 EXPLICITLY rather than left alone: these records are reused frame after frame and a moon that somehow acquired a moon-caster count would otherwise keep casting it forever. ⚠ moon.planet?. on the planet-class arm because that arm's own note twenty lines below records that a planet-class moon's material is swapped in production; the Planet wrapper is the thing being read here, and the optional chain states that this code does not assume it. ⛔ RIDES THIS LINE — a new line here shifts every symbol-anchored citation below it in this file.
        if (moon.isPlanetMoon) {
