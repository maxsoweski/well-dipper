...
export class Planet {
  ...
  */
  ...
  dispose() {
    ...
    this.surface.material.dispose(); disposeLabBake(this.surface); unregisterStormAB(this.surface); unregisterRaysAB(this.surface);   // ⭐ 2026-09-01 — releases the four cube render targets and the ribbon child this body owns (idempotent; a legacy-material body has no record and this is a no-op). RIDES THIS LINE.   // ⭐ 2026-09-03 — the ray A/B's registry is released on the same line for the same reason (idempotent; a legacy-material body has no record).
    if (this.ring) {
