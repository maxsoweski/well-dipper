...
describe('3. channel 1 — the uniform diff, run not read', () => {
  ...
  it('the losses with NO lab mechanism really have none — measured over the lab material', () => {
    ...
    expect([...labNames].filter((n) => /shadow/i.test(n)).sort()).toEqual(['uShadowMoonCount', 'uShadowMoonPos', 'uShadowMoonRadius', 'uShadowPlanetCount', 'uShadowPlanetPos', 'uShadowPlanetRadius']);
    // ⚠ SIX, NOT EIGHT — and the two that are missing from this list are missing because of the FILTER, not because of the port. P-03's subject list also carries starPos1 and starPos2, whose lab counterparts are uStarPos1/uStarPos2; those match /star/i and are asserted on the line below rather than here. Stated because a reader counting this list against the ledger row will otherwise find six where the row says eight and have to re-derive which is wrong.
    expect([...labNames].filter((n) => /^uStarPos/.test(n)).sort()).toEqual(['uStarPos1', 'uStarPos2']);
