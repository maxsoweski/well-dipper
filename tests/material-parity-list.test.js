...
describe('2. the collapse in per-body variation', () => {
  it('45 of the game material’s 73 uniforms vary across bodies; 52 of the lab’s 369 do', () => {
    ...
    expect(LEDGER.labVarying).toEqual([   // ⭐⭐ AND EIGHT LEFT ON 2026-09-04 — uMountainAmp, uChasmaDepth, uScarpStrength, uPlateauStrength, uTesseraStrength, uLavaCoverage, uSubStrength, uDustDepth — INTO THE WRITTEN-AND-CONSTANT RESIDUE BELOW, AND THAT IS A DECLARED RULING, NOT A DYING WIRE. Max 2026-09-04 ruled the bar "grown from the engine, not painted on": these eight read neither the accumulated height nor its gradient, so they are gated OFF IN THE GAME until FOLLOWUP (b) lands (workstream world-engine-feature-gates). ⛔ THIS LEDGER MOUNTS THE LAB MATERIAL ON A GAME BODY through applyDriverPacks, i.e. the GAME's composition point, so it sees the GAME's map (GAME_GATE_RULINGS) and these eight resolve to a constant 0. ⭐ THE LAB FRONT-END IS UNAFFECTED and still varies all eight: world-engine-lab.html composes through applyDriverPacksToState, which takes the CONVERGENCE map (GATE_RULINGS) and does not carry the development gates — asserted in tests/world-engine-feature-gates.test.js AC-3. ⚠ How to tell this from the dead wire this residue normally means: flip one boolean in solidRelief.js SOLID_RELIEF_GAME_GATES and the name comes straight back, which that suite's AC-5 demonstrates.   // ⭐⭐ AND TWENTY-THREE MORE ON 2026-09-04, `solidRelief` (pack #11, workstream solid-relief-deck) — the whole F1/F4/F5/F6/F8/F15/F16/F18/F19/F21 relief deck. ⛔ ALL TWENTY-THREE LAND HERE AND NOT ONE IN THE WRITTEN-AND-CONSTANT RESIDUE BELOW, WHICH IS THIS INSTRUMENT'S VERDICT ON THE WIRE: a forwarded gate whose input is dead shows up in the residue and nowhere else (that is how fluvialDeck's two-spelling erosion defect was caught), and none of these did. Six of them are the seeded relief AXES, which vary only because src/objects/Planet.js `labPackCtx` answers them from the game's own macroSeed — read off the condition bundle instead they would be ONE value on every body in the galaxy and would sit in the residue as six dead wires.
    ...
    expect(LEDGER.written.filter((n) => !LEDGER.labVarying.includes(n)))
      ...
      .toEqual(['uChasmaDepth', 'uDuneDensity', 'uDustDepth', 'uEjectaLump', 'uKarstDensity', 'uLavaCoverage', 'uMountainAmp', 'uPlateauStrength', 'uRayBrightness', 'uRayCount', 'uRaySharp', 'uScarpStrength', 'uSeaLevel', 'uSubStrength', 'uTermStrength', 'uTerraceCount', 'uTesseraStrength']);   // ⛔⛔ EIGHT OF THESE FIFTEEN ARE HERE FOR THE OPPOSITE REASON TO THE ONE THIS LIST DOCUMENTS, AND READING THEM AS DEAD WIRES WOULD BE EXACTLY BACKWARDS. uMountainAmp, uChasmaDepth, uScarpStrength, uPlateauStrength, uTesseraStrength, uLavaCoverage, uSubStrength and uDustDepth joined on 2026-09-04 by a FIFTH route this list had not seen before: A DECLARED GAME GATE. Their laws are live, their inputs vary, and they still reach 103/124/122/124/46/103/40/68 bodies in the LAB — they read a constant 0 here only because the GAME's gate map resolves them to zero, on Max's bar "grown from the engine, not painted on" (workstream world-engine-feature-gates). ⭐ THE TELL THAT SEPARATES THIS ROUTE FROM A DYING WIRE, which is what the other seven names mean: flip one boolean in solidRelief.js SOLID_RELIEF_GAME_GATES and the name returns to labVarying with its full body count — demonstrated by tests/world-engine-feature-gates.test.js AC-5, which is the gate that keeps this claim honest. A name here with NO such gate anywhere is still the defect this list is for.   // ⭐⭐ FOUR -> SEVEN ON 2026-09-03 (F3's ray half, workstream wire-ejecta-rays-lab-into-game), AND ALL THREE ARRIVE BY `uTermStrength`'s ROUTE — A COLLAPSED POPULATION — NOT BY A DEAD LAW, A DEAD INPUT OR A DEFERRAL. `uRayCount`/`uRaySharp` are declared CONSTANTS (6 and 8, src/worldengine/base/ejectaRays.js) and were never going to vary; they are `uTerraceCount`'s case exactly. `uRayBrightness` is the one to read carefully: its law is live and its input varies — but ONLY ON PLAIN MOONS, and THIS LEDGER MOUNTS PLANETS ONLY. MEASURED over this pass's own corpus (`lab-procedural-0…199`, 852 planets, 2026-09-03): `rayBrightnessOf` returns exactly 0 on 852 of 852, because every planet the generator makes carries air and the law is `atmosphere ? 0 : 1`. Over the 24-seed rocky corpus WITH its moons the same law returns 0.70–0.94 on 56 bodies with 19 distinct values (tests/driver-pack-ejectarays.test.js AC-2). ⛔ SO A ZERO HERE IS THE POPULATION, NOT A DYING WIRE, and the gate that says so is that suite — a name in this residue whose value is constant on EVERY corpus is the defect this list is for.   // ⭐⭐ AND `uSeaLevel` JOINED ON 2026-09-02 BY THE FOURTH ROUTE — DEFERRAL, not a dead law, not a collapsed population and not a dead input. The lab bake host takes the slot to −1 at mount and the router writes the solved level on the body's first draw (see the note on `labVarying` above). ⛔ HOW TO TELL IT FROM A DYING WIRE, because that is what this list normally means: the write still exists and is gated elsewhere — tests/river-bake-host.test.js AC-7 bakes a wet body and asserts the solved sea lands on this slot. A name in this residue with NO such gate anywhere is the defect this list is for.
  });
  ...
  it('⭐⭐ the CARRIED bucket is a NAME bucket — 24 of 29 diverge in value', () => {
    ...
    expect(everyBody.sort()).toEqual([
      ...
      'uDispDomainScale', 'uNoiseScale', 'uTermStrength',   // ⭐⭐⭐ `uTermStrength` REJOINED THIS LIST ON 2026-09-03 (F35) AND IT IS THE FIRST NAME TO COME BACK TO IT. It left at B3 leg 1 WITH `uTermWidth` when the two packs closed the wire; it returns ALONE, and that asymmetry is the finding. Max's 2026-07-16 ruling — "We need to disable terminator gradient totally; it doesn't work but also this is ultimately something that will need to be rendered in the lighting engine of the main game anyway" — is a ruling about the STRENGTH's GATE, so the world-engine material resolves it to +0 while the LEGACY game material's own producer (src/objects/Planet.js:1653) writes the law's value UNGATED. The width and the colour are written unconditionally on both sides and neither moves, which is the check that this is a gate ruling and not a law change. ⛔ EVERY BODY, NOT MOST — and the cause is this ledger's POPULATION, exactly as it is for `uRayBrightness` on the written-but-not-varying line above: this pass mounts PLANETS ONLY (`lab-procedural-0…199`), every planet the generator makes carries air, and the terminator law is non-zero on every body with air. Over a corpus WITH moons the same divergence is partial — 100 of 156 on the 24-seed rocky corpus (tests/driver-pack-terminator.test.js AC-2). ⛔ DECLARED in the F35 contract, closed only by the lighting engine (F52).
    ].sort());
    ...
    expect(agreeing).toEqual([
      ...
      'uTermColor', 'uTermWidth', 'uTerraceCount',   // ⭐⭐⭐ `uTermStrength` LEFT THIS BUCKET ON 2026-09-03 (F35), AND THE NOTE EIGHT LINES UP ALREADY PREDICTED THE SHAPE OF ITS DEPARTURE: it was the one member here that agreed on a CONSTANT rather than on a live per-body law, which Instrument C's header calls weak green. Max's 2026-07-16 ruling took the world-engine material's copy to +0 while the legacy material's ungated producer (src/objects/Planet.js:1653) kept writing the law, so the weak agreement became a real disagreement — it is now on §2's everyBody list with its reason. ⛔ `uTermWidth` AND `uTermColor` STAY, and that is the check that this is a GATE ruling and not a law change: both are still written unconditionally by both packs, on every body.
...
describe('3. channel 1 — the uniform diff, run not read', () => {
  ...
  it('every measured subject is claimed by exactly ONE ledger row', () => {
    ...
    expect(closed.sort()).toEqual([
      ...
      'uSedColor', 'uTermColor', 'uTermWidth',   // ⭐⭐⭐ `uTermStrength` LEFT THIS LIST ON 2026-09-03 (F35) AND THAT IS THE ONE DIRECTION THIS BLOCK'S OWN NOTE FORBIDS DOING SILENTLY, so it is spelled out: the name is not removed to keep an assertion green, it is removed because it STARTED DIVERGING AGAIN and therefore no longer belongs in a list of names that stopped. Max ruled the band off (2026-07-16) and the ruling reaches the world-engine material through the RULED gate policy while the legacy material's ungated producer at src/objects/Planet.js:1653 keeps writing it — a divergence the F35 contract DECLARES rather than closes, pending the lighting engine (F52). ⛔ P-11's row keeps the name in its subject cell, which is what makes this a re-opening on the record instead of a deletion.
      'uWeatheredColor',
    ].sort());
  });
