...
describe('6a — PACKS is an array with pinned MEMBERSHIP, not a pinned length', () => {
  ...
  it('the membership is exactly the names Step 6 ships', () => {
    ...
    expect(PACKS.map((e) => e.name)).toEqual(['giantDeck', 'limbDeck', 'polarDeck', 'rockySurface', 'solidOptics', 'craterDeck', 'solidFeatures', 'giantSurface', 'fluvialDeck', 'stormDeck', 'solidRelief']);   // ⭐⭐ AN ELEVENTH, 2026-09-04 — `solidRelief` (F1/F4/F5/F6/F8/F15/F16/F18/F19/F21, the 23-name relief deck; workstream solid-relief-deck), predicate `!== 'gas'`, so it joins the SOLID applied list and the GAS skipped list and moves no body between materials.   // ⭐ A TENTH, 2026-09-03 — `stormDeck` (F27/F28, the uStorm* family; workstream wire-storm-slice-lab-into-game), predicate `=== 'gas'`, so it joins the GAS applied list and the SOLID skipped list and moves no body between materials.
  });
...
describe('6a — applyDriverPacks composes the array onto a real lab material', () => {
  ...
  it('a gas body: the deck runs, the master gates are 1.0, the bake is real', () => {
    ...
    expect(res.skipped).toEqual(['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);   // ⭐ `fluvialDeck` joins the SKIPPED list on a gas body, 2026-09-02 — same `!== 'gas'` predicate as the three above it, so a gas body never sees a sea level.
    ...
    expect(res.gates).toEqual({ bands: true, jets: true, limb: true, polarVortex: true, craters: true, ejecta: true, terminator: false, greatSpot: true, stormTrain: true });   // ⭐ +2 GATES 2026-09-03 — `greatSpot` and `stormTrain`, the lab's two storm checkboxes become stormDeck's two registry gates (ALL_ON under the game's policy; workstream wire-storm-slice-lab-into-game).   // ⭐ 2026-09-03 `terminator` RE-POINTED true -> false (F35) — Max 2026-07-16: "We need to disable terminator gradient totally; it doesn't work but also this is ultimately something that will need to be rendered in the lighting engine of the main game anyway." This is `applyDriverPacks`, the GAME's composition point, so it takes `gatesFor`'s default, which is now GATE_POLICY_RULED. Every other gate is still true — the ruling answers ONE name, it does not weaken ruling #4.
  ...
  it('a solid body: EXACTLY the rocky pack applies, and nothing it did not declare moved', () => {
    ...
    expect(res.applied).toEqual(['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);   // ⭐ FOUR PACKS SINCE 2026-09-02 — `fluvialDeck` carries the identical predicate, so the collision guard is live for it too; its ten names are disjoint from the other three's, asserted by name lookup in tests/driver-pack-fluvialdeck.test.js.
    ...
    expect(res.gates).toEqual({ craters: true, ejecta: true, terminator: false, aurora: true, edifices: true, chaos: true, frost: true, glacial: true, deltas: true, coast: true, outflow: true, mountains: false, canyons: false, scarps: false, plateaus: false, tessera: false, lava: false, sublimation: false, karst: false, dunes: false, dust: false, massWasting: true });   // ⭐⭐ +2 ON MAX'S UAT 2026-09-04 — dunes and karst. His words: "Everything still has the dunes drawn across the surface." Their gates (`lowGround * gentle * …`) are smoothsteps DOWN from relief, so turning the other eight off FLATTENED the field they read and opened them to 1 everywhere; only the province floor (0.30 / 0.25) was left. massWasting survives because it reads a RESIDUAL (gradIn - gradBase) which goes to exact zero on flats. Karst was not named by Max — ruled off on the identical mechanism rather than waiting for the next UAT to find it.   // ⭐⭐ 2026-09-04 EIGHT NAMES RE-POINTED true -> false — Max's bar, ruled that day: "grown from the engine, not painted on". These eight read neither the accumulated height nor its gradient (measured per combiner, height.glsl.js), so they are DEVELOPMENT-gated OFF in the GAME until they do; karst, dunes and massWasting read the accumulated surface, clear the bar, and stay ON. This is `applyDriverPacks`, the GAME's composition point, so it reads GAME_GATE_RULINGS. ⭐ THE LAB IS UNAFFECTED — `applyDriverPacksToState` reads GATE_RULINGS (convergence only) and all eleven stay ON there, which is the entire point: the features go on being developed in the lab. ⚠ Note the asymmetry with `terminator` two tests above: that one is a CONVERGENCE ruling and binds BOTH front-ends. Reasons + exit conditions: solidRelief.js SOLID_RELIEF_GAME_GATES; workstream world-engine-feature-gates.   // ⭐ THREE NEW NAMES 2026-09-02 — fluvialDeck's. ⛔ There is NO `lakes` key and that is the pack's own ruling: an off gate resolves to +0 and the lab's no-liquid value is -1, so `uSeaLevel` is emitted ungated (src/worldengine/drivers/fluvialDeck.js header).   // ⭐ 2026-09-03 `terminator` RE-POINTED true -> false (F35) — Max 2026-07-16: "We need to disable terminator gradient totally; it doesn't work but also this is ultimately something that will need to be rendered in the lighting engine of the main game anyway." This is `applyDriverPacks`, the GAME's composition point, so it takes `gatesFor`'s default, which is now GATE_POLICY_RULED. Every other gate is still true — the ruling answers ONE name, it does not weaken ruling #4.
    ...
    // ⭐ FIVE PACKS SINCE 2026-09-04 — `solidRelief` (pack #11, workstream solid-relief-deck), same
    // `!== 'gas'` predicate again, so the union grows once more and the gate keeps its strength for
    // the same stated reason. Its twenty-three names are disjoint from the other four's, measured at
    // the parent in tests/fixtures/solidrelief-pack-drivers-baseline.json and re-asserted by name
    // lookup in tests/driver-pack-solidrelief.test.js.
    const DECLARED = new Set([...ROCKY_SURFACE_UNIFORMS, ...SOLID_OPTICS_UNIFORMS, ...SOLID_FEATURES_UNIFORMS, ...FLUVIAL_DECK_UNIFORMS, ...SOLID_RELIEF_UNIFORMS]);
...
describe('6d — no Sol body reaches the pack path, and the reason is provenance', () => {
  ...
  it('a PLANET-class moon, however, IS covered — main.js stamps the parent system seed', () => {
    // src/main.js:7757 `_systemSeed: systemData.seed,` — so a Sol planet-class moon arrives at
...
describe('6e — ⭐ the flag is ON by default since B7 (was OFF), and either value selects a DIFFERENT material', () => {
  ...
  it('the E caption can print the flag AND its source off the body itself (§12.5 fact 6)', () => {
    ...
    expect(lab.gates).toEqual({ bands: true, jets: true, limb: true, polarVortex: true, craters: true, ejecta: true, terminator: false, greatSpot: true, stormTrain: true });   // ⭐ +2 GATES 2026-09-03 — `greatSpot` and `stormTrain`, the lab's two storm checkboxes become stormDeck's two registry gates (ALL_ON under the game's policy; workstream wire-storm-slice-lab-into-game).   // ⭐ 2026-09-03 `terminator` RE-POINTED true -> false (F35) — Max 2026-07-16: "We need to disable terminator gradient totally; it doesn't work but also this is ultimately something that will need to be rendered in the lighting engine of the main game anyway." This is `applyDriverPacks`, the GAME's composition point, so it takes `gatesFor`'s default, which is now GATE_POLICY_RULED. Every other gate is still true — the ruling answers ONE name, it does not weaken ruling #4.
  ...
  it('⛔ INVERTED AT STEP 10a — a WORLD-ENGINE SOLID planet now SWAPS, and Sol is what stays at zero', () => {
    ...
    expect(lab.packsApplied).toEqual(['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);
    expect(lab.gates).toEqual({ craters: true, ejecta: true, terminator: false, aurora: true, edifices: true, chaos: true, frost: true, glacial: true, deltas: true, coast: true, outflow: true, mountains: false, canyons: false, scarps: false, plateaus: false, tessera: false, lava: false, sublimation: false, karst: false, dunes: false, dust: false, massWasting: true });   // ⭐⭐ +2 ON MAX'S UAT 2026-09-04 — dunes and karst. His words: "Everything still has the dunes drawn across the surface." Their gates (`lowGround * gentle * …`) are smoothsteps DOWN from relief, so turning the other eight off FLATTENED the field they read and opened them to 1 everywhere; only the province floor (0.30 / 0.25) was left. massWasting survives because it reads a RESIDUAL (gradIn - gradBase) which goes to exact zero on flats. Karst was not named by Max — ruled off on the identical mechanism rather than waiting for the next UAT to find it.   // ⭐⭐ 2026-09-04 EIGHT NAMES RE-POINTED true -> false — Max's bar, ruled that day: "grown from the engine, not painted on". These eight read neither the accumulated height nor its gradient (measured per combiner, height.glsl.js), so they are DEVELOPMENT-gated OFF in the GAME until they do; karst, dunes and massWasting read the accumulated surface, clear the bar, and stay ON. This is `applyDriverPacks`, the GAME's composition point, so it reads GAME_GATE_RULINGS. ⭐ THE LAB IS UNAFFECTED — `applyDriverPacksToState` reads GATE_RULINGS (convergence only) and all eleven stay ON there, which is the entire point: the features go on being developed in the lab. ⚠ Note the asymmetry with `terminator` two tests above: that one is a CONVERGENCE ruling and binds BOTH front-ends. Reasons + exit conditions: solidRelief.js SOLID_RELIEF_GAME_GATES; workstream world-engine-feature-gates.   // ⭐ 2026-09-03 `terminator` RE-POINTED true -> false (F35) — Max 2026-07-16: "We need to disable terminator gradient totally; it doesn't work but also this is ultimately something that will need to be rendered in the lighting engine of the main game anyway." This is `applyDriverPacks`, the GAME's composition point, so it takes `gatesFor`'s default, which is now GATE_POLICY_RULED. Every other gate is still true — the ruling answers ONE name, it does not weaken ruling #4.
    expect(labPipelineAdmits(b.d, b.cond).packs).toEqual(['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);
    ...
    for (const x of solSolid) {
      ...
      expect(adm.packs, `${x.id} must be CLAIMED, or this control proves nothing`)
        .toEqual(banded ? ['giantDeck', 'rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief'] : ['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);
    ...
    for (const b of [...GEN_GAS, ...GEN_SOLID]) {
      ...
      if (compositionClass(b.cond) === 'gas') {
        ...
        expect(packs, b.id).toEqual(banded
          ? ['giantDeck', 'rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief'] : ['rockySurface', 'solidOptics', 'solidFeatures', 'fluvialDeck', 'solidRelief']);
