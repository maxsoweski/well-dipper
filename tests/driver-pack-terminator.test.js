...
describe('AC-2 — every other driver of every pack is byte-inert', () => {
  it('deep-compare against the f0b93aa fixture: ZERO drivers other than uTermStrength differ', () => {
    ...
    expect(differ).toEqual([]);
    expect([...added]).toEqual(['solidFeatures.uFrostLatChill']);   // ⭐ ONE DECLARED ADDITION (2026-09-05, workstream frost-budget): uFrostLatChill stopped being a lab knob (shaders/uniforms.js:268 said so outright) and is now DRIVEN per body from atmospheric pressure. Subtracted BY NAME rather than the assertion being loosened — an UNDECLARED new driver still reds, which is the whole point of this gate.
